---
spec_id: admin/hisense-5u7l
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U7L Control Spec"
...
retrieved_at: 2026-06-25T10:15:18.819Z
last_checked_at: 2026-06-25T10:31:04.670Z
generated_at: 2026-06-25T10:31:04.670Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "connection parameters (TCP/IP port, baud rate, parity) are not stated in the provided source fragment. The `Known protocol: TCP/IP` label from the operator conflicts with the hex payloads in the source (which are framed RS-232 ASCII). Transport below is marked serial with no serial config fields populated, awaiting source."
  - "serial config not stated in source - baud_rate, data_bits, parity, stop_bits omitted (not guessed)."
  - "TCP port / IP addressing not stated in source."
  - "source documents only discrete actions / queries, not continuous variables"
  - "source describes request/response only; no unsolicited events documented"
  - "no multi-step sequences described in source"
  - "no safety warnings, interlock procedures, or power-on sequencing requirements in source"
verification:
  verdict: verified
  checked_at: 2026-06-25T10:31:04.670Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 5U7L Control Spec

## Summary
The HiSense 5U7L is a hospitality / commercial display TV exposing an RS-232 serial control interface. This spec covers the full ASCII mnemonic command set documented in the vendor IP-control document, including power, input selection, picture / sound tuning, parental / lock controls, and remote-keypad pass-through.

<!-- UNRESOLVED: connection parameters (TCP/IP port, baud rate, parity) are not stated in the provided source fragment. The `Known protocol: TCP/IP` label from the operator conflicts with the hex payloads in the source (which are framed RS-232 ASCII). Transport below is marked serial with no serial config fields populated, awaiting source. -->

## Transport
```yaml
protocols:
  - serial
# UNRESOLVED: serial config not stated in source - baud_rate, data_bits, parity, stop_bits omitted (not guessed).
# UNRESOLVED: TCP port / IP addressing not stated in source.
auth:
...
- levelable  # inferred: volume / brightness / contrast / etc. control present
```
...
## Variables
```yaml
# UNRESOLVED: source documents only discrete actions / queries, not continuous variables
```

## Events
```yaml
# UNRESOLVED: source describes request/response only; no unsolicited events documented
```
...
interlocks: []
# UNRESOLVED: no safety warnings, interlock procedures, or power-on sequencing requirements in source
```

## Notes
- Two hex variants are documented per command: a "TV Specific" form and a "Generic HEX Command (all TVs)" form. The TV-specific form carries the last three digits of the unit's MAC address in the ASCII portion (e.g. `53 34 36 35` = ASCII `S465` for MAC `...465`); the generic form uses `SALL` (`53 41 4C 4C`). Each frame ends in `0D` (carriage return). The penultimate byte is a checksum.
- The 159-action / 37-feedback deterministic catalogue is the authoritative command list; this spec does not re-author them inline.
- Frame format (inferred from header bytes, not stated explicitly): 0x53 'S' prefix, 3-byte address/marker, 4-byte ASCII mnemonic, 4-byte parameter, 1-byte checksum, 0x0D terminator. This inference is for note purposes only and is not asserted in the Transport block.
- Known-protocol label `TCP/IP` from the operator does not match the source, which is RS-232 hex framed on a serial line. Transport therefore defaults to `serial` and TCP fields are left UNRESOLVED rather than guessed.
