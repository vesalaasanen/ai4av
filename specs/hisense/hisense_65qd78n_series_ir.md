---
spec_id: admin/hisense-65qd78n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 65QD78N Series Control Spec"
manufacturer: HiSense
model_family: "65QD78N Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T12:00:59.943Z
last_checked_at: 2026-06-25T16:16:31.585Z
generated_at: 2026-06-25T16:16:31.585Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "serial electrical config (baud/data/parity/stop/flow) stripped from refined doc — original source table only carried the command catalogue."
  ...
  - "remove if pre-pass covers settable params."
  - "no unsolicited-notification semantics documented in source."
  - "no multi-step sequences documented in source."
  - "source carries no safety warnings, interlock procedures, or"
  - "physical connector (DB-9 / RJ-45 / 3.5mm TRS) not in refined source."
  - "cable pinout / null-modem vs straight-through not in refined source."
  - "checksum algorithm not documented — bytes appear arbitrary per row."
  - "response framing/timeout not documented."
  - "BTTN keycode acknowledgement behaviour (returns OKAY but no IR-code mapping) not documented."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:16:31.585Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (15 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 65QD78N Series Control Spec

## Summary
HiSense 65QD78N Series LED/LCD television with RS-232 serial control. Spec covers power, input selection, picture/audio parameters, volume/mute, tuner, channel, caption, OSD, remote-keycode emulation, hotel/commercial lock settings, and accompanying status queries. Commands are ASCII mnemonics embedded in a hex frame.

<!-- UNRESOLVED: serial electrical config (baud/data/parity/stop/flow) stripped from refined doc — original source table only carried the command catalogue. -->
...
Frame format (from source): each command is hex byte sequence. Prefix `53 34 36 35` (= ASCII "S465", TV-specific using MAC last-3-digits) OR `53 41 4C 4C` (= ASCII "SALL", generic broadcast). Body = ASCII mnemonic + 4-digit parameter (e.g. `50 57 52 30 30 30 31` = "POWR0001"). Followed by single checksum byte, terminator `0D` (CR). Query frames use prefix `51 34 36 35` / `51 41 4C 4C` (= ASCII "Q465"/"QALL"). Checksum algorithm not stated in source. `????` placeholder marks parameter position in query/response rows.

## Traits
```yaml
- powerable    # POWR0000/0001 standby + power on; PWRE power-on-enable
- routable     # INPT input select (TV/AV/Component/HDMI1-4/VGA); POIS power-on input
- queryable    # 35 query mnemonics (???? forms) return state
- levelable    # VOLM, BRIT, CONT, COLR, TINT, SHRP, BKLV, MAVL, VLFL level set
```
...
## Variables
```yaml
# Level-set mnemonics with continuous ranges are emitted as parameterized Actions
# by the pre-pass (VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV/MAVL/VLFL). No separate
# Variables beyond those. UNRESOLVED: remove if pre-pass covers settable params.
```

## Events
```yaml
# UNRESOLVED: no unsolicited-notification semantics documented in source.
```
...
interlocks: []
# UNRESOLVED: source carries no safety warnings, interlock procedures, or
# power-on sequencing requirements. RSET9999 (factory reset) and PWRE (RS-232
# remote boot) exist as commands but carry no stated guard procedure.
```

## Notes
- Commands ship in two addressable flavours: TV-specific (`S465`/`Q465`, keyed to MAC last-3-digits, e.g. "465") and generic broadcast (`SALL`/`QALL` to all TVs on bus). Use generic form unless single-device targeting required.
- `BTTN####` rows emulate IR remote keycodes (digits 0-9, transport UP/DOWN/LEFT/RIGHT/OK, transport PLAY/PAUSE/STOP/FFW/FRW/NEXT/PREV, colour keys, MENU/EXIT/BACK, INPUT, MUTE, SLEEP, MTS/SAP, LiveTV, HiSmart, HiMedia, POWER, CH+/CH-, VOL+/VOL-, DASH, CC). Pure keypress emulation — no state return.
- Query responses all share identical hex body regardless of current value (mnemonic + `????`); discriminator is the ASCII decimal in the "Parameter/Return Value" column. Implementer must parse returned decimal.
- Several source rows show checksum byte `0D` reused as payload end-byte where arithmetic overflowed (e.g. INPT0006 body `0D` coincides with terminator) — present verbatim, do not "fix".
- `POWR0000` label is "Stand by" (not full power-off); true AC-off is via `PBTN0000` AC-only power-off control mode.
- Source is refined excerpt (command table only); full manual's connection/connector/pinout section not in this artifact.
<!-- UNRESOLVED: physical connector (DB-9 / RJ-45 / 3.5mm TRS) not in refined source. -->
<!-- UNRESOLVED: cable pinout / null-modem vs straight-through not in refined source. -->
<!-- UNRESOLVED: checksum algorithm not documented — bytes appear arbitrary per row. -->
<!-- UNRESOLVED: response framing/timeout not documented. -->
<!-- UNRESOLVED: BTTN keycode acknowledgement behaviour (returns OKAY but no IR-code mapping) not documented. -->
