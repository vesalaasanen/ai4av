---
spec_id: admin/hisense-55u78k-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U78K Series Control Spec"
manufacturer: HiSense
model_family: "55U78K Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-04T08:39:35.254Z
last_checked_at: 2026-06-25T10:31:03.755Z
generated_at: 2026-06-25T10:31:03.755Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source describes RS-232/ASCII framing but does not explicitly state the transport (TCP port, baud). Treated as IP control per input hint; see Transport notes."
...
# HiSense 55U78K Series Control Spec

## Summary
IP-controllable HiSense 55U78K Series television. Spec covers the RS-232/ASCII command set exposed over TCP/IP — power, input selection, picture/sound modes, OSD, lock features, remote/panel key, power-on defaults, USB behavior, pixel shift. Each command has a four-letter ASCII mnemonic plus a 4-digit parameter, framed by a 4-byte preamble and 2-byte trailer (HEX byte 0x0D) with checksum.

<!-- UNRESOLVED: source describes RS-232/ASCII framing but does not explicitly state the transport (TCP port, baud). Treated as IP control per input hint; see Transport notes. -->

## Transport
```yaml
# Source describes RS-232-style framed ASCII commands. Per input hint, device
# exposes these over TCP/IP. Source does NOT state port, baud, or framing rules.
protocols:
  - tcp
  - serial
# Per-source framing: every command row is a 14-byte frame:
#   [SOF=0x53 'S'][DEST 3 ASCII bytes][CMD 4 ASCII][PARAM 4 ASCII][CHK][0x0D]
# DEST = MAC-last3 ASCII for per-TV targeting, or "ALL" for broadcast.
# CHK = XOR checksum of bytes 0..11 (decimal) excluding SOF and trailer.
# This is a TCP wrapper around an RS-232-style protocol; per-source.
addressing:
...
## Traits
```yaml
powerable:    # POWR0000/0001 commands present
routable:     # INPT input-select commands present
queryable:    # every command has a ?-form query
levelable:    # brightness/contrast/color/tint/sharpness/volume/backlight ranges
```
...
## Variables
```yaml
# Continuous settable parameters, range per source:
- id: brightness
  type: integer
  range: [0, 100]
  command_template: "BRIT{value}"
- id: contrast
  type: integer
  range: [0, 100]
  command_template: "CONT{value}"
- id: color_saturation
  type: integer
  range: [0, 100]
  command_template: "COLR{value}"
- id: tint
  type: integer
  range: [0, 100]
  command_template: "TINT{value}"
- id: sharpness
  type: integer
  range: [0, 20]
  command_template: "SHRP{value}"
- id: volume
  type: integer
  range: [0, 100]
  command_template: "VOLM{value}"
- id: backlight
  type: integer
  range: [0, 100]
  command_template: "BKLV{value}"
- id: max_volume_range
  type: integer
  range: [0, 100]
  command_template: "MAVL{value}"
- id: volume_locked_level
  type: integer
  range: [0, 100]
  command_template: "VLFL{value}"
```
...
## Safety
```yaml
confirmation_required_for: []  # UNRESOLVED: no safety interlock text in source
interlocks: []                 # UNRESOLVED: no interlock text in source
```

## Notes
- Protocol wraps a 14-byte RS-232-style frame on top of TCP/IP. Frame layout per source: `0x53` SOF + 3-byte destination (MAC-last3 ASCII, or `SALL`/literal `ALL` for broadcast) + 4-byte command mnemonic + 4-byte parameter + 1-byte XOR checksum + `0x0D` trailer. Checksum computed over bytes 0–11 of the frame (per-source column labels show this byte as the second-to-last of the 14-byte row).
- Source shows TWO hex columns: "TV-specific" (per-MAC targeted) and "Generic" (broadcast `SALL`). Both encode the same logical command. Pre-pass actions carry one of the two — operators should confirm whether the implementation sends broadcast or per-MAC frames.
- `PWRE` query form returns the same byte pattern for both `0` (disable) and `1` (enable) responses in the source — the per-response return column is identical between rows 5 and 6. Likely a transcription artefact in the upstream PDF; flagged for verify.
- `INPT0000` ("Change Input Signal One at a Time") is a toggle, not a discrete input.
- Button-codes (`BTTN####`) duplicate every remote-control key as a command; the deterministic pre-pass enumerated all of them.
- Source title row references "RS-232 Remote Power On" / "RS-232 Remote Boot" in the `PWRE` command descriptions — implies the framing is the same as the legacy RS-232 control port, only re-served over TCP.
<!-- UNRESOLVED: TCP port number, baud rate, serial config — none stated in source. Inferred protocols: [tcp, serial] per the framing language + the input hint; treat as TCP wrapper until verified. -->
