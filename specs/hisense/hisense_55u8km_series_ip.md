---
spec_id: admin/hisense-55u8km-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U8KM Series Control Spec"
...
retrieved_at: 2026-05-14T10:38:46.139Z
last_checked_at: 2026-06-25T16:56:43.396Z
generated_at: 2026-06-25T16:56:43.396Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source documents no TCP/IP transport details (port, addressing, framing over IP) despite entity/file naming suggesting IP. All transport config below is serial only."
  - "no TCP/IP port or base URL in source. Header labelled protocol \"TCP/IP\" but the refined document contains only RS-232C serial framing; do not assume a TCP port."
...
# HiSense 55U8KM Series Control Spec

## Summary
HiSense Prosumer TV (55U8KM series) RS-232C serial control spec. Covers power, input selection, picture/audio parameters, tuner, captions, remote-key simulation, and hotel/custom-install lock features via fixed-length ASCII command frames over a DB9 D-sub serial port. Source is vendor RS-232/IR protocol manual; discrete IR Pronto CCF codes also documented but out of scope for this serial spec.

<!-- UNRESOLVED: source documents no TCP/IP transport details (port, addressing, framing over IP) despite entity/file naming suggesting IP. All transport config below is serial only. -->
...
serial:
  ...
  stop_bits: 1
  flow_control: none
  code: ascii  # Communication Code: ASCII (per source)
  connector: DB9_D-sub_female_chassis  # female chassis mount on TV
  pinout:
    male_9pin: [N/C, RXD, TXD, DTR, GND, DSR, RTS, CTS, Power_Input]
    female_9pin: [RI, TXD, RXD, DSR, GND, DTR, CTS, RTS, Power_Input_DCD]
  frame:
    direction_byte: 1   # S = Set, Q = Query
    client_id: 3         # last 3 bytes of Ethernet MAC (Smart TV) OR menu-selected ID (Feature TV); "ALL" = broadcast
    command: 4           # A-Z mnemonic
    data: 4              # 0-9,A-Z,#,?
    checksum: 1          # 8-bit; sum of whole string incl checksum byte = 0
    termination: 0x0D    # carriage return
  ack:
    client_id: 3
    colon: 0x3A
    ack: 1               # OKAY / EROR / WAIT
    data: 4
    checksum: 1
    termination: 0x0D
auth:
...
<!-- UNRESOLVED: no TCP/IP port or base URL in source. Header labelled protocol "TCP/IP" but the refined document contains only RS-232C serial framing; do not assume a TCP port. -->

## Traits
```yaml
- powerable    # inferred: PWRE/POWR power on/off + standby commands present
- queryable    # inferred: extensive ???? query commands returning state (INPT/PMOD/VOLM/MUTE/etc.)
- levelable    # inferred: BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM continuous value set commands
- routable     # inferred: INPT input-source selection commands
```
...
## Variables
```yaml
# Continuous settable parameters (brightness/contrast/color/tint/sharpness/backlight/volume/
# volume-range/volume-locked-level) are emitted as discrete-value actions by the deterministic
# pre-pass per source row granularity. No additional variable entries beyond those actions.
```

## Events
```yaml
# Source documents no unsolicited notifications. TV only responds to Set/Query with ACK frames.
...
interlocks:
  - pwre_query_not_available_in_standby  # source note: PWRE query unavailable in STANDBY mode
  - custom_install_menu_must_be_enabled  # RS-232 port must be Enabled in Custom Install menu (access code 7310) before serial control works
  - power_on_command_setting_enables_standby_remote_power  # PWRE=1 required for RS-232 remote power-on while TV in standby
# No explicit power-on sequencing warnings beyond Custom Install enablement.
```

## Notes
- Protocol is **case sensitive** (source note 1). All mnemonics uppercase.
- Command frames carry a per-TV client ID = last 3 hex bytes of Ethernet MAC (Smart TV) or menu-selected ID (Feature TV). "ALL" broadcasts to every TV on a multi-TV serial bus.
- Two hex-encoding columns per source row: TV-specific (MAC-suffix client id) and generic "ALL" broadcast. Deterministic actions use the generic/ALL form for cross-unit applicability.
- ACK sequence for power-on may return WAIT then OKAY (see source example: POWRON → WAIT → OKAY).
- Discrete IR (Pronto CCF) codes for power/input/digits/arrows/PIP/etc. are documented in the same source file but belong to an IR control spec, not this serial spec.
- Source protocol-manual revisions span V1.0 (2014) through V3.6 (2017); 55U8KM is a current-series mapping onto this long-standing command set.

<!-- UNRESOLVED: source documents no TCP/IP transport (no port, no base URL, no IP framing). Entity/file named "_ip" but refined text is RS-232/IR only. Verify whether a separate IP-control document exists before publishing. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: exact 55U8KM model-row in source "Models" table is blank in the refined extract; model name taken from device header. -->
