---
spec_id: admin/hisense-5u98n
schema_version: ai4av-public-spec-v1
revision: 1
title: "Hisense 5U98N Control Spec"
...
retrieved_at: 2026-04-26T16:18:45.183Z
last_checked_at: 2026-06-25T17:49:53.143Z
generated_at: 2026-06-25T17:49:53.143Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source does not explicitly name the 5U98N as a supported model (the \"Models\" field in the source is blank)."
  - "no TCP/IP / network control content present in source despite input context assuming it."
  - "firmware version compatibility not stated in source."
  - "none beyond the merged actions."
  - "source documents no unsolicited / push notifications from the TV."
  - "source documents no multi-step command sequences."
  - "5U98N not explicitly listed as supported in source."
  - "no IP/network control material in source (TCP/IP assumption unverified)."
  - "COVERAGE GAP — the source's final command-table region is a malformed text blob (lines ~427+) listing additional mnemonics not present in the deterministic 137-action pre-pass set: OSD# (OSD Mode enable/disable + query), INPM (Input Mode LOCKED/SELECTABLE/AC RESET/STANDBY RESET + query), POIS (Power-On Input Select LAST/Air/AV/Component), and a truncated \"POWER ON INPUT SELE...\" row. If the deterministic merge does not capture these, the spec is under-enumerated and the verifier should flag/re-amend them."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:53.143Z
  matched_actions: 165
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (9 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# Hisense 5U98N Control Spec

## Summary
Serial (RS-232C) and discrete-IR control protocol for the Hisense 5U98N Prosumer TV. The unit is driven over a DB9 serial link at 9600 8N1 using fixed-length ASCII command/ack frames with a per-frame checksum. A large command catalogue covers power, input selection, picture/audio settings, volume, tuner, channel, caption, OSD, lock/key, and remote-button-simulation functions, most with paired set and query (`????`) forms. Discrete IR codes (Pronto CCF) are also documented.

This spec was prepared for the 5U98N from the refined vendor protocol document. The source is a generic **Hisense Prosumer TV "RS-232/IR Protocol"** guide (doc revisions V1.0–V3.6); the 5U98N is **not explicitly named** in the source, so command availability should be confirmed against the target unit. Note: the input context assumed **TCP/IP** control, but the actual source document contains **no IP/network control material** — it is exclusively RS-232 + IR. The transport below reflects the source as written.

<!-- UNRESOLVED: source does not explicitly name the 5U98N as a supported model (the "Models" field in the source is blank). -->
<!-- UNRESOLVED: no TCP/IP / network control content present in source despite input context assuming it. -->
<!-- UNRESOLVED: firmware version compatibility not stated in source. -->

## Transport
```yaml
# Source documents RS-232C serial control only (plus discrete IR, which is not a
# connection-based transport). No TCP/IP/HTTP/UDP/OSC content exists in the source.
protocols:
  - serial
serial:
  ...
  flow_control: none     # "Flow Control None" - communication code: ASCII
auth:
  type: none  # inferred: no login / password / auth procedure described in source
```

Connection / framing notes (from source):
- **Connector:** female DB9 D-sub chassis mount on the TV. Male 9-pin pinout: 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR, 7 RTS, 8 CTS, 9 Power Input. USB-to-serial adapter required for PC/host without a DB9 port.
- **Frame format (fixed length):** `OP(1) CLIENT_ID(3) COMMAND(4) DATA(4) CHECKSUM(1) CR(1)`.
  - `OP` = `S` (Set) or `Q` (Query).
  - `CLIENT_ID` = last 3 hex digits of the TV's Ethernet MAC address (Smart TV), or a value selected in the TV menu (Feature TV); `ALL` = broadcast to every TV on the bus.
  - `CHECKSUM` = 8-bit checksum such that the whole byte sequence **including the checksum byte** sums to zero.
  - `TERMINATION` = Carriage Return (`0x0D`).
- **Acknowledgement frame:** `CLIENT_ID(3) ':' ACK(4) DATA(4) CHECKSUM(1) CR(1)`. Common ACK tokens: `OKAY`, `EROR`, `WAIT`.
- **Case sensitivity:** the protocol is **case sensitive** (source note 1).
- **Dual encoding:** every command is shown in both ASCII and HEX forms; a generic (broadcast) HEX variant uses client ID `ALL`.

## Traits
```yaml
traits:
  - powerable   # inferred: POWR power on/off + PWRE remote-power-on commands present
  - queryable   # inferred: extensive ???? state-query commands present
  - routable    # inferred: INPT input-source selection present
  - levelable   # inferred: VOLM / BRIT / CONT / COLR / BKLV level controls present
```
...
## Variables
```yaml
# No settable parameter outside the discrete actions above is documented as a
# standalone variable. Level-type values (brightness, contrast, volume, backlight,
# sharpness, tint, colour, volume range, volume-locked level) are expressed as
# parameterised set/query actions in the merged action set.
# UNRESOLVED: none beyond the merged actions.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited / push notifications from the TV.
# All device output is in response to a command (synchronous ACK/query reply).
```
...
interlocks: []
# Source contains no explicit safety warnings or hardware interlocks.
# Operational prerequisite (setup, not an interlock): the RS-232 port must be
# enabled in the TV's Custom Install menu before serial control works - power on
# the TV, open Quick Settings, enter numeric "7 3 1 0", then set
# "Custom Installation" → Enable. For RS-232 power-on while the TV is in
# standby, also set "Power On Command" → Enable before exiting that menu.
# These are never inferred as safety items; stated verbatim from source text.
```

## Notes
- **Source ≠ TCP/IP:** the refined source is the Hisense Prosumer TV *RS-232/IR Protocol* guide (revisions V1.0–V3.6, latest V3.6 17-Apr-2017). It contains **no IP/TCP/REST/UDP control content**. The `protocols: [serial]` choice above reflects the source; the input-context assumption of TCP/IP could not be substantiated from this document.
- **Model coverage:** the source's "Models" field is blank and the 5U98N is not named; revision notes only mention "Supported Series model name change". Treat command availability as series-level and verify on the 5U98N.
- **RS-232 enablement required:** serial control is inert until the Custom Install menu enables the port (Quick Settings → `7 3 1 0` → Custom Installation → Enable). RS-232 wake-from-standby additionally needs "Power On Command" → Enable.
- **Bus addressing:** multiple TVs can share the bus; target an individual set by MAC-tail client ID, or use `ALL` to broadcast. Query on a single-TV system vs MAC-specific query on a multi-TV system are both supported (V3.2).
- **Checksum:** implement as the 8-bit value that makes the full frame (checksum byte included) sum to zero — do not omit it; the merged HEX payloads carry it verbatim.
- **Case sensitivity:** all mnemonics are case sensitive.
- **IR:** the source also lists discrete IR codes (Pronto CCF / hex) for power, inputs, picture/sound/aspect, digits, navigation, transport, PIP, etc. These are not modelled as transport actions here (IR is not a connection-based control); consult the source tables for IR payloads.

<!-- UNRESOLVED: firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: 5U98N not explicitly listed as supported in source. -->
<!-- UNRESOLVED: no IP/network control material in source (TCP/IP assumption unverified). -->
<!-- UNRESOLVED: COVERAGE GAP — the source's final command-table region is a malformed text blob (lines ~427+) listing additional mnemonics not present in the deterministic 137-action pre-pass set: OSD# (OSD Mode enable/disable + query), INPM (Input Mode LOCKED/SELECTABLE/AC RESET/STANDBY RESET + query), POIS (Power-On Input Select LAST/Air/AV/Component), and a truncated "POWER ON INPUT SELE..." row. If the deterministic merge does not capture these, the spec is under-enumerated and the verifier should flag/re-amend them. -->
