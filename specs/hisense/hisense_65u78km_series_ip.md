---
spec_id: admin/hisense-65u78km-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 65U78KM Series Control Spec"
manufacturer: HiSense
model_family: "65U78KM Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-02T01:43:54.405Z
last_checked_at: 2026-06-25T16:16:30.611Z
generated_at: 2026-06-25T16:16:30.611Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source document is a command table only — no network port, no connection/baud details, no firmware compatibility, no safety/interlock procedures, no variable/unsolicited-event documentation."
  - "port number not stated in source"
  - "no standalone settable-parameter (continuous-variable) documentation"
  ...
  - "response/ack frame format not documented (only \"OKAY\" strings shown)."
  - "command inter-command timing / pacing not documented."
  - "maximum connection lifetime / idle disconnect behaviour not documented."
  - "whether multiple commands can be pipelined in one TCP segment not documented."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:16:30.611Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 65U78KM Series Control Spec

## Summary
The HiSense 65U78KM Series is a commercial/hospitality-grade LED TV controllable over a TCP/IP link using a binary ASCII-hex command protocol. This spec covers power, input/source selection, picture and audio adjustments, tuner/channel control, OSD/caption settings, remote & panel key lock states, and hospitality (B2B) configuration. Every command is transmitted as a hex byte sequence; each setting/query carries a device-addressing prefix, a 4-character ASCII mnemonic plus a 4-digit parameter, and a trailing checksum + `0D` terminator.

<!-- UNRESOLVED: source document is a command table only — no network port, no connection/baud details, no firmware compatibility, no safety/interlock procedures, no variable/unsolicited-event documentation. -->

## Transport
```yaml
protocols:
  - tcp  # inferred from "TCP/IP" known protocol + IP-control source filename
addressing:
  port: null  # UNRESOLVED: port number not stated in source
auth:
  type: none  # inferred: no auth procedure in source
# No serial block emitted - transport is TCP/IP. The string "RS-232 Remote
# Power On" appears in one command's description (PWRE) but names a TV feature,
# not the transport.
```
...
```yaml
traits:
  - powerable   # inferred from POWR/PWRE power on/standby commands
  - routable    # inferred from INPT input-source selection commands
  - levelable   # inferred from VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV level-set commands
  - queryable   # inferred from extensive ???? query (status-read) commands
```
...
## Variables
```yaml
# UNRESOLVED: no standalone settable-parameter (continuous-variable) documentation
# in source beyond the level-set actions already enumerated under Actions.
```
...
# power-on sequencing requirements. Note RSET9999 performs a factory reset and
# should be treated with care by integrators, but no explicit interlock is
# documented in the source - never infer one.
```

## Notes
**Command framing (implementable detail, verbatim from source structure):**
Each hex payload is structured as:
`<addr-prefix> <mnemonic[4]> <param[4]> <checksum> 0D`
- **Address prefix — TV-specific:** `53 34 36 35` (ASCII `S465`) where `465` represents the last three digits of the target TV's MAC address (example given in source header).
- **Address prefix — broadcast / generic:** `53 41 4C 4C` (ASCII `SALL`) applies to all TVs regardless of MAC.
- **Query variant prefixes:** `51 34 36 35` (`Q465`) and `51 41 4C 4C` (`QALL`) for the same two targeting modes.
- **Terminator:** every command ends with `0D` (carriage return).
- **Checksum:** a single byte precedes `0D`; its value differs between the TV-specific and generic variants for the same mnemonic/param (e.g. PWRE0000 → `10` specific vs `D6` generic). The checksum algorithm is not documented in the source — use the literal byte values shown in the source table verbatim. <!-- UNRESOLVED: checksum computation algorithm not documented -->

**Mnemonic families covered:** `PWRE` (remote power-on enable), `POWR` (power on/standby), `INPT` (input select), `PMOD`/`BRIT`/`CONT`/`COLR`/`TINT`/`SHRP`/`ASPT`/`OVSN`/`CTEM`/`BKLV`/`RSTP` (picture), `AMOD`/`VOLM`/`MUTE`/`ASPK`/`RSTA` (audio), `TUNR`/`TSCN`/`CHAN` (tuner/channel), `CC##` (caption), `RSET` (factory reset), `LANG` (OSD language), `PLED` (standby LED), `BTTN####` (IR/remote key emulation, 0–1055 range), `PBTN` (power-off control mode), `MAVL`/`SVOL`/`VLFL` (volume lock/range), `RMOT`/`PANL`/`MENU`/`AVMN`/`OSD#`/`INPM` (key & menu lock), `POIS` (power-on input), `SPKM` (speaker/ARC mode), `B2BM` (B2B/hospitality mode), `USBM` (USB behavior), `PSHF` (pixel shifting).

**Status queries:** every settings family has a matching `????` query (prefix `51…`) returning a numeric/`OKAY` value as documented in the Feedbacks section. Responses are not shown in the source beyond the `Parameter/Return Value` column.

<!-- UNRESOLVED: response/ack frame format not documented (only "OKAY" strings shown). -->
<!-- UNRESOLVED: command inter-command timing / pacing not documented. -->
<!-- UNRESOLVED: maximum connection lifetime / idle disconnect behaviour not documented. -->
<!-- UNRESOLVED: whether multiple commands can be pipelined in one TCP segment not documented. -->
