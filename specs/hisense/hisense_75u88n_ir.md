---
spec_id: admin/hisense-75u88n
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 75U88N Control Spec"
...
retrieved_at: 2026-06-25T12:02:57.036Z
last_checked_at: 2026-06-25T16:16:32.531Z
generated_at: 2026-06-25T16:16:32.531Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source contains only the command table; no electrical, framing, or connection-setup prose. Serial line parameters (baud/data/parity/stop) are not documented."
  - "source documents no serial line parameters (baud_rate, data_bits,"
  - "not stated in source"
  - "source documents no unsolicited notifications."
  - "source documents no multi-step sequences."
  - "source contains no safety warnings, interlock procedures, or"
  - "baud rate, data bits, parity, stop bits, flow control not stated."
  - "checksum algorithm not documented — must copy verbatim bytes per row."
  - "firmware version compatibility not stated."
  - "no response framing/timeout/error-recovery behavior documented."
  - "no MAC-address retrieval procedure documented (needed to build TV-specific frames)."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:16:32.531Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 75U88N Control Spec

## Summary
HiSense 75U88N is a consumer television controlled over RS-232 serial. This spec covers power, input selection, picture/audio settings, channel/tuner, OSD, and hospitality (B2B) controls, plus remote-key emulation via the BTTN mnemonics. Each operation has a 4-character ASCII mnemonic with a 4-digit parameter and a trailing checksum byte.

<!-- UNRESOLVED: source contains only the command table; no electrical, framing, or connection-setup prose. Serial line parameters (baud/data/parity/stop) are not documented. -->

## Transport
```yaml
protocols:
  - serial
# UNRESOLVED: source documents no serial line parameters (baud_rate, data_bits,
# parity, stop_bits, flow_control). Do not assume defaults.
serial:
...
## Traits
```yaml
# - powerable    (POWR/PWRE power on/standby + RS-232 remote boot enable)
# - routable     (INPT input selection across TV/AV/Component/HDMI1-4/VGA)
# - queryable    (every mnemonic accepts ???? query form returning a value)
# - levelable    (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL set absolute levels)
```
...
## Variables
```yaml
# No continuous settable variables beyond the levelable actions already enumerated
# in Actions (brightness/contrast/color/tint/sharpness/backlight/volume).
```
...
# power-on sequencing requirements. Restore-factory (RSET9999) and power
# commands exist but carry no documented interlock.
```

## Notes
- **Frame format.** Each command is transmitted as ASCII hex bytes. Two addressing prefixes exist:
  - TV-specific (MAC-keyed): `53 34 36 35 ...` = `"S465..."` where `465` is the last three digits of the TV's MAC address (example value; real suffix varies per unit).
  - Generic (all-TVs): `53 41 4C 4C ...` = `"SALL..."`.
- **Query frames.** Query commands swap the leading `S` (0x53) for `Q` (0x51), e.g. `51 34 36 35 ...` = `"Q465..."` / `51 41 4C 4C ...` = `"QALL..."`.
- **Body.** After the prefix, frames carry a 4-character mnemonic (PWRE, POWR, INPT, PMOD, BRIT, etc.) and a 4-digit ASCII parameter (`0000`, `0001`, …, or `????` for query). Set commands echo `OKAY` (or `OKAY/ON`, `OKAY/OFF`); queries return the 4-digit value or an enum index.
- **Checksum.** A single checksum byte precedes the terminating `0D` (CR). Its algorithm is not stated in the source; verbatim per-row values from the table must be used. For TV-specific frames the checksum depends on the MAC suffix, so the generic `SALL`/`QALL` form is portable across units.
- **Recommended portable form.** Prefer the `SALL`/`QALL` (Generic HEX) column so a single command set works across any 75U88N without per-unit MAC lookup.
- **CC## mnemonic.** Caption-control rows use the literal `CC##` (0x43 0x43 0x23 0x23); the `##` are literal ASCII hash characters, not placeholders.
- **OSD# mnemonic.** OSD-mode rows likewise use a literal `OSD#` (0x4F 0x53 0x44 0x23).
- **Mixed-language labels.** Some rows are authored in Chinese in the source (`频道切换` = channel switch); meaning is preserved in the English "Command/Return Value Explanation" column.
- **Duplicate query rows.** Several query mnemonics list multiple return-value rows (e.g. `INPT????` returns 1/3/4/6/9/10/11/12 depending on current input) but share one query frame; the deterministic extractor emitted these as feedback values.

<!-- UNRESOLVED: baud rate, data bits, parity, stop bits, flow control not stated. -->
<!-- UNRESOLVED: checksum algorithm not documented — must copy verbatim bytes per row. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: no response framing/timeout/error-recovery behavior documented. -->
<!-- UNRESOLVED: no MAC-address retrieval procedure documented (needed to build TV-specific frames). -->
