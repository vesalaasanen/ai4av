---
spec_id: admin/hisense-65u86lua
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 65U86LUA Control Spec"
manufacturer: HiSense
model_family: 65U86LUA
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T18:15:09.540Z
last_checked_at: 2026-06-25T18:30:53.050Z
generated_at: 2026-06-25T18:30:53.050Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "IR discrete codes are documented as Pronto CCF/hex tables but have no `ir` transport in the AI4AV enum; not represented as Actions."
  - "source \"Models\" section is blank; model 65U86LUA taken from the input device name, not from the source table."
  - "no settable parameters exist outside the discrete/parameterized"
  - "source documents no unsolicited notifications. The WAIT ACK is a"
  - "source documents no named multi-step command sequences."
  - "source contains no safety warnings, interlock procedures, or"
  - "source \"Models\" section is blank; cannot confirm 65U86LUA against a published model list."
...
# HiSense 65U86LUA Control Spec

## Summary
The HiSense 65U86LUA is a prosumer 4K UHD television controllable over an RS-232C serial interface (DB9 D-sub) and by discrete IR codes (Pronto CCF). This spec covers the ASCII fixed-length serial command/query protocol documented in the Hisense Prosumer TV RS-232/IR protocol reference (revision V3.6, 17-Apr-2017): power, input selection, picture/audio parameters, channel, caption, OSD, and installation-lock commands, plus their status-query counterparts. Discrete IR code tables (CCF/hex) are referenced but not enumerated as actions here, as the AI4AV protocol enum has no `ir` transport.

<!-- UNRESOLVED: IR discrete codes are documented as Pronto CCF/hex tables but have no `ir` transport in the AI4AV enum; not represented as Actions. -->
<!-- UNRESOLVED: source "Models" section is blank; model 65U86LUA taken from the input device name, not from the source table. -->

## Transport
```yaml
protocols:
  - serial
# Communication code: ASCII (documented). Frame is fixed-length (see Notes).
# No IP/HTTP/UDP/OSC transport documented.
serial:
...
# Physical connector: standard DB9 D-sub, female chassis mount on TV.
# Pinout (Male D-sub 9-pin): 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR,
#   7 RTS, 8 CTS, 9 Power Input.
# USB-to-Serial adapter required to connect from a USB host (sold separately).
# addressing/ port: N/A (serial-only device; no TCP port documented).
```

## Traits
```yaml
# All traits inferred (Tier 2) from command examples present in source:
- powerable   # inferred: POWR0000/0001 power standby/on commands present
- routable    # inferred: INPT input-source selection commands present
- queryable   # inferred: extensive ???? status-query commands present
- levelable   # inferred: VOLM/BRIT/CONT/BKLV/COLR/TINT/SHRP set commands present
```
...
## Variables
```yaml
# UNRESOLVED: no settable parameters exist outside the discrete/parameterized
# Actions already enumerated (VOLM, BRIT, CONT, BKLV, COLR, TINT, SHRP, MAVL,
# VLFL are all carried as Actions with command payloads). Nothing to populate.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited notifications. The WAIT ACK is a
# solicited interim response to a Set command, not an unsolicited event.
```
...
# power-on sequencing requirements. "Restore Factory Settings" (RSET9999) is a
# destructive reset but the source lists no confirmation/interlock gating.
```

## Notes

**Protocol framing (fixed-length ASCII).** Every transmission and acknowledgement is a fixed-length ASCII string terminated by Carriage Return (0x0D). The protocol is **case-sensitive** (documented). Frame layout:

- Command (PC/Controller): `OP DIR(1B) | CLIENT ID(3B) | COMMAND(4B) | DATA(4B) | CHECKSUM(1B) | TERM(1B=CR)`
  - `OP`/operation-direction: `S` = Set, `Q` = Query.
- Acknowledgement (TV): `CLIENT ID(3B) | ':'(0x3A) | ACK(1B) | DATA(4B) | CHECKSUM(1B) | TERM(1B=CR)`
  - `ACK`: most common are `OKAY`, `EROR`, `WAIT`.

**Checksum.** 8-bit checksum of the whole ASCII byte sequence; the checksum byte is chosen so that the sum of the entire string *including* the checksum byte equals zero (mod 256). The source provides pre-computed checksum bytes for every command (both MAC-specific and generic variants).

**Client ID / addressing.** `CLIENT ID` is 3 bytes, hex (0-9, A-F). On networked Smart TVs it is the last 3 bytes of the Ethernet MAC address; on feature TVs it is selected in the TV menu. `ALL` (ASCII `41 4C 4C`) is broadcast to all TVs on a multi-TV RS-232 bus. The source gives every command in two hex forms: MAC-specific (example `465`) and generic/broadcast (`ALL`).

**Data characters.** `DATA` uses `0-9, A-Z, #, ?`. `####`/`????` denote query wildcards or returned-value fields.

**Prerequisite: Custom Install menu.** RS-232 control is disabled by default. To enable: power on the TV, open the Quick Settings menu on the remote, then enter `7 3 1 0` on the number keys to reach the Custom Install menu. Set *Custom Installation* → *Enable*. To allow the TV to be powered on / set to standby over RS-232 while in standby, also set *Power On Command* → *Enable* before exiting. Note from source: the QUERY POWER ON COMMAND SETTING (`PWRE????`) is **not available in STANDBY mode**.

**Two command encodings.** Each command is documented in both ASCII mode (e.g. `S5FAPOWER232[0xCD][0x0D]`) and HEX mode (raw byte sequence, e.g. `53 35 46 41 50 4F 57 45 52 32 33 32 CD 0D`). The deterministic Actions carry the ASCII mnemonic payload (e.g. `POWR0001`); a driver must prepend `S`+`CLIENT ID`, append checksum + `0x0D`, and (optionally) translate to the hex byte form shown in the source's TV-specific / generic HEX columns.

**Revision history.** Latest documented revision is V3.6 (17-Apr-2017). The IR discrete-code tables date back to V1.0/V2.0 (2014); RS-232 commands were added in V3.0 (13-May-2016) and refined through V3.6.

<!-- UNRESOLVED: source "Models" section is blank; cannot confirm 65U86LUA against a published model list. -->
<!-- UNRESOLVED: no firmware version compatibility stated. -->
<!-- UNRESOLVED: IR discrete codes (Pronto CCF + 04FB hex data codes) are present in source but have no `ir` AI4AV transport and are therefore not represented as Actions/Feedbacks. -->
<!-- UNRESOLVED: multi-TV bus topology and cabling diagrams are referenced ("Connecting to one/multiple TVs") but the source figures are not machine-readable. -->
