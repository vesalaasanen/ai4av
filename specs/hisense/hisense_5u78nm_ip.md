---
spec_id: admin/hisense-5u78nm
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U78NM Control Spec"
...
retrieved_at: 2026-06-25T17:16:46.073Z
last_checked_at: 2026-06-25T17:49:48.446Z
generated_at: 2026-06-25T17:49:48.446Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "the operator context labels this entity \"TCP/IP\" (entity `hisense_5u78nm`, source filename `..._ip.refined.md`), but the provided source document is the \"RS-232/IR Protocol for Hisense Prosumer TV\" and contains NO TCP/IP, Telnet, port, or network-transport information. The Transport section reflects only what the source documents (RS-232 serial). TCP/IP transport cannot be populated from this source."
  - "the target model 5U78NM is NOT explicitly named in the source's model list; the source documents the 55F1600 / 55U1600 / 65U1600 / 75U1600 / 86U1600 Prosumer family as sharing this protocol. Listed as the entity's device on operator authority; field-verify compatibility."
  - "no TCP/IP / port / base_url information exists in the source despite the device's \"IP\" context. Do not assume a port."
  - "the source exposes state only as discrete set/query command pairs (power, mode, scalar values); no additional free-form settable variables beyond those enumerated as actions."
  - "the source documents no unsolicited / push notification messages from the TV; the protocol is strictly request (Set/Query) → acknowledgement."
  - "the source documents no named multi-step macro sequences. `RSTP1000`, `RSTA2000`, and `RSET9999` are single-command resets, not macros."
  - "the source states no destructive-action confirmation procedure or hardware interlock requiring operator confirmation. `RSET9999` (Restore Factory Settings) is destructive but the source specifies no confirmation gate. The RS-232 activation sequence (Custom Install menu enable) is an installation step, not a runtime interlock; see Transport/Notes."
  - "firmware version compatibility not stated in source."
  - "voltage / current / power specs not in this protocol document."
  - "TCP/IP transport (port, framing over socket, auth) absent from source despite the device's IP context."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:48.446Z
  matched_actions: 182
  action_count: 182
  confidence: medium
  summary: "deterministic presence proof: 182/182 payloads verbatim in source; stratified Sonnet sample corroborated (10 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 5U78NM Control Spec

## Summary
The HiSense 5U78NM is a Prosumer / commercial display controllable via an ASCII, fixed-length RS-232 serial protocol (9600 8N1, no flow control) using a checksummed command/acknowledgement frame. This spec covers that serial command catalogue — power, input routing, picture/audio settings, channels, lock modes, and IR-button emulation — plus status queries.

<!-- UNRESOLVED: the operator context labels this entity "TCP/IP" (entity `hisense_5u78nm`, source filename `..._ip.refined.md`), but the provided source document is the "RS-232/IR Protocol for Hisense Prosumer TV" and contains NO TCP/IP, Telnet, port, or network-transport information. The Transport section reflects only what the source documents (RS-232 serial). TCP/IP transport cannot be populated from this source. -->
<!-- UNRESOLVED: the target model 5U78NM is NOT explicitly named in the source's model list; the source documents the 55F1600 / 55U1600 / 65U1600 / 75U1600 / 86U1600 Prosumer family as sharing this protocol. Listed as the entity's device on operator authority; field-verify compatibility. -->
...
Transport is RS-232C over a DB9 (female chassis-mount) connector. Physical pinout (male D-sub 9-pin, TV side): 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR, 7 RTS, 8 CTS, 9 Power Input. Communication code is ASCII. `auth.type: none` is inferred — the source describes no login, password, or authentication procedure for the serial interface.

To activate serial control the TV's Custom Install menu must first be enabled (power on, open Quick Settings Menu, enter `7 3 1 0`, set Custom Installation to Enable); to allow RS-232 power-on while the TV is in standby, the in-menu Power On Command must be set to Enable. See Notes for frame format and checksum.

<!-- UNRESOLVED: no TCP/IP / port / base_url information exists in the source despite the device's "IP" context. Do not assume a port. -->
...
Inferred from the source command set (Tier 2): `powerable` from `POWR`/`PWRE` power and standby commands; `queryable` from the many `????` status-query commands; `routable` from `INPT` input-source selection; `levelable` from `VOLM`/`BRIT`/`CONT`/`BKLV`/`SHRP`/etc. scalar value commands.
...
- id: power_on_volume_povl（0000_0100）_okay_0_100
  label: "POWER ON VOLUME POVL`（`0000-0100`）` OKAY 0 - 100"
  kind: action
  command: "53 34 36 35 50 4F 56 4C 30 30 33 30 0D 0D 53 41 4C 4C 50 4F 56 4C 30 30 33 30 D3 0D"
  params: []

- id: acl_enable_acle0000_okay_acl_disable
  label: "ACL ENABLE ACLE0000 OKAY ACL DISABLE"
  kind: action
  command: "53 34 36 35 41 43 4C 45 30 30 30 30 14 0D 53 41 4C 4C 41 43 4C 45 30 30 30 30 DA 0D"
  params: []

- id: acl_enable_acle0001_okay_acl_enable
  label: "ACL ENABLE ACLE0001 OKAY ACL ENABLE"
  kind: action
  command: "53 34 36 35 41 43 4C 45 30 30 30 31 13 0D 53 41 4C 4C 41 43 4C 45 30 30 30 31 D9 0D"
  params: []
```
...
- id: query_power_on_volume_povl????_0_100_0_100
  label: "Query POWER ON VOLUME POVL???? 0-100 0-100"
  kind: query
  query_command: "51 34 36 35 50 4F 56 4C 3F 3F 3F 3F D3 0D 51 41 4C 4C 50 4F 56 4C 3F 3F 3F 3F 99 0D"

- id: query_acl_acle????_0_acl_disable
  label: "Query ACL ACLE???? 0 ACL DISABLE"
  kind: query
  query_command: "51 34 36 35 41 43 4C 45 3F 3F 3F 3F DA 0D 51 41 4C 4C 41 43 4C 45 3F 3F 3F 3F A0 0D"
```
...
[]
```
<!-- UNRESOLVED: the source exposes state only as discrete set/query command pairs (power, mode, scalar values); no additional free-form settable variables beyond those enumerated as actions. -->

## Events
```yaml
[]
```
<!-- UNRESOLVED: the source documents no unsolicited / push notification messages from the TV; the protocol is strictly request (Set/Query) → acknowledgement. -->

## Macros
```yaml
[]
```
<!-- UNRESOLVED: the source documents no named multi-step macro sequences. `RSTP1000`, `RSTA2000`, and `RSET9999` are single-command resets, not macros. -->
...
interlocks: []
```
<!-- UNRESOLVED: the source states no destructive-action confirmation procedure or hardware interlock requiring operator confirmation. `RSET9999` (Restore Factory Settings) is destructive but the source specifies no confirmation gate. The RS-232 activation sequence (Custom Install menu enable) is an installation step, not a runtime interlock; see Transport/Notes. -->

## Notes

**Frame format — command (PC → TV), fixed length:** `OPERATION(1) + CLIENT_ID(3) + COMMAND(4) + DATA(4) + CHECKSUM(1) + TERMINATION(1)`. `OPERATION` is `S` (Set) or `Q` (Query). `COMMAND` and `DATA` are 4-byte ASCII fields drawn from `0-9,A-Z,#,?`. `TERMINATION` is carriage return `0x0D`.

**Frame format — acknowledgement (TV → PC):** `CLIENT_ID(3) + ':'(1, 0x3A) + ACK(4) + DATA(4) + CHECKSUM(1) + TERMINATION(1)`. Common ACK values are `OKAY`, `EROR`, `WAIT` (e.g. power-on returns `WAIT` then `OKAY`). Example, turn on set `5FA`: `S5FAPOWRON##[0xC6][0x0D]` → `5FA:WAIT####[0x49][0x0D]` → `5FA:OKAY####[0x4A][0x0D]`.

**Query form:** `Q[CLIENT_ID][COMMAND]????[CHECKSUM]CR`; response `[CLIENT_ID]:OKAY[DATA4][CHECKSUM]CR`.

**Checksum:** 8-bit checksum of the hexadecimal byte sequence; the checksum of the entire string including the CHECKSUM byte equals zero. The verifier/rebuilder must recompute this per command — do not hard-code checksums across different CLIENT_IDs.

**CLIENT_ID:** the last 3 bytes of the Ethernet MAC address (Smart TV) or a menu-selected value (Feature TV); the literal `ALL` (`0x41 0x4C 0x4C`) is broadcast / generic and addresses all TVs. The source provides two parallel hex columns per command — "TV Specific HEX" (MAC last-3, e.g. `465`) and "Generic HEX" (`ALL`); integrators should generally use the generic `ALL` form unless addressing a specific unit.

**Case sensitivity:** the protocol is case sensitive — all mnemonics are uppercase ASCII as shown.

**IR-key emulation:** the `BTTN1xxx` entries emulate physical remote keys (POWER, VOL±, CH±, digits, navigation, playback, colour keys, etc.) over the serial link; their `DATA` field is the 4-digit button code.

**Discrete IR (out of scope):** the source also contains a separate discrete IR code table (NEC-style: low/high custom-byte pair `04`/`FB`, data + complement, plus full Pronto CCF strings) for inputs, power, aspect ratios, channels, digits, navigation, PIP, etc. These are an IR transport, not part of the serial/TCP command set, and are not enumerated as actions here.

<!-- UNRESOLVED: firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: voltage / current / power specs not in this protocol document. -->
<!-- UNRESOLVED: TCP/IP transport (port, framing over socket, auth) absent from source despite the device's IP context. -->
