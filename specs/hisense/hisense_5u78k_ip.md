---
spec_id: admin/hisense-5u78k
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U78K Control Spec"
...
retrieved_at: 2026-05-04T09:03:48.196Z
last_checked_at: 2026-06-25T17:49:47.347Z
generated_at: 2026-06-25T17:49:47.347Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "the task input listed \"Known protocol: TCP/IP\", but the provided source document contains ONLY the RS-232/IR protocol. No TCP/IP addressing, port, or framing is documented anywhere in the source. TCP transport is therefore NOT populated. If a TCP/IP variant exists it must come from a separate source artifact."
  - "TCP/IP transport (port, base_url, framing) not documented in this source."
  - "no distinct settable-parameter table separate from the Actions"
  - "source documents no unsolicited notifications; the TV only emits"
  ...
  - "TCP/IP / IP control variant not documented in this source artifact."
  - "IR (Pronto CCF) discrete codes present in source but not modelled here — IR is not a machine serial transport."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:47.347Z
  matched_actions: 165
  action_count: 165
  confidence: medium
...
# HiSense 5U78K Control Spec

## Summary
The HiSense 5U78K is a Prosumer TV controlled via a fixed-length RS-232C ASCII serial protocol over a DB9 D-sub connector. This spec covers the serial command set (power, input, picture, audio, tuner, channel, OSD, menu/lock, and remote/panel key simulation) as documented in the vendor "RS-232/IR Protocol" reference (Version 3.6, 17 April 2017). A discrete IR code table is also present in the source but is out of scope for the machine serial interface.

<!-- UNRESOLVED: the task input listed "Known protocol: TCP/IP", but the provided source document contains ONLY the RS-232/IR protocol. No TCP/IP addressing, port, or framing is documented anywhere in the source. TCP transport is therefore NOT populated. If a TCP/IP variant exists it must come from a separate source artifact. -->

## Transport
```yaml
# Source documents ONLY RS-232C serial. No TCP/IP content present.
protocols:
  - serial
serial:
  ...
  # Communication Code: ASCII (source). Connector: DB9 D-sub, female chassis mount on TV.
  # Pinout (TV jack, female): 1.N/C 2.RXD 3.TXD 4.DTR 5.GND 6.DSR 7.RTS 8.CTS 9.Power Input
auth:
  type: none  # inferred: no auth procedure in source
# UNRESOLVED: TCP/IP transport (port, base_url, framing) not documented in this source.
```

## Traits
```yaml
traits:
  - powerable    # inferred: POWR0000/POWR0001 power on/off commands present
  - routable     # inferred: INPT0001..0012 input source selection commands present
  - queryable    # inferred: extensive ???? query commands return current state
  - levelable    # inferred: BRIT/CONT/VOLM/BKLV settable 0-100 value commands present
```
...
## Variables
```yaml
# UNRESOLVED: no distinct settable-parameter table separate from the Actions
# command catalogue. All settable values (volume, brightness, contrast, etc.)
# are expressed as discrete serial commands already captured in ## Actions.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited notifications; the TV only emits
# ACK frames in response to commands.
```
...
# or power-on sequencing requirements. Note: RSET9999 performs a factory reset
# and PWRE0001 enables remote RS-232 power-on from standby - treat as
# operationally sensitive, though the source does not label them as safety
# interlocks.
```

## Notes
- **Protocol is case-sensitive** (source, NOTES item 1). Command mnemonics must be sent uppercase as documented.
- **Fixed-length framing.** Set command: `S{CLIENT_ID 3B}{COMMAND 4B}{DATA 4B}{CHECKSUM 1B}0x0D`. Query command: `Q{CLIENT_ID}{COMMAND 4B}????{CHECKSUM}0x0D`.
- **Checksum:** 8-bit value computed so the running sum of the entire byte sequence *including the checksum byte* equals zero. Termination is `0x0D` (CR).
- **Client ID:** last 3 bytes of the Ethernet MAC address for Smart TV models, or a menu-selected ID for Feature TV models. Broadcast addressing uses `ALL` (ASCII `41 4C 4C`).
- **RS-232 port must be enabled first** in the TV Custom Install menu (access via Quick Settings → numeric `7 3 1 0` → Custom Installation → Enable). To wake the TV from standby over RS-232, set "Power On Command" to Enable (PWRE0001).
- **Connector gender:** TV exposes a female DB9 chassis jack. A male DB9 with crossed RXD/TXD is implied for the controller side (source lists both male and female pinouts).
- **Sample byte-level exchange (from source):** power-on TV `5FA` → `53 35 46 41 50 4F 57 52 30 30 30 31 05 0D`, ACK `35 46 41 3A 4F 4B 41 59 23 23 23 23 4A 0D` (may first return `WAIT` while booting).
- **Generic vs MAC-specific payloads:** the source lists two hex forms per command — a MAC-specific row (example client `465`) and a generic `ALL` broadcast row. Use the `ALL` form for fleet-wide/broadcast control and the MAC form for a specific set.
<!-- UNRESOLVED: no firmware version compatibility range stated in source. -->
<!-- UNRESOLVED: TCP/IP / IP control variant not documented in this source artifact. -->
<!-- UNRESOLVED: IR (Pronto CCF) discrete codes present in source but not modelled here — IR is not a machine serial transport. -->
