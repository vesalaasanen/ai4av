---
spec_id: admin/hisense-85u8qg-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "Hisense 85U8QG Series Control Spec"
manufacturer: Hisense
model_family: "85U8QG Series"
aliases: []
compatible_with:
  manufacturers:
    - Hisense
...
retrieved_at: 2026-06-25T19:04:45.959Z
last_checked_at: 2026-06-25T19:09:10.975Z
generated_at: 2026-06-25T19:09:10.975Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "exact supported model list within the \"85U8QG Series\" is not enumerated in the source (the Models section is blank). Firmware version compatibility not stated. IR discrete codes (Pronto CCF) are documented but emitted as a separate transport."
  - "no settable parameter beyond discrete actions is documented as a"
...
verification:
  ...
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (5 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# Hisense 85U8QG Series Control Spec

## Summary
Hisense Prosumer TV (85U8QG Series) controlled via RS-232C serial link over a DB9 D-sub connector. This spec covers the ASCII fixed-length serial command/acknowledgement protocol (set, query, broadcast modes) documented in the vendor RS-232/IR protocol manual. The manual also documents discrete IR Pronto CCF codes; those are a separate IR transport not represented by the serial command payloads in this spec.

<!-- UNRESOLVED: exact supported model list within the "85U8QG Series" is not enumerated in the source (the Models section is blank). Firmware version compatibility not stated. IR discrete codes (Pronto CCF) are documented but emitted as a separate transport. -->
...
serial:
  ...
  stop_bits: 1
  flow_control: none
  code: ascii  # source: "Communication Code: ASCII code"
auth:
  type: none  # inferred: no auth procedure in source
# Framing (from source "Basic Format for Control"):
#   Command:   [OP 1B][CLIENT_ID 3B][COMMAND 4B][DATA 4B][CHECKSUM 1B][CR 1B]
#   Ack (TV):  [CLIENT_ID 3B]:[ACK 1B][DATA 4B][CHECKSUM 1B][CR 1B]
#   OP:        S=Set, Q=Query
#   CLIENT_ID: 0-9,A-F; Smart TV = last 3 bytes of Ethernet MAC; "ALL" = broadcast
#   ACK:       OKAY / EROR / WAIT
#   CHECKSUM:  8-bit, sum of all bytes including checksum byte == 0
#   TERMINATION: CR (0x0D)
# Protocol is case sensitive (source NOTE 1).
# Connector: DB9 D-sub female chassis mount (RS-232C compliant).
```

## Traits
```yaml
traits:
  - powerable   # inferred: POWR0000/0001 (standby/power on) commands present
  - queryable   # inferred: Q-prefixed query commands (PWRE/INPT/PMOD/VOLM/etc.) present
  - levelable   # inferred: VOLM/BRIT/CONT/COLR/SHRP/BKLV level-set commands present
```
...
## Variables
```yaml
# UNRESOLVED: no settable parameter beyond discrete actions is documented as a
# continuous variable with separate query semantics in the source. Level-set
# values (brightness, contrast, volume, backlight) are carried as Actions above.
```

## Events
```yaml
# UNRESOLVED: no unsolicited notifications documented in source. The WAIT ack is
# a deferred-completion acknowledgement, not an unsolicited event.
```
...
interlocks: []
# Source notes a Custom Install menu prerequisite: RS-232 port must be enabled
# (Menu > Quick Settings > enter "7 3 1 0" > Custom Installation > Enable) and
# "Power On Command" set to Enable for RS-232 power-on/standby to function,
# including in standby mode. These are setup prerequisites, not runtime
# safety interlocks - no explicit safety warnings or interlock procedures are
# stated in the source.
```

## Notes
- Serial setup prerequisite: enable the RS-232 port via the Custom Install menu (Quick Settings key, then numeric "7 3 1 0"), set Custom Installation to Enable, and optionally set "Power On Command" to Enable to allow RS-232 wake from standby.
- Connector is a DB9 D-sub female chassis-mount jack (RS-232C compliant). Pinout per source: Male D-sub 9-pin = 1.N/C 2.RXD 3.TXD 4.DTR 5.GND 6.DSR 7.RTS 8.CTS 9.Power Input; Female D-sub 9-pin = 1.RI 2.TXD 3.RXD 4.DSR 5.GND 6.DTR 7.CTS 8.RTS 9.Power Input/DCD. USB-to-serial controller required for PC/USB hosts (sold separately).
- Protocol is case sensitive. Fixed-length ASCII framing with an 8-bit checksum (whole frame including checksum byte sums to zero); terminated by CR (0x0D).
- Two addressing modes: TV-specific (CLIENT_ID = last 3 bytes of Ethernet MAC, e.g. "5FA") and broadcast/generic ("ALL" = 0x41 0x4C 0x4C). After network connection, MAC address is read via Menu > Network > Network Information.
- Query power-on-command setting (PWRE????) is not available in STANDBY mode (source V3.1 note).
- This spec covers the RS-232 serial command set only. The same manual documents discrete IR control (Pronto CCF hex codes for POWER, HDMI1-5, VGA, USB, picture/sound/aspect toggles, digits, arrows, PIP, etc.) — IR is a distinct transport and should be represented in a separate spec if needed.
