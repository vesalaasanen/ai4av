---
spec_id: admin/hisense-5u78n
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U78N Control Spec"
...
retrieved_at: 2026-06-25T17:18:18.836Z
last_checked_at: 2026-06-25T17:49:49.367Z
generated_at: 2026-06-25T17:49:49.367Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "5U78N not enumerated in source model list — protocol-family assumption only. TCP/IP transport referenced in task input but source describes RS-232/IR only; no TCP/IP wiring, port, or framing described — TCP/IP transport UNRESOLVED. Firmware compatibility range not stated."
  - "TCP/IP transport - task input names TCP/IP but source describes RS-232 only; no port/URL/framing given."
  - "no settable parameter beyond discrete actions identified; level"
  - "source documents no unsolicited notifications. WAIT ack is a"
  - "source documents no multi-step sequences."
  - "5U78N model not in source model list."
  - "TCP/IP transport claimed in task input but absent from source."
  - "firmware/version compatibility range not stated."
  - "IR discrete codes not authored (separate transport, out of scope of serial Actions)."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:49.367Z
  matched_actions: 178
  action_count: 178
  confidence: medium
  summary: "deterministic presence proof: 178/178 payloads verbatim in source; stratified Sonnet sample corroborated (9 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 5U78N Control Spec

## Summary
HiSense Prosumer TV controlled via RS-232C serial link (DB9, ASCII, fixed-length framed commands with 8-bit checksum). Source document "RS-232/IR Protocol for Hisense® Prosumer TV" v3.6 (17 April 2017) covers a family of prosumer TV models; this spec applies that protocol to the 5U78N. The source also documents a discrete IR code table (separate transport, not represented in the serial Actions below).

<!-- UNRESOLVED: 5U78N not enumerated in source model list — protocol-family assumption only. TCP/IP transport referenced in task input but source describes RS-232/IR only; no TCP/IP wiring, port, or framing described — TCP/IP transport UNRESOLVED. Firmware compatibility range not stated. -->
...
# UNRESOLVED: TCP/IP transport - task input names TCP/IP but source describes RS-232 only; no port/URL/framing given.
# Enabling RS-232 requires Custom Install menu (Quick Settings + "7 3 1 0") set to Enable.
# Standby RS-232 wake requires "Power On Command" = Enable in Custom Install menu.
```

## Framing
```yaml
# Fixed-length ASCII frames. Case-sensitive.
command_format:
  fields: [operation(1B), client_id(3B), command(4B), data(4B), checksum(1B), termination(1B)]
  operation: "S=Set, Q=Query"
  client_id: "0-9,A-F; Smart TV = last 3 bytes of Ethernet MAC; Feature TV = menu-selected; ALL = broadcast"
  command: "A-Z mnemonic (see Actions)"
  data: "0-9,A-Z,#,?"
  checksum: "8-bit; whole frame including checksum byte sums to zero"
  termination: "CR (0x0D)"
ack_format:
  fields: [client_id(3B), colon(0x3A), ack(4B), data(4B), checksum(1B), termination(1B)]
  ack_values: ["OKAY", "EROR", "WAIT"]
example_set_power_on:
  ascii: "S5FAPOWER232[0xCD][0x0D]"
  ack: "5FA:OKAY####[0x4A][0x0D]"
example_query_input:
  ascii: "Q5FAINPT????[0xBC][0x0D]"
  ack: "5FA:OKAYHDM1[0xCB][0x0D]"
```

## Traits
```yaml
traits:
  - powerable    # inferred: POWR0000/0001 standby/on commands
  - queryable    # inferred: extensive ???? query mnemonics
  - levelable    # inferred: VOLM/BRIT/CONT/COLR/SHRP/BKLV level set+query
  - routable     # inferred: INPT input source selection
```
...
## Variables
```yaml
# UNRESOLVED: no settable parameter beyond discrete actions identified; level
# sets (VOLM/BRIT/CONT/etc.) are represented as parameterized Actions.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited notifications. WAIT ack is a
# synchronous hold, not an async event.
```
...
interlocks:
  - rs232_port_requires_custom_install_enable  # menu: Quick Settings + "7 3 1 0" → Custom Installation = Enable
  - standby_wake_requires_power_on_command_enable  # Custom Install "Power On Command" = Enable for RS-232 standby wake
# No destructive-interlock warnings beyond factory-reset command (RSET9999).
```

## Notes
- Protocol is **case-sensitive** (source note 1).
- Two address modes: per-TV (last 3 MAC bytes, e.g. `5FA`) or broadcast (`ALL`). Source provides both per-TV and generic (`ALL`) HEX for every command.
- Power-on via RS-232 from standby requires the "Power On Command" Custom Install setting = Enable; otherwise standby wake is inert.
- `RSET9999` restores factory settings — treat as destructive.
- Source also contains a **Discrete IR Control** table (POWER/INPUT/HDMI/digits/arrows/etc. as `04FB{data}{complement}` hex). These are a separate IR transport and are NOT represented in the serial Actions above.
- Source version 3.6 dated 2017-04-17; protocol may predate the 5U78N SKU.

<!-- UNRESOLVED: 5U78N model not in source model list. -->
<!-- UNRESOLVED: TCP/IP transport claimed in task input but absent from source. -->
<!-- UNRESOLVED: firmware/version compatibility range not stated. -->
<!-- UNRESOLVED: IR discrete codes not authored (separate transport, out of scope of serial Actions). -->
````

Spec ready. Actions/Feedbacks bodies intentionally empty — merge handles 147 actions + 31 feedbacks. Flagged 5U78N∉source + TCP/IP gap unresolved.
