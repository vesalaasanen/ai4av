---
spec_id: admin/hisense-85u7qg-pro-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 85U7QG PRO Series Control Spec"
manufacturer: HiSense
model_family: "85U7QG PRO Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
source_urls:
  ...
  - https://www.hisense-b2b.com/en/downloads
retrieved_at: 2026-06-16T01:49:34.832Z
last_checked_at: 2026-06-25T19:11:02.216Z
generated_at: 2026-06-25T19:11:02.216Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP port number not stated in the refined source. Recovery notes indicate typical Hisense IP control ports are 5000/8080/8088 but these are not confirmed against this device — left null per no-assumption policy."
  - "ASCII vs HEX framing selection (when each is used, whether socket accepts both, newline/CR termination beyond the documented `0D` trailer) not specified."
  - "Connection lifecycle (keep-alive, idle timeout, max concurrent connections) not documented."
  - "Response framing for queries beyond the parameter/return column (delimiters, error tokens for out-of-range values) not fully specified."
  - "port number not stated in refined source"
  - "confirm absence of push events on device - source lists none."
  - "full original PDF may contain safety/interlock pages not carried into"
  - "firmware/OS version compatibility not stated."
  - "whether the 85U7QG PRO accepts the full catalogue or a subset (some HiSense consumer sets omit Commercial/B2B commands) — verify on-device."
  - "exact TCP port, line-ending expectations for the ASCII form (the HEX form's `0D` trailer is explicit; ASCII-form terminator is not), and whether the socket is persistent or one-command-per-connection."
  - "ASCII query response token format (e.g. does `VOLM????` return `VOLM0050` or bare `50`?) — HEX form shows return param in column but ASCII reply string is undocumented."
verification:
  verdict: verified
  checked_at: 2026-06-25T19:11:02.216Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (11 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 85U7QG PRO Series Control Spec

## Summary
The HiSense 85U7QG PRO Series is an 85-inch ULED/Mini-LED 4K Google TV from HiSense's consumer "Prosumer" line (sold to integrators). This spec covers its external control command set — a fixed-catalogue ASCII mnemonic protocol (e.g. `PWRE`, `POWR`, `INPT`, `VOLM`) sent over TCP/IP. The same mnemonics are documented in two on-wire forms: ASCII text, and a HEX-byte framing with two checksum/addressing variants (TV-specific via MAC-address-derived suffix, or a generic `ALL` broadcast form). The catalogue spans power, input selection, picture/audio parameters, aspect ratio, volume/mute, tuner, captions, OSD, hotel/Commercial (B2B) lockout settings, discrete button-emulation codes, and per-setting status queries.

<!-- UNRESOLVED: TCP port number not stated in the refined source. Recovery notes indicate typical Hisense IP control ports are 5000/8080/8088 but these are not confirmed against this device — left null per no-assumption policy. -->
<!-- UNRESOLVED: ASCII vs HEX framing selection (when each is used, whether socket accepts both, newline/CR termination beyond the documented `0D` trailer) not specified. -->
<!-- UNRESOLVED: Connection lifecycle (keep-alive, idle timeout, max concurrent connections) not documented. -->
<!-- UNRESOLVED: Response framing for queries beyond the parameter/return column (delimiters, error tokens for out-of-range values) not fully specified. -->

## Transport
```yaml
protocols:
  - tcp  # known protocol per source artifact; command set carried over TCP socket
# addressing.port: null  # UNRESOLVED: port number not stated in refined source
#   (recovery notes list candidate Hisense IP ports 5000/8080/8088 but none confirmed here)
auth:
  type: none  # inferred: no auth procedure in source
# NOTE: no serial: block emitted - this spec covers the IP/TCP transport of the
# command set. The source labels several commands "RS-232 Remote Power On" but
# that is command semantics, not a separate serial transport declaration here.
```

## Traits
```yaml
traits:
  - powerable  # inferred from POWR0000/0001 + PWRE power-on-command-enable commands
  - queryable  # inferred from extensive `????` status-query mnemonics (INPT/PMOD/VOLM/MUTE/etc.)
  - levelable  # inferred from continuous-value set commands: VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV (0000-0100 / 0000-0020 ranges)
# routable NOT asserted: INPT selects input on a single display output, not a
# multi-output matrix route.
```
...
## Variables
```yaml
# No discrete variables beyond those implied by the parameterized set/query action
# pairs already covered in Actions/Feedbacks (brightness, contrast, color, tint,
# sharpness, backlight, volume, volume-range, volume-locked-level). Omit.
```

## Events
```yaml
# No unsolicited-notification entries documented. The protocol is request/response
# only (no push events for input changes, power state transitions, etc.).
# UNRESOLVED: confirm absence of push events on device - source lists none.
```

## Macros
```yaml
# No multi-step sequences documented in source. RSTP1000 / RSTA2000 / RSET9999 are
# single-command resets, already covered in Actions - not macros.
```
...
# appear in the refined command-table source.
# Operational caution (not source-derived safety text): POWR controls standby/
# power-on; PBTN power-off-control-mode (AC ONLY vs ALL) governs whether power-off
# also applies to AC - verify mode before issuing power commands in integrated rigs.
# UNRESOLVED: full original PDF may contain safety/interlock pages not carried into
# the refined extract; this spec reflects only the command table.
```

## Notes
- **Command forms:** Every mnemonic in the source carries two HEX representations — a *TV-specific* form (byte 2-3 derive from the device MAC address; example `'465'` → `34 36 35`) and a *Generic* form using the literal `41 4C 4C` ("ALL") broadcast prefix. Integrators targeting a single known unit should use the TV-specific form; the generic form addresses any compliant HiSense set on the link.
- **Checksum:** The penultimate byte in each HEX frame is a per-command checksum (not documented algorithm — appears to be a fixed per-mnemonic value in this table, NOT computed at runtime). The trailer byte `0D` is CR. Implementers should send the exact bytes as listed rather than recompute.
- **Parameter ranges:** Numeric mnemonics use 4-digit zero-padded decimal: `VOLM（0000-0100）`, `BRIT（0000-0100）`, `SHRP（0000-0020）`, `BKLV（0000-0100）`, `MAVL/VLFL` similar. Tint/Color/Contrast share the 0000-0100 scale. Out-of-range behavior is not documented.
- **Discrete-button emulation:** `BTTN10xx` codes (1000-1055) emulate IR-remote keypresses (digits 0-9, CH/VOL, transport keys, color keys, D-pad). These are fire-and-forget — no query counterpart.
- **Hotel/B2B lockout:** `RMOT`/`PANL`/`MENU`/`AVMN`/`OSD#`/`INPM`/`SVOL`/`VLFL`/`B2BM`/`USBM`/`PBTN` are integrator/commercial lockout controls; several states (`AC RESET`, `STANDBY RESET`) describe when the lock reverts rather than a lock level.
- **Caption/OSD oddities:** `CC##` and `OSD#` mnemonics contain literal `##`/`#` characters — these are part of the opcode, not placeholders. Send verbatim.
- **Source provenance:** This spec derives from the refined command-table extract of the HiSense Prosumer/B2B IP control document (per recovery notes: first-party Hisense serial/IP commands, EU/APAC "PRO" naming convention). The refined artifact preserves only the command table; prose sections of the original manual were not carried forward.

<!-- UNRESOLVED: firmware/OS version compatibility not stated. -->
<!-- UNRESOLVED: whether the 85U7QG PRO accepts the full catalogue or a subset (some HiSense consumer sets omit Commercial/B2B commands) — verify on-device. -->
<!-- UNRESOLVED: exact TCP port, line-ending expectations for the ASCII form (the HEX form's `0D` trailer is explicit; ASCII-form terminator is not), and whether the socket is persistent or one-command-per-connection. -->
<!-- UNRESOLVED: ASCII query response token format (e.g. does `VOLM????` return `VOLM0050` or bare `50`?) — HEX form shows return param in column but ASCII reply string is undocumented. -->
````
