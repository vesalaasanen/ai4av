---
spec_id: admin/hisense-75a51hua-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 75A51HUA Series Control Spec"
manufacturer: HiSense
model_family: "75A51HUA Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T18:22:02.075Z
last_checked_at: 2026-06-25T18:30:54.406Z
generated_at: 2026-06-25T18:30:54.406Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "the \"Models\" section of the source is blank; only the input device"
  - "firmware version compatibility not stated in source."
  - "no TCP/HTTP/UDP transport documented; the network connection is used"
  - "electrical/voltage/current/power specifications not in source."
...
verification:
  ...
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (7 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 75A51HUA Series Control Spec

## Summary
The HiSense 75A51HUA Series is a prosumer/commercial TV controlled via an RS-232C
serial interface over a DB9 connector, using a fixed-length ASCII command protocol.
The device is addressed by a 3-byte client ID (last three bytes of the Ethernet MAC
address on Smart TV units, or menu-selected on feature TVs; `ALL` broadcasts to every
set on a multi-TV chain). A parallel discrete IR layer (Pronto CCF codes) is also
documented. This spec covers power control, input selection, picture/audio settings,
volume and mute, tuner/channel/caption, OSD language, and hotel/commercial lock
features, together with a full set of status queries.

<!-- UNRESOLVED: the "Models" section of the source is blank; only the input device
     name (75A51HUA Series) is used. A per-model supported-command matrix is not given. -->
<!-- UNRESOLVED: firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: no TCP/HTTP/UDP transport documented; the network connection is used
     solely to obtain the MAC address that forms the serial CLIENT ID. -->
...
## Traits
```yaml
# All inferred from command/ query examples present in the source:
- powerable   # inferred: POWR power on/standby + PWRE remote-power-on enable
- queryable   # inferred: extensive `????` status-query commands (INPT/PMOD/VOLM/etc.)
- levelable   # inferred: continuous-value sets - VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV/MAVL/VLFL
...
## Variables
```yaml
# Continuous settable parameters (VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV/MAVL/VLFL) are
# represented as discrete-value Actions by the deterministic extraction; no separate
# variable entries authored.
```
...
interlocks: []
# No explicit safety warnings or interlock procedures stated in source. Note: the
# RSET9999 command performs a factory reset and PBTN controls standby power-off mode -
# treat as state-changing, but the source documents no confirmation/interlock requirement.
```

## Notes
- **Frame format (fixed length, ASCII):** Command = `OP`(1) `CLIENT_ID`(3) `COMMAND`(4) `DATA`(4) `CHECKSUM`(1) `TERMINATION`(1=`0x0D` CR). Operation byte is `S` (Set) or `Q` (Query).
- **Checksum:** 8-bit checksum over the hex byte sequence of the whole command such that the entire string (including the checksum byte) sums to zero.
- **Acknowledgement:** `<CLIENT_ID>:<ACK><DATA><CHECKSUM><CR>` where ACK ∈ `OKAY`/`EROR`/`WAIT`.
- **Protocol is case-sensitive** (source note 1).
- **CLIENT_ID:** last three bytes of the Ethernet MAC address (Smart TV), or a menu-selected value (feature TV); `ALL` broadcasts to all sets on the bus. Identify the MAC via Menu → Network → Network Information.
- **Connector:** DB9 D-sub female chassis mount. Male pinout: 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR, 7 RTS, 8 CTS, 9 Power Input. A USB-to-Serial controller (sold separately) is required for host PCs without a serial port.
- **Enable step:** RS-232 control must be enabled in the Custom Install menu (power on, open Quick Settings, enter `7 3 1 0`, set Custom Installation → Enable). Set Power On Command → Enable to keep the port active in standby (PWRE query is unavailable in standby).
- **Dual addressing modes:** HEX commands are given per-TV (MAC-specific, e.g. `465`) and as generic broadcast (`ALL`) — both columns are present in every command row.
- **IR layer:** discrete Pronto CCF codes are documented for power, input, aspect, digits, navigation, PIP, transport, etc. as an alternate physical control; the BTTN serial commands simulate those IR remote keys.
- Examples (ASCII mode): turn on set `5FA` → `S5FAPOWR0001[checksum][CR]`, ack `5FA:WAIT####[checksum][CR]` then `5FA:OKAY####[checksum][CR]`; query input → `Q5FAINPT????[checksum][CR]`, ack `5FA:OKAYHDM1[checksum][CR]`.

<!-- UNRESOLVED: electrical/voltage/current/power specifications not in source. -->
<!-- UNRESOLVED: firmware version compatibility range not stated. -->
<!-- UNRESOLVED: no TCP/UDP/HTTP port or URL documented — network is MAC-identification only. -->
<!-- UNRESOLVED: "Models" matrix in source is blank; per-model command support unverified. -->
