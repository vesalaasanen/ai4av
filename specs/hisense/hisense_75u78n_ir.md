---
spec_id: admin/hisense-75u78n
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 75U78N Control Spec"
...
retrieved_at: 2026-06-25T18:37:07.491Z
last_checked_at: 2026-06-25T19:11:01.319Z
generated_at: 2026-06-25T19:11:01.319Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "firmware version compatibility not stated in source."
  - "supported IR command set varies by model — source directs to each TV's User Manual."
  - "discrete IR (Pronto CCF) commands are documented separately and not represented as actions here (one-way, no acknowledgement; protocol enum has no `ir`)."
  - "source defines parameter ranges inline within set commands (e.g."
  ...
  - "exact 75U78N firmware range supported by this protocol not stated."
  - "some source rows render OCR artefacts (e.g. CCF \"718E\"/\"728D\", \"HDM1\" vs \"HDMI1\") — verify against device before relying on hex."
verification:
  verdict: verified
  checked_at: 2026-06-25T19:11:01.319Z
  matched_actions: 165
  action_count: 165
  confidence: medium
...
# HiSense 75U78N Control Spec

## Summary
HiSense 75U78N is a prosumer TV controllable via RS-232C serial (DB9) and discrete IR (Pronto CCF). This spec covers the RS-232 serial command set: power, input source, picture/audio settings, tuner, channel, caption, OSD, remote/panel lock, and remote-key simulator (BTTN) commands. Serial framing is fixed-length ASCII with an 8-bit checksum and CR termination; commands carry a 3-byte client ID (last 3 bytes of Ethernet MAC, TV-menu selection, or `ALL` broadcast).

<!-- UNRESOLVED: firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: supported IR command set varies by model — source directs to each TV's User Manual. -->
<!-- UNRESOLVED: discrete IR (Pronto CCF) commands are documented separately and not represented as actions here (one-way, no acknowledgement; protocol enum has no `ir`). -->

## Transport
```yaml
# Serial RS-232C command set is the action surface. Discrete IR (Pronto CCF)
# also documented but one-way and not in the protocol enum.
protocols:
  - serial
serial:
  baud_rate: 9600
  ...
  stop_bits: 1
  flow_control: none
  code: ascii            # Communication Code: ASCII (documented)
...
```yaml
traits:
  - powerable   # inferred from POWR power on/off + PWRE power-on-command commands
  - queryable   # inferred from extensive query command set (???? queries)
  - levelable   # inferred from volume/brightness/contrast/backlight/sharpness level commands
  - routable    # inferred from input source selection (INPT) commands
```
...
## Variables
```yaml
# UNRESOLVED: source defines parameter ranges inline within set commands (e.g.
# BRIT 0000-0100, SHRP 0000-0020, VOLM 0000-0100, BKLV 0000-0100, MAVL/VLFL
# 0000-0100). These are carried as action params in the merged Actions, not as
...
## Notes
- **Setup required:** RS-232 port is disabled by default. Power on the TV → Quick Settings Menu → enter `7 3 1 0` → Custom Install menu → set Custom Installation to **Enable**. For RS-232 wake-from-standby, also set **Power On Command → Enable**.
- **Protocol is case-sensitive** (source note 1).
- **Frame format (fixed length):** `OPERATION(1B) CLIENT_ID(3B) COMMAND(4B) DATA(4B) CHECKSUM(1B) TERMINATION(1B=0x0D)`. OPERATION = `S` (set) or `Q` (query). Acknowledgement frame: `CLIENT_ID(3B) :(0x3A) ACK(4B) DATA(4B) CHECKSUM(1B) 0x0D`. ACK values: `OKAY`, `EROR`, `WAIT`.
- **Client ID:** last 3 bytes of Ethernet MAC address (Smart TV), or menu-selected (Feature TV); `ALL` = broadcast to every TV on a multi-TV serial bus.
...
<!-- UNRESOLVED: some source rows render OCR artefacts (e.g. CCF "718E"/"728D", "HDM1" vs "HDMI1") — verify against device before relying on hex. -->
