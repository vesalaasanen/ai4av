---
spec_id: admin/hisense-5u8n
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U8N Control Spec"
...
retrieved_at: 2026-05-04T09:23:28.072Z
last_checked_at: 2026-06-25T18:30:51.613Z
generated_at: 2026-06-25T18:30:51.613Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP port number, connection framing, and login procedure not stated in the refined source excerpt. Source excerpt contains command table only."
  - "port number not stated in source"
  - "no safety warnings, interlock procedures, or power-on sequencing"
  - "TCP port number not stated."
  - "connection framing (raw bytes vs telnet vs framed) not stated."
...
# HiSense 5U8N Control Spec

## Summary
The HiSense 5U8N is a commercial/hospitality display controllable over a TCP/IP link. The source documents a full hex command catalogue covering power, input selection, picture/audio settings, tuner, channel, OSD, button emulation, and hotel/commercial lock settings, each with both a TV-specific (MAC-address-keyed) and a generic variant.

<!-- UNRESOLVED: TCP port number, connection framing, and login procedure not stated in the refined source excerpt. Source excerpt contains command table only. -->
...
## Traits
```yaml
# - powerable    (POWR on/standby commands present)
# - queryable    (many ???? status queries present)
# - levelable    (VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV value set commands present)
...
## Variables
```yaml
# Settable numeric/enum parameters are represented as discrete Actions above
# (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL ranges). No additional
# variables to author.
variables: []
```
...
# requirements stated in source. Factory reset (RSET9999) and picture/audio
# reset (RSTP1000/RSTA2000) present but no caution text accompanies them.
```

## Notes
- Every command has two hex forms: a TV-specific form prefixed `53 34 36 35` (ASCII "S46\5" — MAC-address-keyed, last three digits per TV, example '465') and a generic form prefixed `53 41 4C 4C` (ASCII "SALL") that targets all TVs on the bus. Both terminate with a checksum byte followed by `0D` (CR).
- All payloads end in `0D` (carriage return). The byte preceding `0D` is a per-command checksum; its algorithm is not described in the source excerpt.
- Query commands use the `????` placeholder in the ASCII mnemonic and `3F 3F 3F 3F` in the hex body; response value is returned as a numeric/enum.
- Mute and caption control use non-standard mnemonics: `MUTE`, `CC##`, `OSD#` (the `#` characters are literal).
- Remote-key-emulation commands (`BTTN####`) replicate IR remote buttons (digits, transport, colour keys, D-pad) rather than setting a device state directly.
- The 5U8N is a hospitality/commercial model: lockable features include volume control (`SVOL`), input mode (`INPM`), menu access (`MENU`), OSD mode (`OSD#`), remote/panel keys (`RMOT`/`PANL`), power-off control (`PBTN`), B2B function mode (`B2BM`), and USB behaviour (`USBM`).

<!-- UNRESOLVED: TCP port number not stated. -->
<!-- UNRESOLVED: connection framing (raw bytes vs telnet vs framed) not stated. -->
<!-- UNRESOLVED: checksum algorithm for trailing byte not documented in source. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: no auth / login procedure in source (auth.type inferred none). -->
