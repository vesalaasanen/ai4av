---
spec_id: admin/hisense-75qd75n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 75QD75N Series Control Spec"
manufacturer: HiSense
model_family: 75QD75N
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T09:17:17.452Z
last_checked_at: 2026-06-25T09:20:03.882Z
generated_at: 2026-06-25T09:20:03.882Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source provides no connection parameters (port, baud, data bits, parity, stop bits, flow control), no auth/login procedure, no firmware compatibility range, and no power/voltage/current specifications. See Notes for protocol-framing detail and the TCP/IP label discrepancy."
  - "serial config not stated in source (baud_rate, data_bits, parity,"
  - "no continuous variables distinct from parameterized Actions."
  - "source documents no unsolicited event/notification frames."
  - "source defines no multi-step command sequences."
  - "source contains no safety warnings, interlock procedures, or"
  - "connection parameters (baud_rate, data_bits, parity, stop_bits, flow_control, RS-232 connector pinout), firmware compatibility range, and any safety/electrical specs are not present in the supplied source. The \"TCP/IP\" hint in the spec brief is contradicted by the byte-level evidence in the source and has been overridden to `serial`; if a true IP-control path exists, it requires a separate source document."
verification:
  verdict: verified
  checked_at: 2026-06-25T09:20:03.882Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (7 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 75QD75N Series Control Spec

## Summary
RS-232 control protocol for the HiSense 75QD75N commercial flat-panel display. Source document is a command table mapping ASCII mnemonics (POWR, INPT, VOLM, etc.) to literal hex payloads framed with a 4-byte device identifier, 4 ASCII command bytes, parameter digits, a checksum, and `0x0D` terminator. Two frame variants are given per command: a TV-specific form using the last three digits of the device MAC (example `465`) and a generic "all TVs" form using the literal string `SALL`.

<!-- UNRESOLVED: source provides no connection parameters (port, baud, data bits, parity, stop bits, flow control), no auth/login procedure, no firmware compatibility range, and no power/voltage/current specifications. See Notes for protocol-framing detail and the TCP/IP label discrepancy. -->

## Transport
```yaml
# CRITICAL: emit ONLY the sub-key groups that match the protocols[] you chose.
# Do not emit `serial:` block for a TCP-only device.
# Do not emit `addressing.port` for a serial-only device.
# Unused protocol groups are NOT "UNRESOLVED" - they are "N/A" and must be OMITTED entirely.
#
# auth.type inference rule (Tier 2): if the source describes no login/password/auth
# procedure, emit `auth.type: none  # inferred: no auth procedure in source`.
# Only mark auth as UNRESOLVED when the source hints at auth but does not specify it.
protocols:
  - serial
```

**Worked example — serial-only device (RS-232):**
```yaml
protocols:
  - serial
serial:
...
protocols:
  - serial
# UNRESOLVED: serial config not stated in source (baud_rate, data_bits, parity,
# stop_bits, flow_control). The byte-level framing in the source (0x0D terminator,
# fixed-length ASCII command + parameter + checksum frame) is consistent with
# RS-232, but no transport parameters are documented in the supplied source.
auth:
  type: none  # inferred: no auth procedure in source
```
...
## Traits
```yaml
- powerable       # inferred from POWR / PBTN / PWRE / POIS commands
- routable        # inferred from INPT input-select commands
- queryable       # inferred from ???? query variants of nearly every command
- levelable       # inferred from VOLM / BRIT / CONT / COLR / TINT / SHRP / BKLV / MAVL / VLFL range commands
```
...
## Variables
```yaml
# UNRESOLVED: no continuous variables distinct from parameterized Actions.
# Settable ranges (BRIT 0-100, CONT 0-100, COLR 0-100, TINT 0-100, SHRP 0-20,
# BKLV 0-100, VOLM 0-100, MAVL 0-100, VLFL 0-100) are represented as
# parameterized Actions, not as separate Variables.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited event/notification frames.
# Remove this section if not applicable to downstream consumer.
```

## Macros
```yaml
# UNRESOLVED: source defines no multi-step command sequences.
```
...
# power-on sequencing requirements. PWRE (RS-232 Remote Power On enable) and
# PBTN (power-off control mode: AC-only vs all) are operational toggles, not
# safety interlocks.
```

## Notes
**Protocol label.** The `Known protocol: TCP/IP` hint in the spec brief does not match the source evidence. Every command in the source is given as a literal hex byte sequence ending in `0D` (carriage return) with no IP/port/base-URL parameters, no socket framing, and no auth — classic RS-232 framing. Transport has been populated as `serial` per the source. If a separate IP-control path exists for the 75QD75N, it is not documented in the supplied source and would require a separate spec.

**Frame structure (inferred from the byte columns).** Every command frame is 14 bytes:
- bytes 0–3: device identifier. `53 34 36 35` = ASCII `S465` (TV-specific, last three octets of the device MAC — example `465` in the source header) **or** `53 41 4C 4C` = ASCII `SALL` (generic, addressed to all TVs on the bus).
- byte 4: direction/frame-type marker. `50` (`P`) = command/poll, `51` (`Q`) = query request. Verified across the `POWR` (P) vs `POWR????` (Q) and `INPT0001` (P) vs `INPT????` (Q) pairs.
- bytes 4–7: 4-character ASCII command mnemonic (e.g. `POWR`, `INPT`, `VOLM`).
- bytes 8–11: 4 ASCII hex digits — command parameter for set/query, or `0000`/`3F 3F 3F 3F` (`????`) for query requests.
- byte 12: checksum (XOR-style or sum, not specified in source).
- byte 13: `0D` (carriage return, frame terminator).

**Query response semantics.** Queries use the `????` placeholder in the parameter slot; the device replies with the same 14-byte frame, byte 4 still `Q` (`51`), the four mnemonic bytes unchanged, and bytes 8–11 set to the current state code (e.g. `30 30 30 30` = `0000` for "Auto" / "TV" / "Off" / "English", `30 30 30 31` = `0001` for "Power on" / "HDMI1", etc.). Mapping tables (return code → human label) are encoded in the deterministic Actions pass.

**Channel model.** The source assumes a single logical channel (one display); the `Z2`/`Z3` multi-zone scheme seen on other Hisense commercial lines is not present here.

**Remote-button dispatch.** The BTTN1xxx family (rows for CH+, CH-, VOL-, VOL+, POWER, MUTE, 0–9, MENU, EXIT, color buttons, transport keys, arrow keys, OK/ENTER, INPUT, MediaPlayer, ConnectedHome/HiSmart, etc.) provides a parallel key-press surface using the same 14-byte RS-232 frame — useful when integrating with control systems that prefer key events to high-level commands.

**Lock-down commands.** RMOT (remote key), PANL (panel key), MENU (menu access), AVMN (AV setting menu), OSD# (OSD mode), INPM (input mode lock), SVOL (volume control mode), VLFL (volume locked level), B2BM (B2B function mode), USBM (USB behavior) are institutional/professional settings for hospitality and digital-signage deployments. Operators should treat them as one-way: a misconfiguration (e.g. SVOL `LOCKED` + VLFL `0000`) can lock out end users.

**`Restore Factory Settings` (RSET9999).** Single-shot factory reset — confirm before issuing on a deployed unit.
```

<!-- UNRESOLVED: connection parameters (baud_rate, data_bits, parity, stop_bits, flow_control, RS-232 connector pinout), firmware compatibility range, and any safety/electrical specs are not present in the supplied source. The "TCP/IP" hint in the spec brief is contradicted by the byte-level evidence in the source and has been overridden to `serial`; if a true IP-control path exists, it requires a separate source document. -->
