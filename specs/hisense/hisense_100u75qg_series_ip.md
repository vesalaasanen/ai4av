---
spec_id: admin/hisense-100u75qg-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 100U75QG Series Control Spec"
manufacturer: HiSense
model_family: "100U75QG Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T16:22:10.347Z
last_checked_at: 2026-06-25T16:56:37.592Z
generated_at: 2026-06-25T16:56:37.592Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "input metadata declared the known protocol as TCP/IP, but the source document contains no TCP/IP transport, port, or IP command set — only RS-232 (serial) and Discrete IR. TCP/IP is therefore treated as N/A per source."
  - "the source is a generic \"Hisense® Prosumer TV\" RS-232/IR protocol manual; its Models section is blank and does not explicitly enumerate the 100U75QG model."
  - "source describes no unsolicited notifications from the TV."
  - "source documents no named multi-step command sequences."
  - "no explicit safety warnings or error-recovery interlocks beyond the above setup requirements are stated in the source."
  - "the final portion of the source (after the AVMN rows) is a malformed text run that re-lists and extends the command table with additional groups — OSD MODE (OSD#), INPUT MODE (INPM), and POWER ON INPUT SELECTION (POIS) — whose individual rows did not extract cleanly. Coverage of these final command groups is uncertain; they are not represented in the emitted action set."
...
# HiSense 100U75QG Series Control Spec

## Summary
The HiSense 100U75QG Series is a flat-panel TV controlled via an RS-232 (serial) ASCII command protocol. This spec covers the serial command set — set/query operations for power, input source, picture, audio, tuner, OSD, key/lock modes, and remote-button simulation — together with the fixed-length framed message format and 8-bit checksum. The source manual also documents Discrete IR control (Pronto CCF code tables); IR is noted here but is not representable in the serial-transport action set.

<!-- UNRESOLVED: input metadata declared the known protocol as TCP/IP, but the source document contains no TCP/IP transport, port, or IP command set — only RS-232 (serial) and Discrete IR. TCP/IP is therefore treated as N/A per source. -->
<!-- UNRESOLVED: the source is a generic "Hisense® Prosumer TV" RS-232/IR protocol manual; its Models section is blank and does not explicitly enumerate the 100U75QG model. -->

## Transport
```yaml
# Source documents RS-232C serial control only (DB9 D-sub, ASCII code).
# No TCP/IP / UDP / HTTP / OSC transport is described in the source, so those
# protocol groups are OMITTED (N/A), not UNRESOLVED.
protocols:
  - serial
serial:
...
## Traits
```yaml
- powerable   # inferred: POWR/PWRE power on/off commands present
- queryable   # inferred: numerous ???? query commands returning state present
- levelable   # inferred: VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV level controls present
- routable    # inferred: INPT input selection spans TV/AV/Component/VGA/HDMI1-4
```
...
## Variables
```yaml
# No separately-modelled variables. All settable parameters (volume, brightness,
# contrast, colour saturation, tint, sharpness, backlight, volume range, volume
# locked level) are carried as parameterized Actions (VOLM/BRIT/CONT/COLR/TINT/
# SHRP/BKLV/MAVL/VLFL) by the deterministic action set.
```

## Events
```yaml
# UNRESOLVED: source describes no unsolicited notifications from the TV.
# TV responses are solicited acknowledgements only (see Notes).
```
...
confirmation_required_for:
  - restore_factory_settings  # source: RSET9999 → "Reset Function Mode" (factory reset)
interlocks:
  - "RS-232 port must be enabled first via the Custom Install menu (Quick Settings → numeric '7 3 1 0' → Custom Installation → Enable) before serial control is active."
  - "To power the TV on / to standby over RS-232, 'Power On Command' must be set to Enable in the Custom Install menu; the PWRE query is not available in STANDBY mode."
  - "Serial connector pin 9 is labelled 'Power Input / DCD' (female) and 'Power Input' (male) - observe connector pinout; do not miswire."
# UNRESOLVED: no explicit safety warnings or error-recovery interlocks beyond the above setup requirements are stated in the source.
```

## Notes
**Message framing (fixed-length ASCII):**
- Command (PC → TV): `OPERATION(1) CLIENT_ID(3) COMMAND(4) DATA(4) CHECKSUM(1) TERMINATION(1=CR 0x0D)`.
- Acknowledgement (TV → PC): `CLIENT_ID(3) :(colon, 0x3A) ACK(1) DATA(4) CHECKSUM(1) CR`. `ACK` is one of `OKAY`, `EROR`, `WAIT` (most common). `WAIT` is returned for asynchronous operations (e.g. power-on boot) and is followed by a subsequent `OKAY`.
- **The protocol is case sensitive.**
- CHECKSUM: 8-bit checksum of the hex byte sequence such that the checksum of the whole string — including the CHECKSUM byte — equals zero.
- TERMINATION: Carriage Return (0x0D). Colon `:` is ASCII 0x3A.

**Addressing (CLIENT ID):** 3 bytes, `0-9,A-F`. For Smart TV = last 3 bytes of the Ethernet MAC address; for Feature TV = a value selected in the TV menu. `ALL` = broadcast to all TVs (generic command form, e.g. `SALL...`). The source provides both a TV-specific hex command (MAC last-3 digits, e.g. `465`) and a generic `ALL` broadcast hex form for every row.

**Example (from source):** turn on set `5FA` → `S5FAPWR0N##[0xCD][0x0D]`, ACK `5FA:WAIT####[0x49][0x0D]` then `5FA:OKAY####[0x4A][0x0D]`. Query current input of `5FA` → `Q5FAINPT????[0xBC][0x0D]`, ACK `5FA:OKAYHDM1[0xCB][0x0D]`.

**Setup:** serial control requires the Custom Install menu to be enabled (see Safety). USB-to-Serial controller (sold separately) is needed to connect to a PC/controller USB port. The interface connector is a standard DB9 D-sub female chassis-mount.

**Discrete IR control:** the source includes extensive Discrete IR code tables (per-function DATA CODE + complement, COMPLETE HEX CODE, and Pronto CCF code files) for power, inputs, picture/sound mode, aspect ratio, digits, navigation, PIP, etc. IR is a separate physical control path from RS-232 and is not captured in the serial-transport action set above.

<!-- UNRESOLVED: the final portion of the source (after the AVMN rows) is a malformed text run that re-lists and extends the command table with additional groups — OSD MODE (OSD#), INPUT MODE (INPM), and POWER ON INPUT SELECTION (POIS) — whose individual rows did not extract cleanly. Coverage of these final command groups is uncertain; they are not represented in the emitted action set. -->
<!-- UNRESOLVED: protocol revision (doc lists V1.0–V3.6, 2014–2017) is the document's own revision history, not a device/firmware protocol-version field; device protocol version not stated. -->
<!-- UNRESOLVED: no electrical voltage/current/power ratings are stated in this control-protocol source. -->
<!-- UNRESOLVED: query/response ACK values beyond OKAY/EROR/WAIT (e.g. per-command error returns) are not enumerated in source. -->
