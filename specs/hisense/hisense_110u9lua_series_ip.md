---
spec_id: admin/hisense-110u9lua-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 110U9LUA Series Control Spec"
manufacturer: HiSense
model_family: "110U9LUA Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-03T17:18:52.571Z
last_checked_at: 2026-06-25T16:56:39.549Z
generated_at: 2026-06-25T16:56:39.549Z
...
verification:
  ...
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (3 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 110U9LUA Series Control Spec

## Summary
RS-232C serial control spec for HiSense Prosumer TV. Covers power, input select, picture/audio mode, volume, channel, plus commercial-install settings (panel/remote key lock, menu access, volume limit). Also documents discrete IR Pronto CCF codes. Framing fixed-length ASCII: operation + client ID (MAC last 3 bytes) + 4-byte command + 4-byte data + 8-bit checksum + CR.

<!-- UNRESOLVED: source doc titled generic "Hisense Prosumer TV" — 110U9LUA not named in Models section (blank). No TCP/IP port stated (user input said TCP/IP but source is serial/IR only). Firmware version range not stated. -->

## Transport
```yaml
# RS-232C serial only per source. No TCP/IP documented despite user "Known protocol: TCP/IP".
protocols:
  - serial
serial:
  baud_rate: 9600
...
#   Command:  OPERATION(1) CLIENT_ID(3) COMMAND(4) DATA(4) CHECKSUM(1) CR(1)
#   ACK:      CLIENT_ID(3) ':' ACK(4) DATA(4) CHECKSUM(1) CR(1)
#   OPERATION: S=Set, Q=Query
#   CLIENT_ID: last 3 bytes of Ethernet MAC (Smart TV); menu-selected (Feature TV); "ALL"=broadcast
#   CHECKSUM: 8-bit, sum of whole string incl checksum byte = 0
#   TERMINATION: CR 0x0D
#   Protocol is case sensitive.
#   ACK tokens: OKAY, EROR, WAIT
```

## Traits
```yaml
# - powerable    (POWR power on/standby commands present)
# - routable     (INPT input source select + POIS power-on input select present)
# - queryable    (Q-prefix queries: PWRE, INPT, PMOD, BRIT, CONT, COLR, TINT, SHRP, ASPT, OVSN, CTEM, BKLV, AMOD, VOLM, MUTE, ASPK, TUNR, CC, LANG, PLED, PBTN, MAVL, SVOL, VLFL, RMOT, PANL, MENU, AVMN, OSD#, INPM)
# - levelable    (VOLM, BRIT, CONT, COLR, TINT, SHRP, BKLV, MAVL, VLFL set numeric ranges)
```
...
## Variables
```yaml
# Ranged set-params (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL) modeled as
# parameterized Actions by the deterministic pass; no standalone Variables.
```

## Events
```yaml
# Source documents no unsolicited notifications. All TV output is ACK in
...
# procedures. Custom Install menu must be enabled (Quick Settings + "7310")
# before RS-232 port active; Power On Command=Enable required for standby
# RS-232 control. These are setup steps, not safety interlocks.
```

## Notes
- **Source discrepancy:** user declared protocol `TCP/IP` + entity `hisense_110u9lua_series_ip`, but refined source doc is "RS-232/IR Protocol for Hisense® Prosumer TV" — serial + IR only. No TCP/IP port, base URL, or network command framing anywhere. Spec uses `serial`.
- **Model mismatch:** source `## Models` section blank. "110U9LUA" never appears. Doc is generic across HiSense Prosumer TV line (revisions V1.0–V3.6, 2014–2017). Compatibility assumed, not confirmed.
- **Dual addressing:** each command has both TV-specific (MAC last 3 hex digits, e.g. `S465...`) and generic broadcast (`SALL...`) HEX forms. Checksums differ per address.
- **Dual encoding:** ASCII mode (readable mnemonic) + HEX mode (space-separated hex bytes) both shown in source.
- **Client ID `ALL`** = broadcast to all TVs on multi-TV RS-232 chain.
- **`PWRE`** (power-on command enable) gates whether TV responds to power commands in standby — must be `0001` before `POWR0001` wakes from standby.
- **ACK variants:** `OKAY` (success), `WAIT` (async in progress, e.g. power-on boot), `EROR` (reject). `WAIT` may precede a later `OKAY`.
- **Setup prerequisite:** Custom Install menu: power on → Quick Settings → numeric `7310` → set Custom Installation = Enable. Enables RS-232 port.
- Doc revision history present: V1.0 2014-04-02 (IR), V2.0 2014-07-02 (CCF), V3.0 2016-05-13 (RS-232), V3.1–V3.6 (2016–2017 refinements). Latest doc version dated but no firmware tie.
<!-- UNRESOLVED: firmware compatibility range not stated. IR command set may vary per actual TV model (source note 1: "Check User Manual for your specific TV"). No power/voltage/current specs in this doc. -->
