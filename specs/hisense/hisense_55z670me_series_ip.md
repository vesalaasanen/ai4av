---
spec_id: admin/hisense-55z670me-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55Z670ME Series Control Spec"
manufacturer: HiSense
model_family: "55Z670ME Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
source_urls:
  ...
  - "https://www.hisense-b2b.com/Attachment/DownloadFile?downloadId=687"
retrieved_at: 2026-05-04T08:50:03.122Z
last_checked_at: 2026-06-25T17:49:45.252Z
generated_at: 2026-06-25T17:49:45.252Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "artifact filename suffix \"_ip\" and input metadata name TCP/IP control, but the source text documents RS-232C serial + IR only. No TCP transport details (port, framing, addressing) appear in the source; Transport below reflects the source as written."
  - "source \"Models:\" list is blank; model name taken from device metadata."
  - "firmware version compatibility not stated in source."
  - "input metadata names TCP/IP and the artifact is suffixed \"_ip\", but"
  - "nothing to populate."
  - "no events described in source."
  - "no macros described in source."
  - "source contains no explicit safety warnings, interlock procedures,"
  - "TCP/IP transport not substantiated by this source — reconcile with operator."
  - "firmware version compatibility not stated."
  - "source \"Models:\" list blank; coverage for other series members unverified."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:45.252Z
  matched_actions: 165
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (11 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 55Z670ME Series Control Spec

## Summary
HiSense Prosumer TV (55Z670ME Series) commercial control spec covering RS-232C serial control plus discrete IR. Serial interface over a DB9 D-sub connector at 9600 8N1, ASCII fixed-length frames with an 8-bit checksum. Covers power, input selection, picture/audio parameters, volume/mute, tuner, captions, OSD, and a remote/panel-key simulation catalogue.

<!-- UNRESOLVED: artifact filename suffix "_ip" and input metadata name TCP/IP control, but the source text documents RS-232C serial + IR only. No TCP transport details (port, framing, addressing) appear in the source; Transport below reflects the source as written. -->
<!-- UNRESOLVED: source "Models:" list is blank; model name taken from device metadata. -->
...
protocols:
  - serial
serial:
  baud_rate: 9600          # stated: "Baud Rate 9600bps (UART)"
...
# UNRESOLVED: input metadata names TCP/IP and the artifact is suffixed "_ip", but
# the source text contains no TCP transport details. `tcp` is intentionally NOT
# listed because no port/framing evidence exists in the source (port not assumed).
# Reconcile if a separate IP-control document exists.
```

Connector: standard DB9 D-sub, female chassis mount on the TV. Source pinout:
...
Electrical: RS-232C compliant. A USB-to-Serial controller (sold separately) is required to connect the TV to a PC/controller USB port.

Frame format (fixed length):
- Command (PC/Controller): `OPERATION(1) CLIENT_ID(3) COMMAND(4) DATA(4) CHECKSUM(1) TERMINATION(1)`
  - OPERATION: `S` = Set, `Q` = Query
  - CLIENT_ID: 0-9,A-F; Smart TV = last 3 bytes of Ethernet MAC; Feature TV = selected in TV menu; `ALL` = broadcast
  - COMMAND: A-Z (see command table)
  - DATA: 0-9,A-Z,`#`,`?`
  - CHECKSUM: 8-bit; the whole command string including the checksum byte sums to zero
  - TERMINATION: CR (0x0D)
- Acknowledgement (TV): `CLIENT_ID(3) : ACK DATA(4) CHECKSUM(1) TERMINATION(1)`; common ACKs are `OKAY`, `EROR`, `WAIT`
- Query ACK: `CLIENT_ID : OKAY <DATA4> CHECKSUM CR`
- The protocol is case sensitive.

Setup requirement: the RS-232 port must be enabled in the TV's Custom Install menu (power on, Quick Settings Menu key, then enter numeric string `7 3 1 0`; Custom Installation → Enable). For RS-232 power-on while in standby, set Power On Command to Enable before exiting. After networking the TV, Menu > Network > Network Information shows the MAC address used as CLIENT_ID.

## Traits
```yaml
traits:
  - powerable   # inferred: POWR on/off + PWRE remote-power-on commands present
  - queryable   # inferred: extensive Q???? query commands returning state present
  - levelable   # inferred: BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM value-set commands present
```
...
## Variables
```yaml
# No settable parameters exist outside the enumerated actions. Level parameters
# (brightness/contrast/color/tint/sharpness/backlight/volume 0-100, etc.) are
# represented as parameterized set actions, not standalone variables.
# UNRESOLVED: nothing to populate.
```

## Events
```yaml
# Source documents no unsolicited notifications. ACKs (OKAY/EROR/WAIT) are
# solicited responses, not events.
...
# or power-on sequencing requirements. Note: RSET9999 restores factory settings
# (destructive reset) but is not flagged as a safety interlock in the source.
```

## Notes
- Protocol revisions in source: V1.0 (02-Apr-2014, discrete IR) → V3.6 (17-Apr-2017). Latest RS-232 command set documented is V3.6.
- `PWRE` (Power On Command Setting) query is not available in STANDBY mode (per V3.1 revision note).
- Each source command row carries two transport encodings: TV-specific HEX (CLIENT_ID = last 3 hex digits of MAC, e.g. `465`) and generic HEX using CLIENT_ID `ALL` (broadcast). ASCII command strings and both HEX forms are documented verbatim in the merged action payloads.
- Discrete IR codes (Pronto CCF + complete hex) are also tabulated in the source but are out of scope for this serial/IP control spec.
- Query commands use operation `Q` and DATA `????`; the ACK returns the current value in the DATA field. Set commands use operation `S` and acknowledge `OKAY` (or `WAIT` for power-on).
<!-- UNRESOLVED: TCP/IP transport not substantiated by this source — reconcile with operator. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: source "Models:" list blank; coverage for other series members unverified. -->
