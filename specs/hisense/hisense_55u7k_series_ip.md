---
spec_id: admin/hisense-55u7k-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U7K Series Control Spec"
...
retrieved_at: 2026-06-25T08:26:52.627Z
last_checked_at: 2026-06-25T09:20:01.772Z
generated_at: 2026-06-25T09:20:01.772Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source manual is titled \"Prosumer TV\" with model numbers 55F1600 / 55U1600 / 65U1600 / 75U1600 / 86U1600; 55U7K is the family slug provided but is not explicitly listed in the document. Protocol is assumed shared across this product generation — confirm against actual 55U7K firmware before relying on edge commands."
  - "operator flagged \"TCP/IP\" as known protocol, but the refined source documents only RS-232/DB9. No Telnet, REST, or network command channel is described in the source. If TCP/IP control exists for 55U7K it is not captured here."
  - "no event/notification mechanism described in source"
  - "no explicit safety/warning text, electrical hazard, or interlock procedure beyond the install-menu enable step above."
  - "discrete IR command set not represented as Actions (parallel control plane to RS-232)."
  - "full enumeration of every TV-specific HEX command per device MAC not reproduced — broadcast/generic form is canonical for unknown-MAC control."
  - "no fault / error-recovery / WAIT-timeout sequencing documented beyond the single POWRON WAIT→OKAY example."
  - "power, voltage, current, or power-consumption specifications not in source."
  - "firmware compatibility range across the 55U7K family not stated."
verification:
  verdict: verified
  checked_at: 2026-06-25T09:20:01.772Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 55U7K Series Control Spec

## Summary
RS-232 serial control protocol for HiSense prosumer TVs, used by the 55U7K series. DB9 female chassis connector, 9600 8N1 ASCII framed protocol with fixed-length frames, 8-bit additive checksum, and per-device 3-byte client ID derived from MAC. This spec covers the documented command set for power, input selection, picture/sound tuning, channel control, locks, hotel/install modes, and remote-key passthrough.

<!-- UNRESOLVED: source manual is titled "Prosumer TV" with model numbers 55F1600 / 55U1600 / 65U1600 / 75U1600 / 86U1600; 55U7K is the family slug provided but is not explicitly listed in the document. Protocol is assumed shared across this product generation — confirm against actual 55U7K firmware before relying on edge commands. -->
<!-- UNRESOLVED: operator flagged "TCP/IP" as known protocol, but the refined source documents only RS-232/DB9. No Telnet, REST, or network command channel is described in the source. If TCP/IP control exists for 55U7K it is not captured here. -->

## Transport
```yaml
# Source describes only RS-232/DB9. No TCP/Telnet/HTTP in source.
protocols:
  - serial
serial:
  baud_rate: 9600
...
Notes on framing (source "Basic Format for Control"):
- Frame = OPERATION (1B) + CLIENT_ID (3B) + COMMAND (4B) + DATA (4B) + CHECKSUM (1B) + TERMINATION (1B, CR = 0x0D).
- OPERATION: `S` set, `Q` query.
- CLIENT_ID: hex digits 0-9/A-F. Smart TV = last 3 bytes of MAC. Feature TV = menu-selected. `ALL` = broadcast.
- ACK frame = `CLIENT_ID : OKAY[|WAIT|EROR] DATA[4B] CHECKSUM[1B] CR`. Separator is ASCII `:` (0x3A).
- CHECKSUM rule: 8-bit sum of all preceding bytes (including the CHECKSUM byte) equals zero. Document explicitly notes case-sensitive.
- Connector: DB9 D-sub female chassis mount on TV. Electrical: RS-232C compliant. Pinout: 2 RXD, 3 TXD, 5 GND.

## Traits
```yaml
# powerable:  POWR set, PWRE enable for standby power-on
# routable:   INPT select, POIS power-on input selection
# queryable:  every command has a matching ???? query
# levelable:  VOLM, MAVL, VLFL volume/level range
```
...
- id: restore_factory_settings_rset9999_okay_reset_function_mode
  label: "Restore FactorySettings RSET9999 OKAY Reset Function Mode"
  kind: action
...
- id: cc_bttn1027_okay
  ...
  kind: action
  command: "CC 53 34 36 35 42 54 54 4E 31 30 32 37 0C 0D 53 41 4C 4C 42 54 54 4E 31 30 32 37 D2 0D"
  params: []
...
- id: next_>>_<<_bttn1021_okay_next_>>_<<
...
- id: power_on_command_enable_query_power_on_command_setting_pwre0000_okay_disable_rs_232_remote_power_on
  ...
  kind: query
  query_command: "53 34 36 35 50 57 52 45 30 30 30 30 10 0D 53 41 4C 4C 50 57 52 45 30 30 30 30 D6 0D"
...
- id: power_on_command_enable_query_power_on_command_setting_pwre0001_okay_enable_rs_232_remote_power_on
  ...
  kind: query
  query_command: "53 34 36 35 50 57 52 45 30 30 30 31 0F 0D 53 41 4C 4C 50 57 52 45 30 30 30 31 D5 0D"
...
## Variables
```yaml
# Discrete command-driven state. No continuous/long-form variables beyond the command set itself.
# Volume range MAVL and locked level VLFL accept 0000-0100 as parameter tokens inside the DATA field.
- id: volume_adjust_range
  type: integer
  description: MAVL parameter range, expressed in DATA field as 0000-0100 (= 0-100).
  source: "VOLUME RANGE MAVL(0000-0100) ... 0 - 100"
- id: volume_locked_level
  type: integer
  description: VLFL parameter range, expressed in DATA field as 0000-0100 (= 0-100).
  source: "VOLUME LOCKED LEVEL VLFL(0000-0100) ... 0 - 100"
- id: brightness_range
  type: integer
  description: BRIT parameter range, 0000-0100 (0-100).
  source: "Set Brightness Value BRIT(0000-0100)"
- id: contrast_range
  type: integer
  description: CONT parameter range, 0000-0100 (0-100).
  source: "Set Contrast Value CONT(0000-0100)"
- id: color_range
  type: integer
  description: COLR parameter range, 0000-0100 (0-100).
  source: "Set Color Saturation Value COLR(0000-0100)"
- id: tint_range
  type: integer
  description: TINT parameter range, 0000-0100 (0-100).
  source: "Set TINT Value TINT(0000-0100)"
- id: sharpness_range
  type: integer
  description: SHRP parameter range, 0000-0020 (0-20).
  source: "Set Sharpness Value SHRP(0000-0020)"
- id: backlight_range
  type: integer
  description: BKLV parameter range, 0000-0100 (0-100).
  source: "SET BACKLIGHT VALUE BKLV(0000-0100) ... 0000-0100"
```

## Events
```yaml
# Source does not document any unsolicited push notifications from the TV. All responses are solicited ACKs to commands/queries.
# UNRESOLVED: no event/notification mechanism described in source
```

## Macros
```yaml
# Source documents one multi-step procedure in prose ("TV Setup" section) for enabling serial control.
- id: enable_serial_control
  description: Pre-flight to enable RS-232 on TV via Custom Install menu + PWRE command.
  source: "TV Setup ... Custom Install menu ... 7 3 1 0 ... Custom Installation = Enable ... set Power On Command to Enable ... then issue POWRON## via RS-232"
  steps:
    - "Power on TV, press Quick Settings Menu key, enter 7 3 1 0 on number keys"
    - "Custom Install menu appears"
    - "Scroll to 'Custom Installation', set to Enable (Left/Right arrow), press OK"
    - "If RS-232 needed while TV in standby: set 'Power On Command' to Enable before exiting"
    - "After network connection: Menu > Network > Network Information to read MAC (last 3 bytes = client ID)"
    - "Issue PWRE0001 (Enable RS-232 Remote Power On) so POWR works from standby"
```

## Safety
```yaml
confirmation_required_for:
  - restore_factory_settings  # RSET9999 - explicit factory reset; irreversible
interlocks:
  - PWRE0001 must be issued before POWR commands are accepted from standby (source: "set the Power On Command to Enable ... allows the TV to be turned on or set to standby remotely via RS-232")
# UNRESOLVED: no explicit safety/warning text, electrical hazard, or interlock procedure beyond the install-menu enable step above.
```

## Notes
Source is `RS-232/IR Protocol for Hisense® Prosumer TV, Version 3.6, 17 April 2017`, refined excerpt covering the RS-232 command reference. Header lists models 55F1600, 55U1600, 65U1600, 75U1600, 86U1600 — the 55U7K family slug supplied by the operator is not on that line. Treat the command set as candidate for 55U7K pending hardware verification; protocol framing (9600 8N1, DB9, fixed-length ASCII + checksum) is generation-typical.

Protocol is case-sensitive (source: "The protocol is case sensitive."). All command DATA fields are 4 ASCII characters; CHECKSUM is the 8-bit two's-complement sum of all preceding bytes such that the sum of the full frame equals 0x00. Source provides both TV-specific HEX (using example MAC `465` → bytes `53 34 36 35`) and generic HEX (using broadcast client ID `ALL` → bytes `53 41 4C 4C`). Use the broadcast `ALL` form when client ID is unknown.

Discrete IR codes are also documented in the source (Power, Input, HDMI1-5, digits, navigation, PIP) — not modeled here, as this spec covers the serial protocol.

`LANG` is used in the source as the OSD language command mnemonic; the pre-emitted action id list reflects this (e.g. `osd_lang0000_okay_english`). ACKs for `POWR` may return `OKAY` immediately or `WAIT` then `OKAY` after a longer operation (example 1 in source).

Source published under default vendor-manual assumption. License declared CC-BY-4.0; operator should re-verify distribution rights before publication.

<!-- UNRESOLVED: discrete IR command set not represented as Actions (parallel control plane to RS-232). -->
<!-- UNRESOLVED: full enumeration of every TV-specific HEX command per device MAC not reproduced — broadcast/generic form is canonical for unknown-MAC control. -->
<!-- UNRESOLVED: no fault / error-recovery / WAIT-timeout sequencing documented beyond the single POWRON WAIT→OKAY example. -->
<!-- UNRESOLVED: power, voltage, current, or power-consumption specifications not in source. -->
<!-- UNRESOLVED: firmware compatibility range across the 55U7K family not stated. -->
```
