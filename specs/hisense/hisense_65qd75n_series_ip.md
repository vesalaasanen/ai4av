---
spec_id: admin/hisense-65qd75n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 65QD75N Series Control Spec"
manufacturer: HiSense
model_family: 65QD75N
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T10:16:38.779Z
last_checked_at: 2026-06-25T10:31:05.584Z
generated_at: 2026-06-25T10:31:05.584Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "transport parameters (baud rate, data bits, parity, stop bits, flow control) not stated in source. TCP/IP transport not documented in the refined source — only hex RS-232 frames are present."
  - "serial baud rate, data bits, parity, stop bits, flow control not stated in source"
  - "no TCP port number stated in source"
  - "section removed — no per-parameter settable variables beyond parameterized actions"
  ...
  - "transport parameters (baud rate, data bits, parity, stop bits, port, pinout) not stated in source"
  - "firmware version compatibility ranges not stated in source"
verification:
  verdict: verified
  checked_at: 2026-06-25T10:31:05.584Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 65QD75N Series Control Spec

## Summary
The HiSense 65QD75N is a commercial-display-class television supporting RS-232 remote control for power, input selection, picture/sound tuning, channel navigation, and remote-key/panel-key lockout management. This spec covers the ASCII-hex framed command set documented in the vendor's RS-232 control guide, including set/query pairs for picture mode, brightness, contrast, color, tint, sharpness, aspect ratio, color temperature, backlight, sound mode, volume, mute, TV speaker, tuner, captions, OSD language, standby LED, and lockout configuration.

<!-- UNRESOLVED: transport parameters (baud rate, data bits, parity, stop bits, flow control) not stated in source. TCP/IP transport not documented in the refined source — only hex RS-232 frames are present. -->
...
<!-- UNRESOLVED: serial baud rate, data bits, parity, stop bits, flow control not stated in source -->
<!-- UNRESOLVED: no TCP port number stated in source -->
...
<!-- UNRESOLVED: section removed — no per-parameter settable variables beyond parameterized actions -->
...
## Notes
The command catalogue uses an ASCII header ("S465" = model/MAC suffix, "SALL" = generic broadcast) followed by a 4-character mnemonic and 4-digit parameter, ending in a checksum byte and `0D` carriage return. Each command has a paired `????` query variant returning the current value as a 4-digit decimal. Frame examples are reproduced in the deterministic Actions fence; the literal hex payloads are required for implementation and must not be re-encoded.

The source documents both per-TV-address frames (with "S" + 3-digit suffix, e.g. `S465`) and generic frames (`SALL`). The 3-digit suffix in the per-TV column is described as "MAC address, last three digits: example '465'" — implying the same physical command is delivered per-MAC-address for selective addressing.

Power-on sequencing: `PWRE0001` (Enable RS-232 Remote Power On) must be issued before `POWR0001` will take effect from RS-232 — the latter by itself only works once the enable is set.

Remote-key/panel-key/menu-access lockouts (`RMOT`, `PANL`, `MENU`, `AVMN`, `OSD#`) are independently toggleable — useful for hospitality/commercial install scenarios but irreversible without a factory reset (`RSET9999`).

The refined source contains only the command table; connection/wiring details (DB9 pinout, baud rate, parity) were not present in the source text provided and must be sourced from the full Hisense RS-232 control guide PDF if needed.

<!-- UNRESOLVED: transport parameters (baud rate, data bits, parity, stop bits, port, pinout) not stated in source -->
