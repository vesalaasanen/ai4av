---
spec_id: admin/hisense-100qd7n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 100QD7N Series Control Spec"
manufacturer: HiSense
model_family: "100QD7N Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-12T19:11:32.395Z
last_checked_at: 2026-06-25T10:31:02.765Z
generated_at: 2026-06-25T10:31:02.765Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "input header labelled this spec \"TCP/IP\", but every command row carries RS-232 hex framing (`53 … 0D` start byte / CR terminator) and the column header reads \"TV Specific HEX command (MAC address, last three digits: example '465')\" — i.e. RS-232 addressing by last-three-hex of the TV's MAC. No TCP/IP framing, port, or HTTP/REST material appears anywhere in the source. Transport is therefore marked UNRESOLVED until the deployment guide is located."
  - "the source documents RS-232 hex payloads only - no baud rate,"
  - "protocol inferred from hex-byte framing; not explicitly named as RS-232 in source"
  - "source lists parameterised numeric ranges (e.g. BRIT 0000-0100,"
  - "source does not document any unsolicited push notifications"
  - "source does not document any multi-step macro or sequencing"
  - "source contains no safety warnings, interlock procedures, or"
  - "RS-232 electrical parameters (baud, data bits, parity, stop bits, flow control), TCP/IP framing details, response-byte format, source-document filename/version, and firmware compatibility range are all unstated in the supplied extract."
verification:
  verdict: verified
  checked_at: 2026-06-25T10:31:02.765Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (8 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 100QD7N Series Control Spec

## Summary
The HiSense 100QD7N Series is a commercial display line. This spec covers the machine-control protocol published for the family — an ASCII-over-RS-232 command set covering power, input selection, picture/sound settings, volume, tuner, captions, OSD language, lockout menus, and remote-button passthrough. Each command is documented with both an ASCII mnemonic (e.g. `POWR0001`, `INPT0009`, `VOLM0050`) and a per-TV hex payload plus a generic hex payload that targets any TV on the bus.

<!-- UNRESOLVED: input header labelled this spec "TCP/IP", but every command row carries RS-232 hex framing (`53 … 0D` start byte / CR terminator) and the column header reads "TV Specific HEX command (MAC address, last three digits: example '465')" — i.e. RS-232 addressing by last-three-hex of the TV's MAC. No TCP/IP framing, port, or HTTP/REST material appears anywhere in the source. Transport is therefore marked UNRESOLVED until the deployment guide is located. -->

## Transport
```yaml
# UNRESOLVED: the source documents RS-232 hex payloads only - no baud rate,
# data bits, parity, stop bits, IP port, base URL, or auth procedure is stated
# anywhere in the document. The input metadata flagged TCP/IP, but the
# command rows themselves are framed as RS-232 hex bytes on serial. Treating
# the protocol as a single "serial-style" transport with all params UNRESOLVED.
protocols:
  - serial  # UNRESOLVED: protocol inferred from hex-byte framing; not explicitly named as RS-232 in source
# addressing: omitted - no TCP/IP/HTTP material in source
# serial: omitted - baud/data_bits/parity/stop_bits/flow_control all UNRESOLVED
# auth: omitted - no auth procedure in source; per Tier 2 rule would be `none` if serial params were stated
```

## Traits
```yaml
- powerable       # inferred from POWER ON/OFF CONTROL (POWR0000/0001) commands
- routable        # inferred from SET INPUT SOURCE SELECTION (INPT0000..0012) commands
- queryable       # inferred from QUERY rows for input/picture/volume/mute/aspect/etc.
- levelable       # inferred from VOLM (volume), BRIT (brightness), CONT (contrast), COLR, TINT, SHRP, BKLV commands
```
...
## Variables
```yaml
# UNRESOLVED: source lists parameterised numeric ranges (e.g. BRIT 0000-0100,
# VOLM 0000-0100, SHRP 0000-0020, BKLV 0000-0100, MAVL 0000-0100, VLFL 0000-0100)
# but those are covered by the corresponding action in the deterministic
# pre-pass. No additional standalone variables are documented.
```

## Events
```yaml
# UNRESOLVED: source does not document any unsolicited push notifications
# from the display. Removed; not applicable.
```

## Macros
```yaml
# UNRESOLVED: source does not document any multi-step macro or sequencing
# commands. Removed; not applicable.
```
...
# power-on sequencing requirements. The "RS-232 Remote Power On" enable
# (PWRE0001) and "POWER OFF CONTROL MODE" (PBTN0000/0001 AC vs ALL) are
# control commands, not safety interlocks; they are documented in Actions.
```

## Notes
- **Framing.** Every command row carries the form `<ASCII mnemonic> 0D` terminated by carriage return, e.g. `POWR0001\r`. The "Generic HEX Command" column is the ASCII-to-hex rendering of that mnemonic; the "TV Specific HEX" column prefixes the payload with the last three hex digits of the TV's MAC (so a TV whose MAC ends `465` is addressed with `53 34 36 35` as the address header instead of `53 41 4C 4C` "SALL").
- **Set vs Query.** Set commands use `53 … 0D` as the first hex byte (`S` = set). Query commands use `51 … 0D` (`Q` = query) and substitute `3F 3F 3F 3F` (`????`) for the parameter field. The deterministic pre-pass preserves this `53/51` distinction in the payload bytes.
- **Per-TV addressing.** Each row shows two hex payloads: a per-TV form (MAC-tail-prefixed, e.g. `53 34 36 35 …`) and a generic form (`53 41 4C 4C …` = `SALL`, meaning "all TVs on the bus"). A controller that wants to target a single TV should use the per-TV hex with its own MAC tail; a broadcast should use the generic `SALL` form.
- **Response convention.** Source documents that set commands return `OKAY` and query commands return the current value, but it does not specify the response framing in hex bytes — only the ASCII/decimal interpretation. Response-byte format is therefore UNRESOLVED.
- **Button passthrough (`BTTN1xxx`).** Rows 136-178 emit a synthetic remote-control keypress (numeric keypad, coloured buttons, transport keys, etc.). These are one-way passthroughs, not state commands.
- **Source provenance.** The supplied refined markdown is a table-only extract; the original RS-232 guide (filename, version, publisher) is not embedded in the file. `derived_from: vendor_manual` is asserted at low confidence; recommend locating the full PDF before promotion to S-tier.
<!-- UNRESOLVED: RS-232 electrical parameters (baud, data bits, parity, stop bits, flow control), TCP/IP framing details, response-byte format, source-document filename/version, and firmware compatibility range are all unstated in the supplied extract. -->
