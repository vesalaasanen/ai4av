---
spec_id: admin/hisense-55u85sua-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U85SUA Series Control Spec"
manufacturer: HiSense
model_family: "55U85SUA Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T17:00:42.024Z
last_checked_at: 2026-06-25T17:49:44.159Z
generated_at: 2026-06-25T17:49:44.159Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP/IP control is reported for this family (entity/file name suggests IP), but this source document covers only RS-232C and IR. No TCP port, framing, or auth for IP control is stated."
  - "source \"Models\" list is blank; model name taken from operator input."
  - "firmware version compatibility not stated in source."
  - "no TCP port stated in source (serial-only document)"
  - "no safety warnings present in source."
...
verification:
  ...
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (8 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 55U85SUA Series Control Spec

## Summary
Control spec for the HiSense Prosumer TV (55U85SUA Series) over an RS-232C serial link (DB9 D-sub). The device speaks a fixed-length ASCII command/response protocol with an 8-bit checksum, a per-TV client ID (last 3 bytes of the Ethernet MAC, or `ALL` for broadcast), and `CR`-terminated frames. The same manual also publishes discrete IR control codes (Pronto CCF); those IR codes are documented in the source but are not part of the serial action set enumerated here.

<!-- UNRESOLVED: TCP/IP control is reported for this family (entity/file name suggests IP), but this source document covers only RS-232C and IR. No TCP port, framing, or auth for IP control is stated. -->
<!-- UNRESOLVED: source "Models" list is blank; model name taken from operator input. -->
<!-- UNRESOLVED: firmware version compatibility not stated in source. -->

## Transport
```yaml
# Source explicitly describes RS-232C serial only. The "_ip" artifact name and
# operator-supplied protocol (TCP/IP) are NOT substantiated by this source -
# TCP transport is therefore UNRESOLVED and omitted, not inferred.
protocols:
  - serial
serial:
  baud_rate: 9600            # stated: "9600bps (UART)"
  data_bits: 8               # stated: "Data Length 8bits"
  parity: none               # stated: "Parity Bit None"
  stop_bits: 1               # stated: "Stop Bit 1bit"
  flow_control: none         # stated: "Flow Control None"
  communication_code: ASCII  # stated: "Communication Code ASCII code"
  connector: DB9_DSUB_FEMALE_CHASSIS  # stated: "standard DB9 D-sub ... female chassis mount"
  # Male D-sub 9-pin pinout (source): 1=N/C 2=RXD 3=TXD 4=DTR 5=GND 6=DSR 7=RTS 8=CTS 9=Power Input
  # Female D-sub 9-pin pinout (source): 1=RI 2=TXD 3=RXD 4=DSR 5=GND 6=DTR 7=CTS 8=RTS 9=Power Input/DCD
auth:
  type: none  # inferred: no login/password/auth procedure described in source
# addressing:
#   port: null  # UNRESOLVED: no TCP port stated in source (serial-only document)
# tcp block omitted: N/A - source is serial-only
```

## Traits
```yaml
# Each trait inferred from explicit command evidence in the source table.
traits:
  - powerable    # inferred: POWR0000/0001 (standby/on) + PWRE power-on-command enable
  - queryable    # inferred: extensive ???? status queries (PWRE, INPT, PMOD, BRIT, VOLM, ... )
  - levelable    # inferred: ranged set values - BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL (0-100 / 0-20)
```
...
## Variables
```yaml
# No settable parameters exist outside the enumerated actions; ranges (e.g.
# BRIT 0000-0100, VOLM 0000-0100, SHRP 0000-0020) are carried as action params.
variables: []
```
...
# issued commands (OKAY / EROR / WAIT).
events: []
```
...
interlocks: []
# Source describes no safety interlock procedures, fault sequences, or
# power-on sequencing requirements beyond the Custom Install menu setup
# (enable RS-232 / Power-On Command). Setup steps recorded in Notes, not
# treated as a safety interlock.
# UNRESOLVED: no safety warnings present in source.
```

## Notes
**Frame format (fixed length, ASCII, case-sensitive per source).** Command (PC/controller → TV):
`<OPERATION><CLIENT_ID><COMMAND><DATA><CHECKSUM><CR>` = 1 + 3 + 4 + 4 + 1 + 1 bytes.
- `OPERATION` — `S` = Set, `Q` = Query.
- `CLIENT_ID` (3 bytes, `0-9,A-F`) — last 3 bytes of the TV's Ethernet MAC (Smart TV), or a value selected in the TV menu (Feature TV). `ALL` = broadcast to every TV on the bus.
- `COMMAND` (4 bytes, `A-Z`).
- `DATA` (4 bytes, `0-9,A-Z,#,?`).
- `CHECKSUM` (1 byte) — 8-bit checksum such that the sum of all bytes in the frame **including the checksum byte** is zero.
- `TERMINATION` — Carriage Return (`0x0D`).

Acknowledgement (TV → controller): `<CLIENT_ID>:<ACK><DATA><CHECKSUM><CR>` — `:` is `0x3A`; common ACK values are `OKAY`, `EROR`, `WAIT`.

**Emitted commands use the generic broadcast form** (`CLIENT_ID` = `ALL`, i.e. ASCII prefix `SALL`/`QALL` = `53 41 4C 4C`). To target a single TV, substitute its 3-byte MAC suffix (e.g. `465` → `53 34 36 35 ...`) and recompute the checksum byte.

**RS-232 enablement (setup, not a runtime command).** Power on the TV → Quick Settings Menu → enter `7 3 1 0` → Custom Install menu → set Custom Installation = Enable (this activates the RS-232 port). To wake the TV from standby over serial, also set Power On Command = Enable before exiting; the `PWRE????` query is **not available in STANDBY mode** (per source revision note V3.1). USB-to-serial adapter required for PC/controller connection (sold separately).

**Electrical/physical.** RS-232C compliant; DB9 female chassis-mount. See Transport block for pinouts.

**Discrete IR control.** The source also publishes Pronto CCF discrete IR codes (POWER, POWER ON/OFF, INPUT, HDMI.1–5, VGA, USB, PICTURE/SOUND MODE, ASPECT RATIO variants, digits 0–9, navigation, PIP, GUIDE, FREEZE, etc.). IR is a separate transport and is **not** represented in the serial `Actions` set; consult the source table for CCF hex if IR emission is needed.

**Other quirks.**
- Protocol is case-sensitive (source note 1).
- `CHAN` row is labelled in Chinese (`频道切换`) in the source; function = channel up/down.
- `RSET9999` performs a factory reset (Reset Function Mode) — destructive; treat with care.
- Source "Models" field is blank; `55U85SUA Series` taken from operator input.
- Document revision history runs V1.0 (2014) through V3.6 (2017); no protocol/firmware version constraint is stated.

<!-- UNRESOLVED: TCP/IP control not covered by this source (no port / framing / auth). -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: multiple-TV daisy-chain wiring ("Connecting to multiple TVs") section is present in source but contains no diagram/text after extraction. -->
````
