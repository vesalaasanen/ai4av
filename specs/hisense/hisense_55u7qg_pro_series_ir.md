---
spec_id: admin/hisense-55u7qg-pro-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U7QG PRO Series Control Spec"
manufacturer: HiSense
model_family: "55U7QG PRO Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T16:48:52.133Z
last_checked_at: 2026-06-25T16:56:42.408Z
generated_at: 2026-06-25T16:56:42.408Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "supported firmware version range not stated in source. Discrete IR command set may vary by model — source directs users to check the User Manual per TV."
  - "no discrete settable-parameter list beyond the parameterized Actions above."
  - "no unsolicited-notification set documented in source."
  - "no multi-step sequences documented in source."
  - "no further explicit safety/interlock sequences stated; restore-factory RSET9999 and power-off modes are documented as control commands, not safety procedures."
...
# HiSense 55U7QG PRO Series Control Spec

## Summary
HiSense Prosumer TV (55U7QG PRO Series) control spec covering the RS-232C serial control interface and discrete IR code tables. Serial control uses a fixed-length ASCII frame protocol with per-command acknowledgement (OKAY/EROR/WAIT). The document also enumerates discrete IR Pronto CCF codes for direct IR emission. The TV's RS-232 port must be enabled in the on-screen Custom Install menu before serial control functions.

<!-- UNRESOLVED: supported firmware version range not stated in source. Discrete IR command set may vary by model — source directs users to check the User Manual per TV. -->
...
serial:
  ...
  connector: DB9_D-sub
  electrical: RS-232C_compliant
  # Physical (male DB9): 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR, 7 RTS, 8 CTS, 9 Power Input
  # Physical (female DB9): 1 RI, 2 TXD, 3 RXD, 4 DSR, 5 GND, 6 DTR, 7 CTS, 8 RTS, 9 Power Input/DCD
  # Frame format (set/query), all fixed-length ASCII:
  #   Command:    [S|Q][CLIENT_ID 3B][COMMAND 4B][DATA 4B][CHECKSUM 1B][CR]
  #   Acknowledgement: [CLIENT_ID 3B]:[ACK][DATA 4B][CHECKSUM 1B][CR]
  # CLIENT_ID = last 3 bytes of Ethernet MAC (Smart TV) / TV-menu-selected (Feature TV); "ALL" = broadcast.
  # CHECKSUM = 8-bit, whole string incl. checksum byte sums to zero. TERMINATION = 0x0D (CR).
  # Protocol is case sensitive.
auth:
  type: none  # inferred: no auth procedure in source
# Discrete IR codes (Pronto CCF) also documented for IR-blaster emission; not enumerated as transport.
```

## Traits
```yaml
traits:
  - powerable   # inferred: POWR0000 standby / POWR0001 power on, plus PWRE remote-power setting
  - queryable   # inferred: extensive ???? query commands (PWRE/INPT/PMOD/BRIT/CONT/COLR/TINT/SHRP/ASPT/OVSN/CTEM/BKLV/AMOD/VOLM/MUTE/ASPK/TUNR/CC/LANG/PLED/PBTN/MAVL/SVOL/VLFL/RMOT/PANL/MENU/AVMN)
  - levelable   # inferred: continuous-value set commands (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL)
```
...
## Variables
```yaml
# UNRESOLVED: no discrete settable-parameter list beyond the parameterized Actions above.
```

## Events
```yaml
# UNRESOLVED: no unsolicited-notification set documented in source.
```
...
interlocks:
  # Source: QUERY POWER ON COMMAND SETTING (PWRE????) is not available in STANDBY mode.
  # RS-232 port must be enabled in Custom Install menu; Power On Command must be set to Enable
  # for the RS-232 port to remain active in standby and allow remote power-on.
  - requirement: "RS-232 Custom Install menu must be Enabled before serial control operates"
  - requirement: "Power On Command = Enable required for standby-mode RS-232 power control"
# UNRESOLVED: no further explicit safety/interlock sequences stated; restore-factory RSET9999 and power-off modes are documented as control commands, not safety procedures.
```

## Notes
- Setup: power on TV → Quick Settings Menu → enter `7310` to open Custom Install menu → set Custom Installation = Enable → optionally Power On Command = Enable → exit. RS-232 port now active.
- CLIENT ID addressing: Smart TV uses last 3 bytes of Ethernet MAC; Feature TV uses an ID selected in the TV menu; `ALL` broadcasts to every TV on a multi-TV serial bus.
- Each command example in the source carries both a TV-specific HEX sequence (MAC-keyed, e.g. `465`) and a generic HEX broadcast (`ALL`) form.
- Discrete IR codes are provided as Pronto CCF hex for an IR blaster; command coverage overlaps but is not identical to the RS-232 set. IR command support varies by model — consult the device User Manual.
- Acknowledgements use OKAY / EROR / WAIT; a power-on command typically returns WAIT then OKAY.
- Pin 9 of the DB9 carries "Power Input" — do not drive without confirming electrical limits.

<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: timing / inter-command spacing not specified. -->
<!-- UNRESOLVED: voltage/current on DB9 pin 9 not specified — do not assume. -->
```
