---
spec_id: admin/hisense-5u86lua-ip
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U86LUA Control Spec"
manufacturer: HiSense
model_family: 5U86LUA
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T17:25:37.756Z
last_checked_at: 2026-06-25T17:49:50.320Z
generated_at: 2026-06-25T17:49:50.320Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP/IP port number not stated in this source. IP Control Guide (separate document) is required to confirm transport details."
  - "firmware version compatibility not stated in source."
  - "discrete IR CCF/Pronto codes (POWER, INPUT, HDMI.1-5, digits, etc.) are present in the source but out of scope for this serial/IP control spec."
  - "TCP port not stated in this source"
  - "TCP keepalive / connection persistence behavior not stated."
  - "multiple-TV daisy-chain wiring diagrams referenced in source (\"Connecting to multiple TVs\") but no topology detail provided."
  ...
  - "TCP port for IP control not in this source — needs the separate IP Control Guide (hisense-b2b downloadId=784)."
  - "firmware version range for 5U86LUA not stated."
  - "source tail (OSD#/INPM/POIS and further rows) is in a malformed unformatted block — confirm those rows are captured by the deterministic action extraction."
  - "multi-TV RS-232 daisy-chain wiring not specified in source."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:50.320Z
  matched_actions: 165
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (13 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 5U86LUA Control Spec

## Summary
HiSense 5U86LUA is a Prosumer TV in the HiSense commercial display lineup. This spec covers external control of the device via a fixed-length 14-byte ASCII command/response framing protocol. The source document (RS-232/IR Protocol for HiSense Prosumer TV) describes the command catalogue in full along with the RS-232C serial electrical/physical layer; the identical command frames apply to IP (TCP socket) control, though the TCP port is not stated in this source (the IP Control Guide is a separate document).

The command frame is: `OPERATION(1) CLIENT_ID(3) COMMAND(4) DATA(4) CHECKSUM(1) TERMINATION(0x0D CR)`. Operation is `S` (Set) or `Q` (Query). CLIENT ID is the last 3 hex digits of the TV's Ethernet MAC address (Smart TV), a menu-selected ID (Feature TV), or `ALL` for broadcast. Each command and acknowledgement lists both a MAC-specific hex form and a generic `ALL` broadcast hex form. The protocol is case-sensitive. Acknowledgement frame is `CLIENT_ID(3) : ACK(4) DATA(4) CHECKSUM(1) CR`; common ACK tokens are `OKAY`, `EROR`, `WAIT`.

<!-- UNRESOLVED: TCP/IP port number not stated in this source. IP Control Guide (separate document) is required to confirm transport details. -->
<!-- UNRESOLVED: firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: discrete IR CCF/Pronto codes (POWER, INPUT, HDMI.1-5, digits, etc.) are present in the source but out of scope for this serial/IP control spec. -->

## Transport
```yaml
# Source documents RS-232C serial transport in full. TCP/IP is the known
# transport for this entity (family suffix _ip) - the source references
# Ethernet MAC address as CLIENT ID and network connection, but does NOT
# state a TCP port. Serial config below is stated verbatim in source
# (line: "Baud Rate 9600bps ... Data Length 8bits ... Parity Bit None
# Stop Bit 1bit Flow Control None Communication Code ASCII code").
protocols:
  - serial
  - tcp  # inferred: source uses Ethernet MAC address as CLIENT ID and references network connection; IP Control Guide is a separate document
addressing:
  port: null  # UNRESOLVED: TCP port not stated in this source
serial:
  baud_rate: 9600
  data_bits: 8
  parity: none
...
**Connector / physical layer (RS-232C):** Standard DB9 D-sub, female chassis mount on the TV.
...
**Setup (Custom Install menu):** Power on TV → Quick Settings Menu → enter `7 3 1 0` → Custom Install menu → set Custom Installation to Enable (enables RS-232 port). To keep the port active in standby, set Power On Command to Enable. After connecting to a network, Menu > Network > Network Information shows the TV's MAC address (used as CLIENT ID).

**Checksum:** 8-bit checksum over the command byte sequence such that the sum of the whole string including the checksum byte equals zero (mod 256). Terminator is CR (0x0D).

<!-- UNRESOLVED: TCP keepalive / connection persistence behavior not stated. -->
<!-- UNRESOLVED: multiple-TV daisy-chain wiring diagrams referenced in source ("Connecting to multiple TVs") but no topology detail provided. -->

## Traits
```yaml
# All inferred from command examples present in source (Tier 2):
- powerable  # inferred: PWRE / POWR power on/off/standby commands
- queryable  # inferred: many ???? query commands (PWRE, INPT, PMOD, BRIT, CONT, VOLM, MUTE, etc.)
- levelable  # inferred: VOLM, BRIT, CONT, COLR, TINT, SHRP, BKLV, MAVL, VLFL set value commands
- routable   # inferred: INPT input source selection (TV/AV/Component/HDMI1-4/VGA)
```
...
## Variables
```yaml
# No discrete settable parameters beyond those represented as Actions
# (VOLM, BRIT, CONT, BKLV, etc. are enumerated as set-actions with literal
# payloads in the deterministic pre-pass). No separate Variables section
# required.
```
...
# All responses are solicited acknowledgements to Set/Query commands.
```
...
# procedures, or power-on sequencing requirements. Note only: the Custom
# Install menu must be Enabled (via code 7310) before RS-232 control
# functions, and Power On Command must be Enabled for standby wake - these
# are setup prerequisites, not runtime interlocks.
```

## Notes
- **Case sensitivity:** The protocol is case-sensitive (source NOTE 1). All mnemonics are uppercase ASCII.
- **Discrete IR:** The source's first half is a full discrete IR code table (Pronto CCF + hex data codes for POWER, INPUT, HDMI.1-5, digits, navigation, PIP, etc.). These IR codes are out of scope for this serial/IP spec but are available in the source for an IR-control companion spec.
- **Client ID semantics:** For a single-TV system, any non-`ALL` ID works; for a multi-TV system, MAC-suffix addressing targets one TV and `ALL` broadcasts to every TV on the bus/socket.
- **ACK variants:** Most Set commands return `OKAY`; power-on (`POWR0001`) may return `WAIT` before `OKAY`; query responses carry the queried value in the DATA field.
- **Protocol revision:** Source documents revisions V1.0–V3.6 (2014–2017). Command set is stable across that range; exact firmware compatibility for 5U86LUA is not stated.

<!-- UNRESOLVED: TCP port for IP control not in this source — needs the separate IP Control Guide (hisense-b2b downloadId=784). -->
<!-- UNRESOLVED: firmware version range for 5U86LUA not stated. -->
<!-- UNRESOLVED: source tail (OSD#/INPM/POIS and further rows) is in a malformed unformatted block — confirm those rows are captured by the deterministic action extraction. -->
<!-- UNRESOLVED: multi-TV RS-232 daisy-chain wiring not specified in source. -->
````

Spec emitted. Actions/Feedbacks empty (deterministic merge handles 137+28). Serial config verbatim from source; TCP port marked UNRESOLVED (separate IP guide needed).
