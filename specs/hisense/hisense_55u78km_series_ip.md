---
spec_id: admin/hisense-55u78km-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U78KM Series Control Spec"
manufacturer: HiSense
model_family: "55U78KM Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-04T20:59:13.117Z
last_checked_at: 2026-06-25T16:16:27.922Z
generated_at: 2026-06-25T16:16:27.922Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP port number not stated in source."
  - "Checksum byte computation method not documented in source."
  ...
  - "checksum algorithm not documented; values are present verbatim per command but cannot be derived for novel parameters"
verification:
  verdict: verified
  checked_at: 2026-06-25T16:16:27.922Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 55U78KM Series Control Spec

## Summary
HiSense 55U78KM Series commercial display controlled over TCP/IP using ASCII-mnemonic HEX command frames terminated by carriage return (`0x0D`). The command catalogue covers power, input source selection, picture/audio adjustment, channel and tuner control, captioning, OSD language, remote-key emulation, and B2B/commercial settings (volume locking, power-on input selection, key locking, USB behaviour, pixel shifting), with status queries for most parameters. Two logical addressing modes are documented: a TV-specific frame (MAC-derived) and a generic broadcast frame.

<!-- UNRESOLVED: TCP port number not stated in source. -->
<!-- UNRESOLVED: Checksum byte computation method not documented in source. -->
...
## Traits
```yaml
# All inferred from command examples present in the source table:
traits:
  - powerable   # inferred: POWR0000/0001 power on/standby commands
  - queryable   # inferred: extensive ???? status-query commands across all params
  - levelable   # inferred: VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV ranged value sets
...
## Variables
```yaml
# All ranged settable parameters (BRIT, CONT, COLR, TINT, SHRP, BKLV, VOLM, MAVL, VLFL)
# are represented as discrete Actions by the deterministic pass, so no separate
# Variables entries apply.
```

## Events
```yaml
# Source documents no unsolicited notifications; all responses are OKAY
# acknowledgements or query reply values.
...
# or power-on sequencing requirements. Factory reset (RSET9999) and power-standby
# (POWR0000) operations exist but carry no documented interlocks.
```

## Notes
- **Frame format:** Every command is a sequence of ASCII hex bytes ending in carriage return (`0D`). Decoded structure: `53 34 36 35` = ASCII `S465` (TV-specific prefix) or `53 41 4C 4C` = ASCII `SALL` (generic broadcast), followed by the 4-char mnemonic + 4-char parameter (e.g. `50 57 52 45 30 30 30 30` = `PWRE0000`), a single checksum byte, then `0D`.
- **Two addressing modes:** The "TV Specific HEX command" column embeds the last three digits of the display's MAC address (column header example: `465`) so frames target a single unit on a shared bus. The "Generic HEX Command" column uses the `SALL` prefix to address all displays. An integrator must substitute the target unit's MAC-suffix; only the `SALL` (generic) column is valid for unmodified broadcast.
- **Checksum:** The byte immediately before the terminating `0D` is a per-frame checksum. Its computation algorithm is not documented in the source. <!-- UNRESOLVED: checksum algorithm not documented; values are present verbatim per command but cannot be derived for novel parameters -->
- **Acknowledgement:** Successful commands return `OKAY` (ASCII). Queries return a numeric code mapping to a documented enum value (e.g. `INPT????` → `1`=TV, `4`=AV, `3`=Component, `9`=HDMI1, `10`=HDMI2, `11`=HDMI3, `12`=HDMI4, `6`=VGA).
- **RS-232 heritage:** Several command labels reference "RS-232" (e.g. `PWRE` = "RS-232 Remote Power On/Boot"). This reflects the protocol's serial origin; this spec documents the TCP/IP transport only. The identical mnemonic set is carried over TCP.
- **Parameter notation:** Ranged set commands in the source use full-width parentheses with a range, e.g. `VOLM（0000-0100）`; query variants replace the parameter with `????` wildcards. Individual fixed-value examples (e.g. `VOLM（0050）`) are also listed as distinct rows.
- **Channel commands carry a Chinese function label** (`频道切换` = "channel switch") in the source, transcribed verbatim by the deterministic pass.
- **Transport gaps:** This source is a command table only — it states no TCP port, no connection/handshake sequence, no serial fallback config, and no firmware/version gating. See UNRESOLVED markers above.
