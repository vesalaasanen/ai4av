---
spec_id: admin/hisense-116u75qgc-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 116U75QGC Series Control Spec"
manufacturer: HiSense
model_family: 116U75QGC
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-03T16:53:57.647Z
last_checked_at: 2026-06-25T16:56:40.500Z
generated_at: 2026-06-25T16:56:40.500Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP port number, connection setup (TCP vs UDP), and any handshake procedure are not stated in the refined source. RS-232 serial configuration is referenced in command descriptions but not documented here."
  - "port number not stated in source"
  - "no settable continuous variables are documented beyond the"
  - "source documents no unsolicited notifications; responses are"
  - "source documents no multi-step command sequences. Remove if not applicable."
  - "source contains no safety warnings, interlock procedures, or"
  - "TCP port number and exact transport semantics (persistent TCP socket vs. UDP datagram) not stated."
  - "RS-232 baud rate / data bits / parity / stop bits not stated in this source."
  - "maximum command rate / inter-command delay not stated."
  - "firmware version compatibility not stated."
  - "meaning of some mnemonics' full enum space (e.g. skipped values like INPT 0002/0005/0007/0008, PMOD 0001, POIS 0009) not explained; only documented enum members are represented."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:56:40.500Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 116U75QGC Series Control Spec

## Summary
HiSense 116U75QGC Series commercial display controlled over IP (TCP) using a hexadecimal, RS-232-style command protocol. The spec covers power control, input/source selection, picture and audio settings, tuner/channel, on-screen display, and a broad set of commercial (hotel/B2B) controls including volume limiting, key/menu lockout, power-on defaults, and pixel shifting. Commands and queries carry their full hex payload verbatim (device-specific and generic/broadcast forms).

<!-- UNRESOLVED: TCP port number, connection setup (TCP vs UDP), and any handshake procedure are not stated in the refined source. RS-232 serial configuration is referenced in command descriptions but not documented here. -->
...
addressing:
  ...
  # Note: per-unit vs. all-unit targeting is embedded in the payload itself
  # (see Notes), not expressed as a host/port.
auth:
  type: none  # inferred: no login/password/auth procedure described in source
```

The command set is IP-based; payloads are framed hex terminated by `0D` (CR). The same mnemonic set is also referenced for RS-232 transport (e.g. "RS-232 Remote Power On"), but no RS-232 electrical configuration is documented in this source, so a `serial:` block is omitted.

## Traits
```yaml
traits:
  - powerable   # inferred: PWRE / POWR power commands present
  - queryable   # inferred: numerous ???? status queries present
...
## Variables
```yaml
# UNRESOLVED: no settable continuous variables are documented beyond the
# parameterized set commands already represented as Actions (e.g. VOLM, BRIT,
# CONT, BKLV). Remove if not applicable.
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited notifications; responses are
# solicited (query replies of "OKAY" or a value). Remove if not applicable.
```
...
# power-on sequencing requirements. Note that factory-reset (RSET9999) and
# power on/off (POWR) commands exist but carry no documented confirmation
# requirement.
```

## Notes

**Frame structure.** Every payload is a sequence of hex bytes terminated by `0D` (CR). Layout:

```
[0x53|0x51] [addr(3 ASCII)] [mnemonic(4 ASCII)] [param(4 ASCII digits)] [checksum(1 byte)] [0x0D]
```

- **Op byte:** `0x53` ('S') = set/command; `0x51` ('Q') = query.
- **Address (3 ASCII chars):** the last three digits of the unit's MAC address (the source uses example `'465'`) for single-unit targeting, or `ALL` (`0x41 0x4C 0x4C`) to address every TV on the network (generic/broadcast form). Each table row therefore provides two hex variants: a device-specific one and a generic one.
- **Mnemonic:** the 4-character command code (e.g. `PWRE`, `POWR`, `INPT`, `VOLM`).
- **Parameter:** 4 ASCII digits. Set commands use literal values (`0000`, `0001`, `0050`, ...); queries use `????` (`0x3F 0x3F 0x3F 0x3F`).
- **Checksum:** one byte = two's complement of the arithmetic sum of all preceding bytes (i.e. `(256 - (sum mod 256)) mod 256`). This is a computed field; do not copy a checksum verbatim when the address or parameter changes — recompute it.
- **Terminator:** `0x0D`.

**Parameter-value commands.** Several mnemonics accept a continuous range shown parenthetically in the source (e.g. `BRIT（0000-0100）`, `SHRP（0000-0020）`, `VOLM（0000-0100）`, `BKLV（0000-0100）`, `MAVL（0000-0100）`, `VLFL（0000-0100）`). The hex examples in the table show one representative value; the same framing/checksum rule applies to any in-range value. Examples for min/mid/max are given separately for `BRIT`, `CONT`, `SHRP`, and `VOLM`.

**Acknowledgements / responses.** Set commands return `OKAY`; query commands return a numeric value corresponding to the same code table (e.g. `INPT????` returns `1` for TV, `4` for AV, `9`–`12` for HDMI1–4, `6` for VGA). Query reply hex payloads are identical for all enum members of a given query — the distinguishing information is the returned value byte, not a distinct frame.

**Transport caveat.** This is the IP-control document (commands reference MAC-address targeting and a generic "all TVs" form), and command descriptions explicitly mention RS-232 transport (e.g. "RS-232 Remote Power On"). The two transports share one command set; only the IP variant is specified here.

<!-- UNRESOLVED: TCP port number and exact transport semantics (persistent TCP socket vs. UDP datagram) not stated. -->
<!-- UNRESOLVED: RS-232 baud rate / data bits / parity / stop bits not stated in this source. -->
<!-- UNRESOLVED: maximum command rate / inter-command delay not stated. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: meaning of some mnemonics' full enum space (e.g. skipped values like INPT 0002/0005/0007/0008, PMOD 0001, POIS 0009) not explained; only documented enum members are represented. -->
