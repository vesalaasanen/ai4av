---
spec_id: admin/hisense-75u8k-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 75U8K Series Control Spec"
manufacturer: HiSense
model_family: "75U8K Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-14T10:38:56.387Z
last_checked_at: 2026-06-25T19:09:08.885Z
generated_at: 2026-06-25T19:09:08.885Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "no TCP/IP transport details (port, addressing) stated in source; TCP port cannot be assumed."
  - "supported model list in source is blank; 75U8K Series taken from operator metadata."
  - "firmware version compatibility not stated in source."
  - "source states no TCP port; never assumed"
  - "settable level parameters (brightness/contrast/backlight/volume/"
...
# HiSense 75U8K Series Control Spec

## Summary
The HiSense 75U8K Series is a prosumer 4K television controllable via a fixed-length ASCII command protocol. The source documents RS-232C serial control (DB9, 9600 8N1, no flow control) with an optional discrete IR command set; the same command framing is intended to carry over the TCP/IP transport flagged by the operator. Commands cover power, input selection, picture/audio parameters, tuner, channel, caption, OSD, volume/mute, and installer lock-down settings, each with a Set (`S`) and Query (`Q`) form plus a checksummed acknowledgement.

<!-- UNRESOLVED: no TCP/IP transport details (port, addressing) stated in source; TCP port cannot be assumed. -->
<!-- UNRESOLVED: supported model list in source is blank; 75U8K Series taken from operator metadata. -->
...
protocols:
  - serial
  - tcp  # inferred from operator "Known protocol: TCP/IP" hint; source itself documents RS-232 only
serial:
  ...
  pinout_female_db9:
    ...
    9: Power Input/DCD
addressing:
  port: null  # UNRESOLVED: source states no TCP port; never assumed
auth:
...
```yaml
traits:
  - powerable   # inferred: PWRE/POWR power on/off commands present
  - queryable   # inferred: extensive query (Q) commands return current state
  - levelable   # inferred: brightness/contrast/backlight/volume set+query levels present
```
...
## Variables
```yaml
# UNRESOLVED: settable level parameters (brightness/contrast/backlight/volume/
# tint/color/sharpness/volume-range/volume-locked-level) are represented as Set
# actions by the deterministic pre-pass. Promote to Variables only if a
# structured continuous-parameter representation is required.
```
...
# procedures. Note: RSET9999 performs a factory reset and POWR/RSTP affect
# device state - treat as high-impact, but no source-stated interlock.
```

## Notes
- **Protocol framing (fixed-length ASCII):** each command is `[OP 1B][CLIENT_ID 3B][COMMAND 4B][DATA 4B][CHECKSUM 1B][CR 1B]`. The TV acknowledgement is `[CLIENT_ID 3B]:[ACK 1B][DATA 4B][CHECKSUM 1B][CR 1B]`.
- **OP direction:** `S` = Set, `Q` = Query.
- **CLIENT ID:** `0-9, A-F`. For Smart TV = last 3 bytes of the Ethernet MAC address; for Feature TV = value selected in the TV menu; `ALL` = broadcast to every TV on the bus.
- **CHECKSUM:** 8-bit checksum over the hex-byte sequence such that the sum of the entire string *including the checksum byte* equals zero.
- **TERMINATION:** Carriage Return (`0x0D`).
- **ACK values:** `OKAY`, `EROR`, `WAIT`.
- **Case sensitivity:** the protocol is case-sensitive (source NOTE 1).
- **Modes:** both ASCII and HEX byte forms are documented (examples given for every command). Example, turn on set `5FA`: `S5FAPOWR0001[checksum][CR]` → `5FA:OKAY####[checksum][CR]`.
- **Custom Install menu:** RS-232 control must be enabled in the TV menu first — power on, open Quick Settings, enter `7 3 1 0`, set Custom Installation = Enable. For standby wake-over-serial, also set Power On Command = Enable. `PWRE` query is not available in STANDBY mode.
- **USB controllers:** connect to a PC/controller's USB port via a separate USB-to-Serial controller.
- **IR:** a large discrete IR code table (Pronto CCF + hex) is also provided; IR is out of scope for this serial/IP control spec.

<!-- UNRESOLVED: TCP/IP port number, addressing scheme, and any IP-specific auth not stated in source — must be obtained from device network settings or a dedicated IP-control document. -->
<!-- UNRESOLVED: HEX-mode example in source contains a likely typo (`HDM1` vs `HDMI1`) in one acknowledgement — verify against device. -->
