---
spec_id: admin/hisense-116u75qg-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 116U75QG Series Control Spec"
manufacturer: HiSense
model_family: "116U75QG Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T10:12:02.671Z
last_checked_at: 2026-06-25T10:31:01.802Z
generated_at: 2026-06-25T10:31:01.802Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "116U75QG model number is the requested device name but the source manual targets the 55–86\" Prosumer line; treat this spec as best-fit until a 116\"-specific manual is found. No firmware range, no protocol version, no IP/network control section."
  - "source does not document any unsolicited notifications"
  - "source does not define multi-step sequences."
verification:
  verdict: verified
  checked_at: 2026-06-25T10:31:01.802Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (3 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 116U75QG Series Control Spec

## Summary
RS-232 serial control spec for the HiSense Prosumer TV family (55"–86" chassis sold under model numbers 55F1600, 55U1600, 65U1600, 75U1600, 86U1600, plus the LTDNxx commercial and HU55 retail variants). Frame format is fixed-length ASCII over DB9 at 9600 baud / 8N1 with checksum + CR terminator; no login or auth required. The same manual also publishes a discrete NEC IR code table (low custom `04` / high custom `FB`).

<!-- UNRESOLVED: 116U75QG model number is the requested device name but the source manual targets the 55–86" Prosumer line; treat this spec as best-fit until a 116"-specific manual is found. No firmware range, no protocol version, no IP/network control section. -->
...
**Connection notes**
- Physical: DB9 D-sub, female chassis mount on the TV. The pinout lists RXD/TXD/GND plus the standard RS-232C handshake lines (RTS/CTS/DTR/DSR). DB9 pin 9 is labeled "Power Input" / "Power Input/DCD" — this is a TV-side power-detection line, not a power source for the control system; do NOT apply external voltage.
- The "Generic HEX Command (all TVs)" column uses client ID `ALL` (`534C4C` = "SLL") and is intended for broadcast. The "TV Specific HEX" column uses client ID `465` (last 3 bytes of the example MAC) and is the per-TV unicast frame.
- Wire format (per source): `OPERATION(1B) | CLIENT ID(3B) | COMMAND(4B) | DATA(4B) | CHECKSUM(1B) | 0x0D` for set commands; `Q | CLIENT ID(3B) | COMMAND(4B) | ???? | CHECKSUM(1B) | 0x0D` for queries. OPERATION is `S` (set) or `Q` (query). Termination is CR (0x0D). The 8-bit checksum is the byte such that the sum of every byte in the command INCLUDING the checksum byte equals 0x00.
- Acknowledgement format: `CLIENT ID(3B) : ACK(4B) DATA(4B) CHECKSUM(1B) 0x0D`. Common ACK tokens: `OKAY`, `EROR`, `WAIT`. Some commands (e.g. `POWR0001`) return `WAIT` before `OKAY` when a state transition is in flight.
- The source explicitly states the protocol is case sensitive.

**TV setup requirement (prerequisite for serial control)**
1. Power on the TV, open the Quick Settings menu, then enter `7 3 1 0` on the number keys to unlock the hidden Custom Install menu.
2. Scroll to `Custom Installation` and set it to `Enable` (Left/Right arrows + OK). This enables the RS-232 port.
3. To allow RS-232 to power the TV on from standby, set `Power On Command` to `Enable` before exiting the Custom Install menu. The equivalent software switch is `PWRE0001` (Disable = `PWRE0000`).

## Traits
```yaml
- powerable       # inferred from POWR0000/0001 + PWRE power-on-enable
- routable        # inferred from INPT input-source selection
- queryable       # inferred from extensive ???? query catalogue
- levelable       # inferred from BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM
```
...
## Variables
```yaml
# Range-parameter commands where the source documents a numeric range
# rather than discrete enum rows. Each is settable AND queryable; treat
# the discrete 0/50/100 examples in the source as illustrations of the
# range, not as separate variables.
- id: brightness
  type: integer
  range: [0, 100]
  command_set: "BRIT{value:0000-0100}"
  command_query: "BRIT????"
  notes: "Source shows 0/50/100 as concrete examples."
- id: contrast
  type: integer
  range: [0, 100]
  command_set: "CONT{value:0000-0100}"
  command_query: "CONT????"
- id: color_saturation
  type: integer
  range: [0, 100]
  command_set: "COLR{value:0000-0100}"
  command_query: "COLR????"
- id: tint
  type: integer
  range: [0, 100]
  command_set: "TINT{value:0000-0100}"
  command_query: "TINT????"
- id: sharpness
  type: integer
  range: [0, 20]
  command_set: "SHRP{value:0000-0020}"
  command_query: "SHRP????"
- id: backlight
  type: integer
  range: [0, 100]
  command_set: "BKLV{value:0000-0100}"
  command_query: "BKLV????"
- id: volume
  type: integer
  range: [0, 100]
  command_set: "VOLM{value:0000-0100}"
  command_query: "VOLM????"
- id: max_volume_range
  type: integer
  range: [0, 100]
  command_set: "MAVL{value:0000-0100}"
  command_query: "MAVL????"
  notes: "Caps the user-adjustable volume range."
- id: volume_locked_level
  type: integer
  range: [0, 100]
  command_set: "VLFL{value:0000-0100}"
  command_query: "VLFL????"
  notes: "Effective when SVOL=LOCKED."
```

## Events
```yaml
# UNRESOLVED: source does not document any unsolicited notifications
# the TV pushes to the controller (no async status / heartbeat frames).
```

## Macros
```yaml
# UNRESOLVED: source does not define multi-step sequences.
# Implicit chains (e.g. "enable PWRE before POWR0001 will wake from standby")
# are dependency notes, not macros.
```
...
interlocks:
  # Pre-flight: the RS-232 port is OFF by default. The Custom Install menu
  # must be unlocked (7-3-1-0 on the number keys) and Custom Installation
  # set to Enable, or no serial traffic will be answered.
  # Additionally, the Power On Command setting (PWRE) must be Enable for
  # POWR0001 to wake the TV from standby.
  - "Custom Installation = Enable (TV menu)"
  - "Power On Command = Enable (PWRE0001) for wake-from-standby"
# No additional safety warnings, interlocks, fault codes, or power-on
# sequencing requirements are stated in the source.
```

## Notes
- **Frame building.** Generic-hex columns in the source use the broadcast client ID `ALL` (`SLL`, bytes `53 4C 4C`); per-TV hex uses the last 3 bytes of the TV's MAC address (example `465`). For an actual deployment, replace `ALL`/`465` with the chosen client ID and recompute the checksum so the sum of all bytes (including the checksum byte) is `0x00`. Termination is always CR (`0x0D`).
- **ACK handling.** `POWR0001` is documented to return `WAIT` before `OKAY`; clients should treat `WAIT` as "state transition in progress, re-query before issuing follow-up commands".
- **Custom Install lock.** The `7 3 1 0` unlock is a hidden service code — not part of the public RS-232 protocol but a hard prerequisite to enable the port.
- **IR coexistence.** The manual also publishes discrete NEC-format IR codes (customer code `04 FB`, IR carrier per Pronto CCF). IR commands are NOT carried over RS-232; the `BTTN****` set is a parallel in-band RS-232 remote-button simulator.
- **Capability check.** The source note "Check User Manual for your specific TV to identify supported IR commands by model" warns that the command set is not uniform across all listed models. `compatible_with.models` includes the six 55–86" chassis named in the source; the requested `116U75QG` model is not enumerated in the source manual and is included by family association only.
- **Case sensitivity.** The protocol is case sensitive per the source.
