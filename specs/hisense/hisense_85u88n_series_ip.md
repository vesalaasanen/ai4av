---
spec_id: admin/hisense-85u88n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 85U88N Series Control Spec"
manufacturer: HiSense
model_family: "85U88N Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
generated_at: 2026-06-25T19:09:09.873Z
...
# HiSense 85U88N Series Control Spec
...
<!-- UNRESOLVED: the supplied source document contains NO TCP/IP / network transport detail (no IP command set, no port, no telnet/REST). Although this family is marked "TCP/IP" in the task metadata, only RS-232C serial is substantiated by the source. -->
<!-- UNRESOLVED: the source is a generic "Hisense Prosumer TV" protocol manual and does not name the 85U88N Series model explicitly; model binding is asserted from the task, not from the document text. -->
...
<!-- UNRESOLVED: no TCP/IP / network control path is documented in the supplied source, despite the family being flagged TCP/IP in task metadata. A separate network-control document would be needed to populate a `tcp`/`http` transport block. -->
<!-- UNRESOLVED: protocol document version is 3.6 (2017-04-17); no device firmware version compatibility range is stated. -->
<!-- UNRESOLVED: the IR table is not enumerated as actions in this revision (serial-only focus); add an `ir` transport/actions pass if IR control is required. -->
