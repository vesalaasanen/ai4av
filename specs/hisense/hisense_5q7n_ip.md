---
spec_id: admin/hisense-5q7n
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5Q7N Control Spec"
...
source_urls:
  ...
  - https://www.scribd.com/document/885282231/Hisense-Ip-Control-Ds-Dm66d-Gm50d-v1-1
retrieved_at: 2026-05-14T10:38:48.072Z
last_checked_at: 2026-06-25T09:20:00.115Z
generated_at: 2026-06-25T09:20:00.115Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source is a pure command table — no prose section stating transport (RS-232 vs IP), baud rate, parity, port, or auth. The filename and prior discovery memo imply the device supports IP control too, but no IP framing, port, base URL, or HTTP/TCP envelope is documented in this excerpt. The 4-byte HEX prefix pattern is consistent with HiSense's documented RS-232 wire format, but that mapping is inferred from prior knowledge, not stated in this source."
  ...
  - "known protocol \"TCP/IP\" was given as a hint for this device family, but the source excerpt documents no IP framing, TCP port, HTTP base URL, or REST method — only the HEX/ASCII command catalogue. If the 5Q7N also accepts these commands over IP, the IP envelope, port, and base URL are not stated here."
  - "source is a command catalogue only; no safety warnings,"
  - "checksum algorithm not documented in this source excerpt."
  - "no section of this excerpt explicitly names the 5Q7N model."
  - "TCP/IP framing, port, base URL, and auth not documented in this source excerpt."
verification:
  verdict: verified
  checked_at: 2026-06-25T09:20:00.115Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 5Q7N Control Spec

## Summary
The HiSense 5Q7N is a consumer television exposing RS-232-based ASCII command/response control with HEX-on-the-wire framing (4-byte MAC-or-tag prefix + `ALL` sentinel + 4-char mnemonic + 4-char param + 2-byte checksum + `0D` terminator). The source document is a command-table excerpt from the Hisense RS-232 / IR External Control protocol family; the same command set is presented in two HEX forms per row — a TV-specific form keyed on the last three digits of the unit's MAC address (column header example: `465`) and a generic form using the literal `ALL` tag in place of the MAC digits. This spec covers the full command catalogue: power, input selection, picture/sound tuning, volume/mute, tuner, channel, closed-caption, OSD language, remote/panel/menu locks, power-on behavior, speaker routing, USB behavior, pixel shifting, factory reset, and remote-button emulation.

<!-- UNRESOLVED: source is a pure command table — no prose section stating transport (RS-232 vs IP), baud rate, parity, port, or auth. The filename and prior discovery memo imply the device supports IP control too, but no IP framing, port, base URL, or HTTP/TCP envelope is documented in this excerpt. The 4-byte HEX prefix pattern is consistent with HiSense's documented RS-232 wire format, but that mapping is inferred from prior knowledge, not stated in this source. -->

## Transport
```yaml
protocols:
  - serial  # inferred: 4-byte HEX prefix + ASCII mnemonic + 4-char param + 2-byte checksum + 0D terminator is the documented HiSense RS-232 wire format; source does not explicitly state "RS-232" in this excerpt
serial:
...
<!-- UNRESOLVED: known protocol "TCP/IP" was given as a hint for this device family, but the source excerpt documents no IP framing, TCP port, HTTP base URL, or REST method — only the HEX/ASCII command catalogue. If the 5Q7N also accepts these commands over IP, the IP envelope, port, and base URL are not stated here. -->

## Traits
```yaml
- powerable  # inferred from POWR, PWRE commands
- routable  # inferred from INPT input-selection and POIS power-on input commands
- queryable  # inferred from per-feature ???? query commands
- levelable  # inferred from VOLM, BRIT, CONT, COLR, TINT, SHRP, BKLV parameter commands
```
...
## Variables
```yaml
# Settable scalar parameters (all carried as 4-digit ASCII zero-padded values):
#
# - id: brightness
#   command_template: "BRIT{0000-0100}"
#   range: 0..100
# - id: contrast
#   command_template: "CONT{0000-0100}"
#   range: 0..100
# - id: color_saturation
#   command_template: "COLR{0000-0100}"
#   range: 0..100
# - id: tint
#   command_template: "TINT{0000-0100}"
#   range: 0..100
# - id: sharpness
#   command_template: "SHRP{0000-0020}"
#   range: 0..20
# - id: backlight
#   command_template: "BKLV{0000-0100}"
#   range: 0..100
# - id: volume
#   command_template: "VOLM{0000-0100}"
#   range: 0..100
# - id: max_volume_range
#   command_template: "MAVL{0000-0100}"
#   range: 0..100
# - id: volume_locked_level
#   command_template: "VLFL{0000-0100}"
#   range: 0..100
```

## Events
```yaml
# Source does not document unsolicited notifications / async events. All
# exchanges in the table are request/response. No Events section emitted.
```

## Macros
```yaml
# Source does not document multi-step macros or sequences.
```
...
interlocks: []
# UNRESOLVED: source is a command catalogue only; no safety warnings,
# interlock procedures, or power-on sequencing requirements are documented.
# The PBTN (POWER OFF CONTROL MODE: AC ONLY vs ALL) command alters whether
# the TV responds to power commands in certain states, but no explicit safety
# prose is provided.
```

## Notes
**Wire format (inferred from command-table HEX columns).** Every row in the source table documents the command in two parallel HEX forms:

- **TV-specific form**: `53 34 36 35` + ASCII body + 2-byte checksum + `0D`. The four-byte prefix decodes to ASCII `S465` — i.e. literal `S` followed by the last three hex/decimal digits of the unit's MAC address. The column header example is `465`.
- **Generic form**: `53 41 4C 4C` + same ASCII body + same checksum algorithm + `0D`. The four-byte prefix decodes to ASCII `SALL` — the broadcast / all-units tag.

The 2-byte checksum immediately precedes the `0D` carriage-return terminator and changes per command and per unit tag; the algorithm is not stated in this excerpt but is the standard Hisense RS-232 XOR/sum checksum. **UNRESOLVED: checksum algorithm not documented in this source excerpt.**

**Request/response symmetry.** Every "set" command (4-char mnemonic + 4-digit zero-padded parameter) has a matching "query" form that replaces the 4-digit parameter with `????`. The query response is the same wire frame with the parameter field replaced by the current numeric value.

**Hex/ASCII dual encoding.** The 4-character mnemonics in the `Command` column are plain ASCII (e.g. `POWR`, `INPT`, `BRIT`); the 4-digit parameter values are ASCII digits (e.g. `0001`, `0050`); only the per-row HEX column on the right expresses the full on-the-wire frame including MAC tag and checksum.

**5Q7N family applicability.** The source document title references Hisense commercial/B2B display control and is shared across many Hisense TV models. The 5Q7N model is not named in this excerpt; command-set applicability to the 5Q7N is inferred from the device-family relationship noted in prior discovery work. **UNRESOLVED: no section of this excerpt explicitly names the 5Q7N model.**

**Known IP protocol gap.** The task brief flags `Known protocol: TCP/IP` for this family, but this excerpt contains no IP framing, port number, HTTP base URL, REST/JSON-RPC method, or TCP envelope. The IP-control interface for the 5Q7N is documented in a separate Hisense B2B PDF (`downloadId=784`), not in this source. **UNRESOLVED: TCP/IP framing, port, base URL, and auth not documented in this source excerpt.**
