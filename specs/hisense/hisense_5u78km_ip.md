---
spec_id: admin/hisense-5u78km
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U78KM Control Spec"
...
retrieved_at: 2026-05-04T09:02:57.630Z
last_checked_at: 2026-06-25T16:16:29.135Z
generated_at: 2026-06-25T16:16:29.135Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source is command table only — no connection/port, framing intro, or setup narrative present. Port not stated in source."
  - "port number not stated in source"
  - "TCP port not stated in source (recovery notes hint 5000/8088 from unrelated models — not used)."
  - "connection setup / handshake / keepalive not documented in source."
  - "response frame format for queries beyond return-value column not documented (OKAY for sets; numeric/enum for queries)."
  - "error / NAK frame format not documented."
  - "firmware version compatibility not stated."
  - "checksum algorithm not documented (appears 8-bit sum-derived; treat each row's byte as authoritative, do not recompute)."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:16:29.135Z
  matched_actions: 196
  action_count: 196
  confidence: medium
...
# HiSense 5U78KM Control Spec

## Summary
HiSense 5U78KM commercial display, IP (TCP) control via hex command frames. Source = IP control command catalogue: power, input select, picture/audio settings, volume, mute, tuner, captions, OSD, lock/mode settings, remote-key emulated IR buttons. Each command emitted as ASCII mnemonic + 4-digit param, wrapped in hex frame with TV address + checksum.

<!-- UNRESOLVED: source is command table only — no connection/port, framing intro, or setup narrative present. Port not stated in source. -->
...
## Traits
```yaml
# - powerable   # inferred: POWR0000/0001 + PWRE commands present
# - queryable   # inferred: many "????" query variants present
# - levelable   # inferred: VOLM, BRIT, CONT, COLR, TINT, SHRP, BKLV ranged values
```
...
## Variables
```yaml
# Ranged set-values (VOLM, BRIT, CONT, COLR, TINT 0000-0100; SHRP 0000-0020;
# BKLV 0000-0100; MAVL, VLFL 0000-0100) are represented as parameterized
# Actions in the merged set - no separate Variables needed.
```
...
interlocks: []
# No safety warnings or interlock procedures in source. RSET9999 (factory
# reset) and PWRE (remote power-on enable) are destructive/sensitive but
# source lists no confirmation requirement.
```

## Notes

**Frame structure** (every command row, same layout):
`{op} {addr} {MNEMONIC}{4-digit-param} {checksum} {0D}`

- `op`: `53` = SET, `51` = QUERY
- `addr`: TV-specific = ASCII MAC last 3 digits (e.g. `34 36 35` = "465"); Generic = `41 4C 4C` = "ALL"
- `mnemonic`: 4 ASCII chars (PWRE, POWR, INPT, PMOD, BRIT, CONT, COLR, TINT, SHRP, ASPT, OVSN, RSTP, CTEM, BKLV, AMOD, RSTA, VOLM, MUTE, ASPK, TUNR, TSCN, CHAN, CC##, RSET, LANG, PLED, BTTN, PBTN, MAVL, SVOL, VLFL, RMOT, PANL, MENU, AVMN, OSD#, INPM, POIS, SPKM, B2BM, USBM, PSHF)
- `param`: 4 ASCII digits (set) or `3F 3F 3F 3F` = "????" (query)
- `checksum`: single byte (varies by addr+content; copy verbatim from source row)
- terminator: `0D` = CR

**Two addressing modes** — source gives both per row:
1. TV-specific (MAC last 3 digits) — target one display
2. Generic "ALL" — broadcast to all displays on link

Use the matching column for the use case; checksums differ between the two.

**IR-key emulation** (BTTN####, codes 1000–1055): emulate physical remote buttons (digits, CH+/-, VOL+/-, color keys, transport keys, etc.). Same frame layout.

<!-- UNRESOLVED: TCP port not stated in source (recovery notes hint 5000/8088 from unrelated models — not used). -->
<!-- UNRESOLVED: connection setup / handshake / keepalive not documented in source. -->
<!-- UNRESOLVED: response frame format for queries beyond return-value column not documented (OKAY for sets; numeric/enum for queries). -->
<!-- UNRESOLVED: error / NAK frame format not documented. -->
<!-- UNRESOLVED: firmware version compatibility not stated. -->
<!-- UNRESOLVED: checksum algorithm not documented (appears 8-bit sum-derived; treat each row's byte as authoritative, do not recompute). -->
