---
spec_id: admin/hisense-65u75qg-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 65U75QG Series Control Spec"
manufacturer: HiSense
model_family: "65U75QG Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-05-14T10:38:54.386Z
last_checked_at: 2026-06-25T09:20:02.813Z
generated_at: 2026-06-25T09:20:02.813Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source provides only the command table — no device overview, no protocol handshake description, no connection setup."
  - "source does not state the TCP/UDP port, transport framing rules beyond the 0x0D terminator byte, baud rate, or connection handshake. Serial parameters (baud/data/parity/stop) are not present in the command-table source. Auth/login procedure is not described — emit `auth.type: none` as a Tier-2 inference."
  - "source documents only discrete command rows (opcodes with numeric"
  - "source describes only request/response command pairs. No"
  - "no multi-step macro sequences described in source."
  - "source is a bare command table - no safety warnings, interlocks,"
verification:
  verdict: verified
  checked_at: 2026-06-25T09:20:02.813Z
  matched_actions: 196
  action_count: 196
  confidence: medium
  summary: "deterministic presence proof: 196/196 payloads verbatim in source; stratified Sonnet sample corroborated (6 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 65U75QG Series Control Spec

## Summary
Control spec for the HiSense 65U75QG Series TV. Source is a vendor IP-control command catalogue providing ASCII mnemonics paired with literal HEX byte frames for both per-TV (MAC-address-keyed) and broadcast ("SALL") transmission. Covers power, input selection, picture/sound tuning, lock/menu modes, and IR-button emulation.

<!-- UNRESOLVED: source provides only the command table — no device overview, no protocol handshake description, no connection setup. -->

## Transport
```yaml
# Source describes commands as ASCII mnemonics + HEX byte sequences (RS-232-style
# framing with 0x0D terminator). The "SALL" generic variant and "53 34 36 35 ..."
# per-TV variant (last 3 digits = MAC last 3) suggest a UDP or TCP broadcast
# control plane, but the transport type, port, IP framing, and discovery are not
# documented in the source. Inferring protocols would be unsafe.
#
protocols:
  - tcp  # inferred: user supplied "TCP/IP" as known protocol; no further evidence in source
```

<!-- UNRESOLVED: source does not state the TCP/UDP port, transport framing rules beyond the 0x0D terminator byte, baud rate, or connection handshake. Serial parameters (baud/data/parity/stop) are not present in the command-table source. Auth/login procedure is not described — emit `auth.type: none` as a Tier-2 inference. -->

## Traits
```yaml
# Source evidence supports:
# - powerable       (POWR0000/0001 - Stand by / Power on; PWRE for RS-232 enable)
# - routable        (INPT0000..0012 - input source selection across TV/AV/Component/HDMI1-4/VGA)
# - queryable       (INPT????, PMOD????, BRIT????, CONT????, COLR????, TINT????,
#                    SHRP????, ASPT????, OVSN????, CTEM????, BKLV????, AMOD????,
#                    VOLM????, MUTE????, ASPK????, TUNR????, CC##????, LANG????,
#                    PLED????, PBTN????, MAVL????, SVOL????, VLFL????, RMOT????,
#                    PANL????, MENU????, AVMN????, OSD#????, INPM????, POIS????,
#                    SPKM????, B2BM????, USBM????, PSHF????)
# - levelable       (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM - picture + volume scalars)
```
...
## Variables
```yaml
# UNRESOLVED: source documents only discrete command rows (opcodes with numeric
# parameter values). No persistent variable schema beyond the inline numeric
# parameter ranges (e.g. BRIT 0000-0100) is described.
```

## Events
```yaml
# UNRESOLVED: source describes only request/response command pairs. No
# unsolicited event / async notification mechanism documented.
```
...
interlocks: []
# UNRESOLVED: source is a bare command table - no safety warnings, interlocks,
# power-on sequencing requirements, or fault-handling procedures described.
```

## Notes

**Frame structure.** Two command frames per row in the source:
1. **Per-TV frame** — ASCII hex beginning `53 34 36 35` (= "S465"; the example
   MAC last-3 digits). Length varies; trailing two bytes are checksum + `0D`
   (CR) terminator.
2. **Generic "SALL" broadcast frame** — ASCII hex beginning `53 41 4C 4C`
   (= "SALL"). Same trailing checksum + `0D` terminator. Use this when targeting
   an unknown TV on the LAN.

**Mnemonics.** The first ASCII bytes of each frame are the printable mnemonic
(e.g. `POWR`, `INPT`, `PMOD`, `BRIT`, `VOLM`, `MUTE`, `BTTN…`). The 4-digit
parameter tail encodes the action variant (e.g. `0001` = on, `0002` = variant
2, `????` = query).

**Two syntaxes observed in raw mnemonics:** bracketed-Chinese parens `BRIT（0000）`
(mis-encoded fullwidth parentheses) and unbracketed forms `BRIT(0000)`. Treat
both as the same parameterised command.

**IR-button emulation.** The `BTTN10xx` row block covers 45+ remote-control key
codes (CH±, VOL±, digits, colour keys, transport keys, d-pad). These are
remote-equivalent commands, not device-state operations.

**Open gaps to revisit.** No evidence in source for: connection setup, port,
discovery, login/auth, firmware version compatibility, or response parsing
beyond the literal `OKAY` acknowledgement.
