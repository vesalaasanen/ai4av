---
spec_id: admin/hisense-5u88l
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 5U88L Control Spec"
...
retrieved_at: 2026-05-04T09:16:51.136Z
last_checked_at: 2026-06-25T17:49:51.291Z
generated_at: 2026-06-25T17:49:51.291Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source file is named \"..._ip.refined.md\" and the protocol uses MAC-address-based client IDs, implying a network/IP transport variant exists, but the provided source text only documents the RS-232C serial transport. TCP port, base URL, and IP framing are not stated in this source."
  - "supported model series list in source is blank (no model enumeration)."
  - "firmware version compatibility not stated in source."
  - "source filename and MAC-based client ID suggest a TCP/IP transport"
  - "source does not enumerate settable parameters as a distinct variable"
  - "source documents no unsolicited notifications; the TV only ACKs commands."
  - "source documents no multi-step sequences."
  - "TCP/IP transport (port, framing, connection model) referenced by filename but not documented in source text."
  - "HEX command tables reference two columns (MAC-specific vs ALL-broadcast); this spec does not yet declare which addressing mode a caller should default to — operator decision."
  - "source contains an \"OSD MODE\", \"INPUT MODE\", and \"POWER ON INPUT SELECTION\" command block (OSD#/INPM#/POIS#) appended as unparsed prose at the end of the table; these may need separate authoring if the deterministic pre-pass did not capture them."
verification:
  verdict: verified
  checked_at: 2026-06-25T17:49:51.291Z
  matched_actions: 165
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (10 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 5U88L Control Spec

## Summary
HiSense Prosumer TV (model 5U88L) controlled via an ASCII, fixed-length serial protocol over an RS-232C DB9 connector. The protocol supports set (S) and query (Q) operations addressed by a 3-byte client ID (last 3 bytes of Ethernet MAC for Smart TVs, or menu-selected ID for Feature TVs; `ALL` for broadcast). Commands span power, input selection, picture/audio settings, aspect ratio, tuner, captions, OSD language, remote-key/panel-key/menu lockout, and remote-button simulation. The source document is titled "RS-232/IR Protocol for Hisense® Prosumer TV" and additionally contains a discrete IR (Pronto CCF) code catalogue not covered by this serial spec.

<!-- UNRESOLVED: source file is named "..._ip.refined.md" and the protocol uses MAC-address-based client IDs, implying a network/IP transport variant exists, but the provided source text only documents the RS-232C serial transport. TCP port, base URL, and IP framing are not stated in this source. -->
<!-- UNRESOLVED: supported model series list in source is blank (no model enumeration). -->
...
protocols:
  - serial
  # UNRESOLVED: source filename and MAC-based client ID suggest a TCP/IP transport
  # variant, but the provided text does not document TCP port, base URL, or framing.
serial:
  baud_rate: 9600
  data_bits: 8
  parity: none
...
```yaml
traits:
  - powerable    # inferred: POWR0000/POWR0001 power commands present
  - queryable    # inferred: numerous ???? query commands present
  - levelable    # inferred: VOLM/BRIT/CONT/COLR/TINT/SHRP/BKLV level commands present
...
## Variables
```yaml
# UNRESOLVED: source does not enumerate settable parameters as a distinct variable
# list; all settable values are represented as discrete Actions (see deterministic Actions).
```

## Events
```yaml
# UNRESOLVED: source documents no unsolicited notifications; the TV only ACKs commands.
```
...
destructive_actions:
  - restore_factory_settings
warnings:
  - "RS-232 port must be enabled in the Custom Install menu before serial control works; access the menu via Quick Settings then numeric string 7310."
  - "Power On Command (PWRE) must be set to Enable for the TV to respond to RS-232 power-on while in standby; the PWRE query is not available in STANDBY mode."
interlocks: []
cooldowns: []
```

## Notes
- **Protocol is case-sensitive** (source note 1). All mnemonics are uppercase ASCII.
- **Frame format (fixed length):** Operation (1B: `S`/`Q`) · Client ID (3B) · Command (4B) · Data (4B) · Checksum (1B) · Termination (1B CR). Client ID examples: `5FA`, `465`, `ALL`.
- **Acknowledgement:** `ClientID:ACK DATA CHECKSUM CR` where ACK ∈ {`OKAY`, `EROR`, `WAIT`}. Some operations (e.g. POWRON) return `WAIT` then `OKAY`.
- **Checksum:** 8-bit sum over the full byte sequence such that the running sum including the checksum byte == 0.
- **Addressing modes:** TV-specific (MAC last 3 digits, e.g. `465`) vs. generic broadcast (`ALL`); each command table row lists both hex forms.
- **DB9 pinout (female chassis):** 1.N/C 2.RXD 3.TXD 4.DTR 5.GND 6.DSR 7.RTS 8.CTS 9.Power Input. USB-to-serial adapter required for PC/host connection (sold separately).
- **Revision history in source:** V1.0–V3.6 (2014–2017); notable V3.1 changed PANL and BTTN prefixing; V3.2 added multi-TV MAC addressing; V3.3 added error-correction commands.
- **Discrete IR (Pronto CCF) codes** are also documented in the source but are out of scope for this serial/IP spec.

<!-- UNRESOLVED: TCP/IP transport (port, framing, connection model) referenced by filename but not documented in source text. -->
<!-- UNRESOLVED: HEX command tables reference two columns (MAC-specific vs ALL-broadcast); this spec does not yet declare which addressing mode a caller should default to — operator decision. -->
<!-- UNRESOLVED: source contains an "OSD MODE", "INPUT MODE", and "POWER ON INPUT SELECTION" command block (OSD#/INPM#/POIS#) appended as unparsed prose at the end of the table; these may need separate authoring if the deterministic pre-pass did not capture them. -->
````

Spec emit done. 3 UNRESOLVED flags worth checking:

1. **TCP/IP** — filename `_ip` + MAC client-ID hint at network variant, but source text only documents RS-232C. Marked UNRESOLVED, emitted `serial` only.
2. **OSD#/INPM#/POIS# block** — lines 426–428 in source render as unparsed prose (table broke). Pre-pass likely missed them. Worth re-running extraction if you want full coverage.
3. **Safety shape fix** — `interlocks: []` (empty string array, not nested objects), `cooldowns: []` as `{action_id,duration_ms}` array empty. Parser should accept now.

Actions/Feedbacks bodies intentionally empty per pre-pass merge.
