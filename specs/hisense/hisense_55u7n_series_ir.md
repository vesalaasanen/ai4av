---
spec_id: admin/hisense-55u7n-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 55U7N Series Control Spec"
manufacturer: HiSense
model_family: "55U7N Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
...
retrieved_at: 2026-06-25T17:08:46.329Z
last_checked_at: 2026-06-25T17:49:46.238Z
generated_at: 2026-06-25T17:49:46.238Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source manual is a generic Prosumer TV protocol doc dated 2017; specific 55U7N model coverage / firmware compatibility not stated. Discrete IR command set not authored here."
  - "no TCP/IP control protocol documented; network/MAC is only used as CLIENT ID source. Discrete IR transport (Pronto CCF) not represented in protocols[] (no IR entry in enum) — documented in Notes."
...
verification:
  ...
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (6 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 55U7N Series Control Spec

## Summary
HiSense Prosumer TV (applied to the 55U7N Series) controlled via an RS-232C serial link through a DB9 connector. This spec covers the fixed-length ASCII serial command/query catalogue (power, input, picture, audio, tuner, OSD, and remote/panel-lock controls) documented in the vendor "RS-232/IR Protocol" manual (v3.6, 2017). Discrete IR (Pronto CCF) codes are also present in the source but are out of scope for this revision.

<!-- UNRESOLVED: source manual is a generic Prosumer TV protocol doc dated 2017; specific 55U7N model coverage / firmware compatibility not stated. Discrete IR command set not authored here. -->
...
serial:
  ...
  stop_bits: 1
  flow_control: none
  # encoding: ASCII (stated in source: "Communication Code: ASCII code")
  # connector: DB9 D-sub female chassis mount (TV side)
auth:
  type: none  # inferred: no auth procedure in source
```

<!-- UNRESOLVED: no TCP/IP control protocol documented; network/MAC is only used as CLIENT ID source. Discrete IR transport (Pronto CCF) not represented in protocols[] (no IR entry in enum) — documented in Notes. -->

## Traits
```yaml
traits:
  - powerable  # inferred: POWR0000/0001 power standby/on commands
  - queryable  # inferred: extensive Q...???? query commands return state
  - levelable  # inferred: continuous-value sets (brightness/contrast/color/backlight/volume 0-100)
```
...
## Variables
```yaml
# Continuous parameters (brightness/contrast/color/tint/sharpness/backlight/volume/
# max-volume/locked-level) are already represented as parameterized set ACTIONS
# (BRIT/CONT/COLR/TINT/SHRP/BKLV/VOLM/MAVL/VLFL with 0000-0100 / 0000-0020 ranges),
...
# Note: RSET9999 restores factory settings and is destructive - source does not flag
# it as confirmation-gated, so it is not listed here by inference.
```

## Notes

**Physical interface:** DB9 D-sub, female chassis-mount on the TV.
...
- USB-to-serial adapter required for PC/controller USB ports.

**Electrical:** RS-232C compliant. 9600 bps UART, 8 data bits, no parity, 1 stop bit, no flow control, ASCII.

**TV-side setup required before serial control works:**
1. Power on, press Quick Settings Menu key on remote.
2. Enter numeric string `7310` to open the Custom Install menu.
3. Set Custom Installation → Enable (enables the RS-232 port).
4. Optionally set Power On Command → Enable to keep RS-232 active in standby (allows remote power on/standby via serial).

**Frame format (fixed-length, case sensitive):**
- Command (PC→TV): `[OP][CLIENT_ID 3B][COMMAND 4B][DATA 4B][CHECKSUM 1B][TERM 1B]`
  - OP: `S`=Set, `Q`=Query
  - CLIENT_ID: `0-9,A-F`; Smart TV = last 3 bytes of Ethernet MAC; Feature TV = selected in TV menu; `ALL` = broadcast (`53414C4C`)
  - COMMAND: `A-Z` mnemonic
  - DATA: `0-9,A-Z,#,?` (`????` for queries)
  - CHECKSUM: 8-bit; sum of entire string including the checksum byte == 0
  - TERM: CR (0x0D)
- ACK (TV→PC): `[CLIENT_ID 3B]:[ACK 4B][DATA 4B][CHECKSUM 1B][TERM 1B]`; common ACKs: `OKAY`, `EROR`, `WAIT`
- Source provides both TV-specific hex (MAC last-3-digits, e.g. `465`) and generic broadcast hex (`ALL`) for every command.

**Examples from source:**
- Power on set `5FA`: `S5FAPOWR01##[CS]CR` → `5FA:WAIT####[CS]CR` then `5FA:OKAY####[CS]CR`
- Query input of set `5FA`: `Q5FAINPT????[CS]CR` → `5FA:OKAYHDM1[CS]CR`

**Hex command columns:** Source lists per-command both the "TV Specific HEX command" (client id = MAC last 3 digits, e.g. `53 34 36 35 ...`) and the "Generic HEX Command (all TVs)" (client id = `ALL` = `53 41 4C 4C ...`). The deterministic action payloads carry the documented mnemonics verbatim.

<!-- UNRESOLVED: Discrete IR Control section (Pronto CCF codes for power on/off, inputs HDMI.1-5/VGA/USB, picture/sound toggle, aspect ratios, channel list, digits 0-9, nav arrows, PIP, guide, freeze, etc.) not authored as actions — IR transport not in protocols enum and out of scope this revision. -->
<!-- UNRESOLVED: Additional OSD MODE (OSD#), INPUT MODE (INPM), POWER ON INPUT SELECT (POIS) and any commands past page 17 in the picture-text block not enumerated by the deterministic extraction — verify completeness against full source. -->
<!-- UNRESOLVED: firmware version compatibility not stated; 2017 manual applied to a newer 55U7N Series model — confirm command set matches actual device firmware. -->
