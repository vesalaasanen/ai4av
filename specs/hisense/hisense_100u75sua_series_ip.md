---
spec_id: admin/hisense-100u75sua-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "HiSense 100U75SUA Series Control Spec"
manufacturer: HiSense
model_family: "HiSense 100U75SUA Series"
aliases: []
compatible_with:
  manufacturers:
    - HiSense
  models:
    - "HiSense 100U75SUA Series"
  firmware: ""
  hardware_revisions: []
  protocol_versions: []
  required_options: []
...
retrieved_at: 2026-06-25T16:25:31.709Z
last_checked_at: 2026-06-25T16:56:38.561Z
generated_at: 2026-06-25T16:56:38.561Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "TCP port / TCP transport details not stated in source."
  - "specific firmware version compatibility not stated in source."
  - "exact supported model list (the source's Models section is blank in the extracted text)."
  - "no TCP port stated in source"
  - "nothing additional to populate from source."
  - "no unsolicited event mechanism documented in source."
  - "nothing to populate from source."
  - "source contains no safety warnings, interlock procedures, or power-on sequencing"
  - "TCP transport details (port, framing-over-TCP, auth) not stated in source."
  - "firmware version compatibility range not stated in source."
  - "exact supported model list not extractable from source (Models section blank)."
  - "data ranges for some level commands inferred from example values only where the source gives explicit bounds (BRIT/CONT/COLR/TINT/BKLV/VOLM/MAVL/VLFL = 0000–0100; SHRP = 0000–0020)."
verification:
  verdict: verified
  checked_at: 2026-06-25T16:56:38.561Z
  matched_actions: 165
  action_count: 165
  confidence: medium
  summary: "deterministic presence proof: 165/165 payloads verbatim in source; stratified Sonnet sample corroborated (12 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# HiSense 100U75SUA Series Control Spec

## Summary
Control spec for the HiSense 100U75SUA Series prosumer TV. Covers the ASCII fixed-length control protocol with an 8-bit checksum framing — power, input source selection, picture/audio settings, tuner, caption, OSD, volume/panel-lock controls, and remote-key simulation — plus status queries for most settable parameters. The source document ("RS-232/IR Protocol for Hisense® Prosumer TV") explicitly documents the RS-232C serial transport (9600 8N1, ASCII, no flow control); it also references Ethernet MAC-address client IDs and network connection, and the operator indicates TCP/IP control is supported, but the source states no TCP port, base URL, or login procedure.

<!-- UNRESOLVED: TCP port / TCP transport details not stated in source. -->
<!-- UNRESOLVED: specific firmware version compatibility not stated in source. -->
<!-- UNRESOLVED: exact supported model list (the source's Models section is blank in the extracted text). -->

## Transport
```yaml
protocols:
  - serial   # explicitly documented in source (RS-232C)
  - tcp      # operator-declared; source references Ethernet MAC client IDs + network connection, but states no port
serial:
  baud_rate: 9600          # "9600bps (UART)"
  data_bits: 8             # "Data Length 8bits"
  parity: none             # "Parity Bit None"
  stop_bits: 1             # "Stop Bit 1bit"
  flow_control: none       # "Flow Control None"
  encoding: ascii          # "Communication Code ASCII code"
addressing:
  port: null  # UNRESOLVED: no TCP port stated in source
auth:
...
## Traits
```yaml
- powerable   # inferred: POWR power on/off + PWRE remote power commands present
- queryable   # inferred: many ???? status-query commands present
- levelable   # inferred: VOLM/BRIT/CONT/BKLV/COLR/TINT/SHRP level commands present
...
## Variables
```yaml
# No settable parameters beyond those already represented as discrete Actions/Feedbacks by the
# deterministic extraction (level ranges such as VOLM/BRIT/CONT/BKLV/COLR/TINT/SHRP/MAVL/VLFL).
# UNRESOLVED: nothing additional to populate from source.
```

## Events
```yaml
# No unsolicited notifications described in source. ACK tokens (OKAY/EROR/WAIT) are solicited
# responses to commands, not push events.
# UNRESOLVED: no unsolicited event mechanism documented in source.
```

## Macros
```yaml
# No multi-step command sequences described explicitly in source.
# UNRESOLVED: nothing to populate from source.
```
...
# requirements for the control interface. Note: a factory-restore command exists (RSET9999) that
# resets the device to function mode - treat as destructive, though the source documents no
# required confirmation/interlock for it.
```

## Notes

### Protocol framing
Fixed-length ASCII frames. A command (PC/controller → TV) is:

`OPERATION_DIRECTION(1B) CLIENT_ID(3B) COMMAND(4B) DATA(4B) CHECKSUM(1B) TERMINATION(1B)`

An acknowledgement (TV → controller) is:

`CLIENT_ID(3B) : ACK(1B+) DATA(4B) CHECKSUM(1B) TERMINATION(1B)`

- **Operation direction:** `S` = Set, `Q` = Query.
- **Client ID:** for Smart TV, the last 3 bytes of the Ethernet MAC address; for Feature TV, selected in the TV menu. `ALL` = broadcast to all TVs.
- **Command:** 4 ASCII letters A–Z (e.g. `PWRE`, `POWR`, `INPT`, `VOLM`, `BTTN`).
- **Data:** 4 chars from `0-9`, `A-Z`, `#`, `?`. `????` denotes a query placeholder.
- **Checksum:** 8-bit checksum of the byte sequence (including the checksum byte) such that the whole string sums to zero.
- **Termination:** Carriage Return, `0x0D`.
- **Colon / ACK:** ASCII `0x3A` separates Client ID from ACK in responses. Common ACK tokens: `OKAY`, `EROR`, `WAIT`.
- **The protocol is case sensitive.**

### ASCII vs HEX forms
The source documents every command in two equivalent forms: ASCII mode (human-readable, e.g. `PWRE0000`) and HEX mode (space-separated hex bytes, e.g. `53 34 36 35 50 57 52 45 30 30 30 30 10 0D`). Both forms are provided per-command in the verbatim table; implementers may use either. A per-TV (MAC-derived, e.g. client `465`) and a generic broadcast (`ALL`) HEX variant are given for each command.

### Enabling RS-232 control
RS-232 must be enabled in the Custom Install menu before serial control works:
1. Power on the TV, press the Quick Settings Menu key on the remote.
2. When the Quick Settings menu appears, enter `7 3 1 0` on the number keys to open the Custom Install menu.
3. Set Custom Installation → Enable (enables the RS-232 port).
4. To allow RS-232 power-on from standby, set Power On Command → Enable before exiting. The `PWRE` query is not available while the TV is in standby.

### Physical interface
Standard DB9 D-sub, female chassis-mount on the TV. Male D-sub 9-pin pinout: 1 N/C, 2 RXD, 3 TXD, 4 DTR, 5 GND, 6 DSR, 7 RTS, 8 CTS, 9 Power Input. A USB-to-Serial controller (sold separately) is required to connect to a PC/controller USB port. Electrical definition: RS-232C compliant.

### Multi-TV systems
The same command set supports addressing a single TV (MAC-derived Client ID) and broadcasting to all TVs (`ALL`) on multi-TV installations. After connecting the TV to a network, Menu → Network → Network Information reveals the TV's MAC address.

### Examples (from source)
- Power on TV `5FA`: `S5FAPOWR0001` + checksum + `0x0D`; TV responds `5FA:WAIT####` then `5FA:OKAY####`.
- Query input of TV `5FA`: `Q5FAINPT????` + checksum + `0x0D`; TV responds `5FA:OKAYHDM1`.

### Out of scope
The source also documents Discrete IR codes (Pronto CCF hex and data-code tables for power, inputs, aspect ratios, digits, navigation, PIP, etc.). These are infrared remote codes, not the serial/IP control protocol, and are not represented in this serial/IP control spec.

<!-- UNRESOLVED: TCP transport details (port, framing-over-TCP, auth) not stated in source. -->
<!-- UNRESOLVED: firmware version compatibility range not stated in source. -->
<!-- UNRESOLVED: exact supported model list not extractable from source (Models section blank). -->
<!-- UNRESOLVED: data ranges for some level commands inferred from example values only where the source gives explicit bounds (BRIT/CONT/COLR/TINT/BKLV/VOLM/MAVL/VLFL = 0000–0100; SHRP = 0000–0020). -->
