---
spec_id: admin/cisco-sx80-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "Cisco SX80 Series Control Spec"
manufacturer: Cisco
model_family: "Cisco SX80"
aliases: []
compatible_with:
  manufacturers:
    - Cisco
...
source_domains:
  - cisco.com
source_urls:
  - https://www.cisco.com/c/dam/en/us/td/docs/telepresence/endpoint/sx-series/tc7/api-reference-guide/sx80-api-reference-guide-tc71.pdf
retrieved_at: 2026-06-24T23:19:19.732Z
last_checked_at: 2026-06-24T23:22:31.480Z
generated_at: 2026-06-24T23:22:31.480Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "firmware version compatibility not stated in source"
  - "flow control not stated in source"
  - "SSH/Telnet TCP port numbers not stated in source"
  - "xConfiguration entries act as settable persistent variables; the full"
...
verification:
  verdict: verified
  checked_at: 2026-06-24T23:22:31.480Z
  matched_actions: 794
  action_count: 794
  confidence: medium
  summary: "deterministic presence proof: 794/794 payloads verbatim in source; stratified Sonnet sample corroborated (8 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# Cisco SX80 Series Control Spec

## Summary
The Cisco SX80 is a TelePresence video conference codec intended for integrator control. This spec covers the codec API, which is organized into four hierarchical groups (Commands, Configurations, Status, Events) and is reachable over RS-232 serial, SSH, Telnet, and HTTP/HTTPS. The same API surface is exposed in line-based "terminal" form over serial/Telnet/SSH and in XML form over HTTP.

<!-- UNRESOLVED: firmware version compatibility not stated in source -->

## Transport
```yaml
# Source documents RS-232, SSH, Telnet, and HTTP/HTTPS access to the codec API.
# Serial: "Connect a standard 9-pin serial cable <115200, N, 8, 1> to the COM port
#   on the SX80 codec." - values stated verbatim.
# SSH: "SSH is a secure TCP/IP connection and it is enabled by default on the codec."
# Telnet: "Telnet is disabled by default. Before connecting ... you have to enable it."
# HTTP/HTTPS: POST/GET against /putxml, /getxml?location=<path>, /formputxml?xmldoc=<xml>.
# Flow control never stated for the serial port.
# TCP port numbers for SSH/Telnet not stated in the source.
protocols:
  - serial
  - tcp
  - http
...
addressing:
  port: null  # UNRESOLVED: SSH/Telnet TCP port numbers not stated in source
  base_url: "http://<ip-address>"  # placeholder host per source; paths: /putxml, /getxml?location=<path>, /formputxml?xmldoc=<xml>
auth:
  # Default admin user has no password set initially; login on serial is configurable
  # via xConfiguration SerialPort LoginRequired: <Off/On>. SSH password prompting is on
  # by default. No fixed credential scheme is stated, so default access is unauthenticated.
  type: none  # inferred: no default password / login optional in source
```

## Traits
```yaml
# Inferred from the command catalogue already authored into Actions:
# - powerable   (Standby Activate/Deactivate, Boot commands present)
# - routable    (audio/video local output connect/disconnect input, video layout assign present)
# - queryable   (xStatus queries for call/audio/video/camera state present)
# - levelable   (Audio Volume Set/Increase/Decrease, camera brightness/gamma levels present)
traits:
  - powerable  # inferred from standby/boot commands
  - routable   # inferred from audio/video routing commands
  - queryable  # inferred from xStatus query commands
  - levelable  # inferred from volume and camera level commands
```
...
- id: x_configuration_conference_active_control_mode
  label: "xConfiguration Conference ActiveControl Mode"
  kind: action
  command: "xConfiguration Conference ActiveControl Mode"
  params: []
...
- id: x_command_call_log_clear
  label: "xCommand CallLog Clear"
  kind: action
...
- id: x_command_call_log_missed_dismiss
  label: "xCommand CallLog Missed Dismiss"
  kind: action
  command: "xCommand CallLog Missed Dismiss"
  params: []
...
- id: x_command_key_press
  label: "xCommand Key Press"
  kind: action
...
- id: x_status_diagnostics_call_n_channels_outgoing_data_channel_n_netstat_1_last_interval_lost
  label: "xStatus Diagnostics Call [n] Channels OutgoingDataChannel [n] Netstat 1 LastIntervalLost"
  kind: query
  query_command: "xStatus Diagnostics Call [n] Channels OutgoingDataChannel [n] Netstat 1 LastIntervalLost"

- id: x_status_diagnostics_call_n_channels_outgoing_data_channel_n_netstat_1_last_interval_received
  label: "xStatus Diagnostics Call [n] Channels OutgoingDataChannel [n] Netstat 1 LastIntervalReceived"
  kind: query
  query_command: "xStatus Diagnostics Call [n] Channels OutgoingDataChannel [n] Netstat 1 LastIntervalReceived"
...
- id: x_status_media_channels_call_n_incoming_audio_channel_n_transport_rtp_local_port
  label: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Local Port"
...
- id: x_status_media_channels_call_n_incoming_audio_channel_n_transport_rtp_local_ip_address
  label: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Local IpAddress"

- id: x_status_media_channels_call_n_incoming_audio_channel_n_transport_rtp_remote_port
  label: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Remote Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTP Remote Port"
...
- id: x_status_media_channels_call_n_incoming_audio_channel_n_transport_rtcp_local_port
  label: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTCP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingAudioChannel [n] Transport RTCP Local Port"

- id: x_status_media_channels_call_n_incoming_video_channel_n_encryption_status
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Encryption Status"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Encryption Status"

- id: x_status_media_channels_call_n_incoming_video_channel_n_channel_role
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] ChannelRole"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] ChannelRole"
...
- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rtp_local_ip_address
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Local IpAddress"
...
- id: x_status_media_channels_call_n_incoming_video_channel_n_video_resolution_x
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Video ResolutionX"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Video ResolutionX"

- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rtp_local_port
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Local Port"
...
- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rtcp_local_port
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTCP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTCP Local Port"

- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rtp_remote_port
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Remote Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTP Remote Port"
...
- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rtcp_local_ip_address
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTCP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RTCP Local IpAddress"

- id: x_status_media_channels_call_n_incoming_video_channel_n_transport_rctp_remote_port
  label: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RCTP Remote Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] IncomingVideoChannel [n] Transport RCTP Remote Port"
...
- id: x_status_media_channels_call_n_outgoing_audio_channel_n_transport_rtp_local_ip_address
  label: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTP Local IpAddress"
...
- id: x_status_media_channels_call_n_outgoing_audio_channel_n_transport_rtcp_local_ip_address
  label: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTCP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTCP Local IpAddress"
...
- id: x_status_media_channels_call_n_outgoing_audio_channel_n_transport_rtcp_local_port
  label: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTCP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingAudioChannel [n] Transport RTCP Local Port"

- id: x_status_media_channels_call_n_outgoing_video_channel_n_encryption_status
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Encryption Status"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Encryption Status"
...
- id: x_status_media_channels_call_n_outgoing_video_channel_n_transport_rtp_local_ip_address
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Local IpAddress"
...
- id: x_status_media_channels_call_n_outgoing_video_channel_n_transport_rtp_local_port
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Local Port"

- id: x_status_media_channels_call_n_outgoing_video_channel_n_transport_rtcp_local_ip_address
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTCP Local IpAddress"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTCP Local IpAddress"
...
- id: x_status_media_channels_call_n_outgoing_video_channel_n_transport_rtcp_local_port
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTCP Local Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTCP Local Port"

- id: x_status_media_channels_call_n_outgoing_video_channel_n_transport_rtp_remote_port
  label: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Remote Port"
  kind: query
  query_command: "xStatus MediaChannels Call [n] OutgoingVideoChannel [n] Transport RTP Remote Port"
...
- id: x_status_network_1_dns_domain_name
  label: "xStatus Network 1 DNS Domain Name"
  kind: query
  query_command: "xStatus Network 1 DNS Domain Name"
...
- id: x_status_video_layout_site_n_output_n_frame_n_layer
  label: "xStatus Video Layout Site [n] Output [n] Frame [n] Layer"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] Frame [n] Layer"
...
- id: x_status_video_layout_site_n_output_n_frame_container_n_position_x
  label: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] PositionX"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] PositionX"

- id: x_status_video_layout_site_n_output_n_frame_container_n_position_y
  label: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] PositionY"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] PositionY"

- id: x_status_video_layout_site_n_output_n_frame_container_n_width
  label: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] Width"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] Width"

- id: x_status_video_layout_site_n_output_n_frame_container_n_height
  label: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] Height"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] FrameContainer [n] Height"
...
- id: x_status_video_layout_site_n_output_n_frame_n_frame_container_id
  label: "xStatus Video Layout Site [n] Output [n] Frame [n] FrameContainerId"
  kind: query
  query_command: "xStatus Video Layout Site [n] Output [n] Frame [n] FrameContainerId"
...
## Variables
```yaml
# UNRESOLVED: xConfiguration entries act as settable persistent variables; the full
# configuration hierarchy is covered by the merged Actions catalogue as x_configuration_*
# actions. No additional free-form variables documented in source.
```

## Events
```yaml
# Event notifications are delivered via the xFeedback mechanism over serial/Telnet/SSH
# and via HTTP POST (xCommand HttpFeedback Register, up to 4 slots, up to 38 expressions
# per session). Source documents example events:
#   OutgoingCallIndication, CallDisconnect, CallSuccessful, FeccActionInd,
#   TString message received, SString message received.
# The full event/status feedback catalogue is merged in by the deterministic pre-pass.
```
...
# power-on sequencing requirements. Some commands are inherently destructive
# (SystemUnit FactoryReset, SystemUnit SoftwareUpgrade, Standby Activate/Deactivate,
# Provisioning StartUpgrade) but no confirmation/interlock policy is stated in source.
```

## Notes
- **API structure**: four hierarchical groups — Commands (`xCommand`), Configurations (`xConfiguration`), Status (`xStatus`), Events (`xEvent`). Top-level help: `?`, `??`, or `help` after connecting.
- **Output modes**: terminal (line-based, default) and XML. Switch with `xPreferences outputmode xml` / `xPreferences outputmode terminal`. Feedback format is identical to the corresponding `xStatus`/`xConfiguration` query result.
- **Case sensitivity**: all commands are case-insensitive.
- **Quoting**: any value containing spaces must be quoted (e.g. `xCommand dial number: "my number contains spaces"`).
- **Value types**: `<x..y>` integer range; `<X/Y/../Z>` literal enum; `<S: x, y>` string with min/max length.
- **Serial config**: baud rate and login-required are set via `xConfiguration SerialPort BaudRate: <9600/19200/38400/57600/115200>` and `xConfiguration SerialPort LoginRequired: <Off/On>`; a codec reboot is required for these to take effect. Default baud rate is 115200.
- **HTTP services**: HTTP/HTTPS services toggled via `xConfiguration NetworkServices HTTP/HTTPS Mode: <Off/On>`; reboot required. Telnet toggled via `xConfiguration NetworkServices Telnet Mode: On` (no reboot required).
- **HTTP URLs** (per source table): `GET /status.xml`, `GET /configuration.xml`, `GET /command.xml`, `GET /valuespace.xml`, `GET /getxml?location=<path>`, `POST /putxml` (Content-Type: text/xml), `GET|POST /formputxml?xmldoc=<xml>`.
- **HTTP feedback**: codec POSTs feedback to a registered `ServerUrl`; payload mirrors polling XML plus an `Identification` node (SystemName, MACAddress, IPAddress, ProductType, ProductID, SWVersion, HWBoard, SerialNumber).
- **Feedback limits**: up to 38 expressions per session; expressions are per-session and must be re-registered after reconnect. Avoid `xFeedback register /Status` (flood risk). FeedbackSlot 3 should be avoided where Cisco TMS is in use.
- **Best practices from source**: prefer direct commands over `xCommand Key Press/Click` (remote-control emulation discouraged); always use full command paths (not `xconf vid` shortcuts); do NOT use `Experimental` command/status/configuration sections in production (not backwards compatible).

<!-- UNRESOLVED: firmware version compatibility not stated in source -->
<!-- UNRESOLVED: SSH/Telnet TCP port numbers not stated in source -->
