---
spec_id: admin/cisco-c-series-tc72
schema_version: ai4av-public-spec-v1
revision: 1
title: "Cisco C Series TC72 Control Spec"
manufacturer: Cisco
model_family: "Cisco C Series TC72"
aliases: []
compatible_with:
  manufacturers:
    - Cisco
...
source_urls:
  ...
  - https://community.cisco.com/t5/switching/unknown-protocol-drops/td-p/1064330
  - https://learningnetwork.cisco.com/s/question/0D53i00000Kt08ZCAR/unknown-protocol-drops
retrieved_at: 2026-05-12T22:10:51.244Z
last_checked_at: 2026-06-25T09:13:43.530Z
generated_at: 2026-06-25T09:13:43.530Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "source covers the TC7.2 software release and references multiple SX/C-series models without stating which commands apply to which sub-model; some configurations (e.g. Camera indices 1-7, Connector counts 1-5) may differ on the TC72 hardware."
  - "TCP port numbers for Telnet/SSH/HTTP/HTTPS not stated in source"
  - "source does not document multi-step macro sequences specific to the TC72."
  - "source mentions \"MotorMoveDetection\" warnings about manual camera adjustment losing pan/tilt feedback (C-Series), but no general safety interlocks are documented."
  - "default SIP ListenPort, HTTP/HTTPS port numbers, and Telnet port not stated in source."
  - "cipher suites, TLS versions, and SSH key exchange algorithms not stated in source."
  - "full HttpFeedback Register argument list — source shows 1-15 Expression slots but truncates the exact slot range."
verification:
  verdict: verified
  checked_at: 2026-06-25T09:13:43.530Z
  matched_actions: 840
  action_count: 840
  confidence: medium
  summary: "deterministic presence proof: 840/840 payloads verbatim in source; stratified Sonnet sample corroborated (7 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# Cisco C Series TC72 Control Spec

## Summary
The Cisco C Series TC72 codec exposes a line-based XML API for configuring and controlling a TelePresence video system. The same API is reachable over RS-232/serial (default 115200 8N1, login optional), Telnet, SSH, and HTTP/HTTPS, with HTTP using POST to `/putxml` and GET to `/getxml?location=<path>` for XML payloads. The API is structured around four top-level groups — Commands, Configurations, Status, Events — plus a feedback subscription mechanism via `xFeedback` or `xCommand HttpFeedback Register`.

<!-- UNRESOLVED: source covers the TC7.2 software release and references multiple SX/C-series models without stating which commands apply to which sub-model; some configurations (e.g. Camera indices 1-7, Connector counts 1-5) may differ on the TC72 hardware. -->
...
addressing:
  ...
  # UNRESOLVED: TCP port numbers for Telnet/SSH/HTTP/HTTPS not stated in source
auth:
  type: password  # default admin user has no password; xCommand SystemUnit AdminPassword Set Password assigns one
```

## Traits
```yaml
- powerable       # inferred from Standby Activate / Deactivate commands
- routable        # inferred from Video Input Set MainVideoSource and layout commands
- queryable       # inferred from xStatus command and extensive Status hierarchy
- levelable       # inferred from Audio Volume Set / Increase / Decrease / Mute commands
```
...
- id: x_configuration_h323_profile_1_1_encryption_min_key_size
  label: "xConfiguration H323 Profile [1..1] Encryption MinKeySize"
  kind: action
  command: "xConfiguration H323 Profile [1..1] Encryption MinKeySize"
  params: []
...
## Feedbacks
```yaml
- id: x_status_video_output_provides_information_about_the_connected_display
  label: "xStatus Video Output provides information about the connected display"
  kind: query
  query_command: "xStatus Video Output provides information about the connected display"
...
- id: x_status_diagnostics_call_n_channels_outgoing_video_channel_n_netstat_1_last_interval_received
  label: "xStatus Diagnostics Call [n] Channels OutgoingVideoChannel [n] Netstat 1 LastIntervalReceived"
  kind: query
  query_command: "xStatus Diagnostics Call [n] Channels OutgoingVideoChannel [n] Netstat 1 LastIntervalReceived"
...
## Variables
```yaml
# Settable parameters are represented as xConfiguration actions (merged into Actions by the deterministic pre-pass).
# No separate Variables body needed.
```
...
- id: call_disconnect
  type: event
  format: "*e CallDisconnect CallId: <id> CauseValue: <n> CauseString: \"<s>\" CauseType: <type> OrigCallDirection: \"<dir>\""
- id: call_successful
  type: event
  format: "*e CallSuccessful CallId: <id> Protocol: \"<proto>\" Direction: \"<dir>\" CallRate: <n> RemoteURI: \"<uri>\" EncryptionIn: \"<onoff>\" EncryptionOut: \"<onoff>\""
- id: fecc_action
  type: event
  format: "*e FeccActionInd Id: <id> Req: <n> Pan: <n> PanRight: <n> Tilt: <n> TiltUp: <n> Zoom: <n> ZoomIn: <n> Focus: <n> FocusIn: <n> Timeout: <n> VideoSrc: <n> m: <n>"
- id: tstring_received
  type: event
  format: "*e TString CallId: <id> Message: \"<s>\""
- id: sstring_received
  type: event
  format: "*e SString String: \"<s>\" Id: <id>"
```

## Macros
```yaml
# UNRESOLVED: source does not document multi-step macro sequences specific to the TC72.
```
...
interlocks: []
# UNRESOLVED: source mentions "MotorMoveDetection" warnings about manual camera adjustment losing pan/tilt feedback (C-Series), but no general safety interlocks are documented.
```

## Notes
- All API commands are case-insensitive (`xCommand`, `XCOMMAND`, `xcommand` all valid).
- Values containing spaces must be quoted (e.g. `xCommand Dial Number: "my number"`).
- Search shortcuts (`xconf vid`) should NOT be used in applications — use complete paths.
- Feedback subscriptions are per-session (RS-232/Telnet/SSH); max 38 expressions per session, up to 4 HTTP feedback slots. Avoid `xFeedback register /Status` — too much data.
- Output modes: `xPreferences outputmode xml|terminal`; default terminal.
- HTTP POST must set `Content-Type: text/xml`; the response is an XML document with `command="True"` results (e.g. `<CameraPositionSetResult item="1" status="OK"/>`).
- `xCommand Key Press` / `xCommand Key Click` emulate the on-screen display — discouraged; use direct commands instead.
- Experimental sections (`xCommand Experimental`, `xConfiguration Experimental`, `xStatus Experimental`) are not backwards-compatible; do not use in production.
- Feedback via HTTP includes an `Identification` block with `SystemName`, `MACAddress`, `IPAddress`, `ProductType`, `ProductID`, `SWVersion`, `HWBoard`, `SerialNumber` so one server can handle multiple codecs.

<!-- UNRESOLVED: default SIP ListenPort, HTTP/HTTPS port numbers, and Telnet port not stated in source. -->
<!-- UNRESOLVED: cipher suites, TLS versions, and SSH key exchange algorithms not stated in source. -->
<!-- UNRESOLVED: full HttpFeedback Register argument list — source shows 1-15 Expression slots but truncates the exact slot range. -->
