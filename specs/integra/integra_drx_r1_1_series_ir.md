---
spec_id: admin/integra-drx-r1-1-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "Integra DRX-R1.1 Series Control Spec"
manufacturer: Integra
model_family: "DRX-R1.1 Series"
aliases: []
compatible_with:
  manufacturers:
    - Integra
...
retrieved_at: 2026-06-25T11:40:58.531Z
last_checked_at: 2026-06-25T12:08:15.723Z
generated_at: 2026-06-25T12:08:15.723Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "command catalogue is complete for the DRX-R1.1 family as documented, but per-source applicability (XM/SIRIUS/HD Radio/Net-Tune variants) is conditional on hardware build and not enumerated here"
  - "persistent settable parameters (e.g. master volume, zone"
  - "no consolidated event table in source; the SST family"
  - "no multi-step sequences described explicitly in source."
  - "no explicit safety warnings, interlocks, or power-on"
verification:
  verdict: verified
  checked_at: 2026-06-25T12:08:15.723Z
  matched_actions: 160
  action_count: 160
  confidence: medium
...
# Integra DRX-R1.1 Series Control Spec

## Summary
AV receiver controlled via the ISCP (Integra Serial Control Protocol) over both 3-wire RS-232C and TCP/IP (eISCP) transports. This spec covers the system/amplifier, tuner, network/net-tune, multi-zone (Zone 2/3/4), and dock/RI command catalogue as documented in the vendor's ISCP reference (v1.15, 2009-08-31). The protocol is point-to-point: a single client maintains one persistent TCP connection to receive unsolicited status updates.

<!-- UNRESOLVED: command catalogue is complete for the DRX-R1.1 family as documented, but per-source applicability (XM/SIRIUS/HD Radio/Net-Tune variants) is conditional on hardware build and not enumerated here -->

## Transport
```yaml
# ISCP is offered over BOTH RS-232C and Ethernet. The source documents
# both transports in detail - emit both protocol groups.
protocols:
  - serial
  - tcp
serial:
...
**RS-232C (ISCP over RS-232):**
- 3-wire, DB-9 female (pin 2 TX, pin 3 RX, pin 5 GND)
- Straight-through cable to PC
- Frame: `!1<CMD><PARAM>[CR|LF|CRLF]` — controller → device
- Status frame: `!1<SST><CODE>[EOF]` — device → controller
- `[EOF]` = ASCII 0x1A

**Ethernet (eISCP over TCP):**
- Destination port **60128** default; receiver configurable 49152–65535 via setup menu (requires standby cycle to apply)
- One persistent client connection required to receive unsolicited status notifications
- Inter-message interval: ≥ 50 ms (eISCP notes); FFW/REW Net-Tune requires ≤ 100 ms between codes
- eISCP frame:
  - Header: `ISCP` (4) + Header Size `0x00000010` (4, BIGENDIAN) + Data Size (4, BIGENDIAN) + Version `0x01` (1) + Reserved `0x000000` (3)
  - Data: `!1<CMD><PARAM>[EOF|EOFCR|EOFCRLF]`

## Traits
```yaml
powerable: true   # PWR / ZPW / PW3 / PW4 commands present
routable: true    # SLI / SLR / SLA / SLZ / SL3 / SL4 input routing commands present
queryable: true   # QSTN queries present across the catalogue
levelable: true   # MVL / ZVL / VL3 / VL4 / SWL / CTL volume + level commands present
```
...
## Variables
```yaml
# UNRESOLVED: persistent settable parameters (e.g. master volume, zone
# volumes, listening mode, sleep timer) are modeled as parameterized
# actions in the source rather than discrete variables. No separate
# "variable" semantic is documented.
```

## Events
```yaml
# Per §2.3, the device sends unsolicited status messages on state change.
# ISCP status messages are framed as `!1<SST><CODE>[EOF]` (eISCP variant
# appends [CR][LF] after [EOF]). The SST mnemonic varies by domain.
# UNRESOLVED: no consolidated event table in source; the SST family
# is implicit across the per-command QSTN response codes.
```
...
# sequencing requirements documented in the source.
# Note: zone volume commands (ZVL/VL3/VL4) are documented as only
# functional when the main zone is ON ("only works when main is ON"
# for ZVL) - this is a documented interlock between zones.
```

## Notes

- **Protocol version:** ISCP v1.15, dated 2009-08-31. Header Size is fixed at `0x00000010`; ISCP version byte is `0x01`. Source is the 2003–2009 ONKYO CORPORATION reference.
- **Unit type identifier:** `1` = Receiver. The full frame is `!1<CMD><PARAM>...`.
- **Two transports, same command set:** the same mnemonics (PWR, MVL, SLI, etc.) work over both RS-232C ISCP and eISCP/TCP; only the framing differs.
- **Connection persistence (eISCP):** a single TCP client must hold one continuous connection to receive unsolicited notifications; only one client may be connected at a time. Disconnect drops the notification stream.
- **Timing:**
  - Receiver responds within 50 ms; otherwise the exchange has failed.
  - ≥ 50 ms gap required between consecutive eISCP messages.
  - FFW / REW Net-Tune commands (`NTC FF`, `NTC REW`) must be sent continuously with no more than 100 ms between codes.
- **Wrap-around commands:** many selectors (SLI, LMD, DIM, LTN, ADY, ADV, DVL, MOT, RAS, etc.) accept `UP` / `DOWN` to cycle; some accept `TG` (toggle) or `DIM` (dimmer cycle).
- **QSTN convention:** every documented command that has observable state also documents a `QSTN` query for it. The QSTN response format mirrors the set codes (e.g. `MVL00`–`MVL64` for volume, `PWR00`/`PWR01` for power).
- **12V Triggers (TGA/TGB/TGC):** per source, only available when each 12V Trigger parameter is set to `OFF` in the Setup Menu. No `QSTN` form is documented.
- **Tuner / network sub-features:** TUN, PRS, RDS, PTS, TPS, XCN/XAT/XTI/XCH/XCT (XM), SCN/SAT/STI/SCH/SCT/SLK (SIRIUS), HAT/HCN/HTI/HDS/HPR/HBL/HTS (HD Radio), and NTC/NAT/NAL/NTI/NTM/NTR/NST/NPR are conditional on the hardware build. The source documents them as part of the family but flags "Tuner Pack Model Only", "XM Model Only", "SIRIUS Model Only", "HD Radio Model Only", "Net-Tune Model Only", or "Network Model Only" prefixes. The DRX-R1.1 family applicability for each is not stated in the source.
- **Display Information (DIF):** two distinct `DIF` tables appear in the source — "Display Information" (program/format/digital-input/digital-format/bass/treble readouts, no `QSTN`) and "Display Mode" (display-mode selector with `QSTN`). Treat as two separate command surfaces sharing the same mnemonic.
- **SPA vs SPB:** the speaker-A/B command uses the same code table but the meaning switches between "MAIN A / MAIN B" and "Front A / Front B" — exclusive. `QSTN` does not distinguish which pair is active.
- **SLC (Speaker Level Calibration):** no `QSTN` documented. Commands are key-equivalents (TEST, CHSEL, UP, DOWN).
- **OSD (Setup Operation):** no `QSTN` documented. Commands are menu navigation key-equivalents.
- **MEM (Memory Setup):** no `QSTN` documented. Commands are STR / RCL / LOCK / UNLK.
- **CDS (Dock via RI):** RI-coupled control surface for a docking station; no `QSTN` documented. Operational only when a compatible RI dock is attached.
