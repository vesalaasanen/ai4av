...
retrieved_at: 2026-06-25T09:37:58.363Z
last_checked_at: 2026-06-25T11:26:00.000Z
generated_at: 2026-06-25T11:26:00.000Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "firmware compatibility range not stated; specific DRX sub-models covered by each command (tuner/XM/SIRIUS/HD Radio/NET-TUNE variants) not enumerated"
  - "source documents enum-typed parameters (volume 0x00-0x64, balance"
  - "source does not enumerate a distinct event-id space separate"
  - "source defines no multi-step macro sequences."
  - "no further safety warnings, interlock procedures, or power-on"
  - "voltage/current/power draw, error-recovery sequences, protocol version beyond ISCP v1, full DRX model sub-family coverage — none stated in source."
verification:
  verdict: verified
  checked_at: 2026-06-25T11:26:00.000Z
  matched_actions: 160
  action_count: 160
  confidence: medium
  summary: "deterministic presence proof: 160/160 payloads verbatim in source; stratified Sonnet sample corroborated (6 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
## Summary
ISCP (Integra Serial Control Protocol) for the DRX-Series AV receivers. Two transports: 3-wire RS-232C at 9600/8/N/1 (ASCII messages terminated by CR/LF/CRLF), and TCP/IP via eISCP wrappers to port 60128 (default; configurable 49152-65535). Point-to-point Command/Question/Event-Notice model; unsolicited status messages pushed on state change.

<!-- UNRESOLVED: firmware compatibility range not stated; specific DRX sub-models covered by each command (tuner/XM/SIRIUS/HD Radio/NET-TUNE variants) not enumerated -->
...
- Serial: 9-pin female D-sub, pin 2 TX, pin 3 RX, pin 5 GND, straight-through cable to PC.
- ISCP message frame: `!` (start) + `1` (unit type, receiver) + 3-char command mnemonic + variable-length parameter + end char. End char on RS-232 = CR, LF, or CRLF. End char on eISCP = EOF (0x1A), optionally followed by CR/LF.
- eISCP packet: 16-byte header (header size 0x10 BIGENDIAN, data size BIGENDIAN, version 0x01, reserved 0x000000) + ISCP message body.
- Receiver expects a continuously held TCP connection; only one client connection at a time. Receive interval must exceed 50 msec; FFW/REW Net-tune codes must be sent continuously with no more than 100 ms gap. Receiver responds within 50 msec or the communication has failed.

## Traits
```yaml
- powerable       # inferred from PWR, ZPW, PW3, PW4 commands
- routable        # inferred from SLI, SLR, SLA, SLZ, SL3, SL4, HDO, RES, VOS commands
- queryable       # inferred from QSTN queries across all command families
- levelable       # inferred from MVL, ZVL, VL3, VL4, TFR/TCT/TSR/TSB/TSW, ZTN, TN3, SWL, CTL, ZBL, BL3 commands
```
...
## Variables
```yaml
# UNRESOLVED: source documents enum-typed parameters (volume 0x00-0x64, balance
# -A..00..+A, tone ±10 in 2-step increments, etc.); no parameter entries
# required by template since each is part of its parent action.
```

## Events
```yaml
# Receiver pushes unsolicited status messages on state change via the held TCP
# connection. Each event mirrors a command's status response (e.g. power,
# volume, input, listening mode, tuner frequency, zone power/volume/selector).
# UNRESOLVED: source does not enumerate a distinct event-id space separate
# from QSTN responses; receiving a status frame without an outstanding query
# is the only event-vs-response distinguisher.
```

## Macros
```yaml
# UNRESOLVED: source defines no multi-step macro sequences.
```
...
interlocks:
  - "Zone2 volume control only functions when main zone is ON (source: ZVL note)."
  - "12V triggers TGA/TGB/TGC require each trigger parameter to be set OFF at Setup Menu (source: TGA/TGB/TGC note)."
# UNRESOLVED: no further safety warnings, interlock procedures, or power-on
# sequencing requirements stated in source.
```

## Notes
- ISCP = Integra Serial Control Protocol. Two transports share the same command vocabulary: raw RS-232C (3-wire, 9600/8/N/1) and eISCP over TCP (port 60128 default, range 49152-65535).
- The unit-type byte in the ISCP frame is hard-coded to `1` (receiver) for this device family.
- `SLK` (SIRIUS Parental Lock) takes a 4-digit password and emits INPUT/WRONG display prompts — model the response as two distinct feedback strings rather than a success/failure ack.
- Tone commands (TFR/TCT/TSR/TSB/TSW/ZTN/TN3) use a packed response format `BxxTxx` for QSTN (bass followed by treble); SLI/HDO/RES et al. use single-byte parameter responses.
- `SLP` sleep timer range is `0x01`–`0x5A` = 1–90 minutes (hex).
- Volume scale is 0–100 (`0x00`–`0x64`) across MVL, ZVL, VL3, VL4.
- `HTS` HD Radio tuner status is a 3-byte packed field (mm=HD/not, nn=current program 1–8, oo=receivable program bitmask in hex).
- Net-tune `FF`/`REW` (NTC) require sustained transmission, ≤100 ms between codes, to be interpreted as continuous.
- Several command families are model-conditional: XM (XCN/XAT/XTI/XCH/XCT), SIRIUS (SCN/SAT/STI/SCH/SCT/SLK), HD Radio (HAT/HCN/HTI/HDS/HPR/HBL/HTS), Net-Tune (NTC), RDS (RDS/PTS/TPS). The protocol catalog is shared but receivers will NAK commands they don't implement.
<!-- UNRESOLVED: voltage/current/power draw, error-recovery sequences, protocol version beyond ISCP v1, full DRX model sub-family coverage — none stated in source. -->
