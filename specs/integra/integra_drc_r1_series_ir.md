---
spec_id: admin/integra-drc-r1-series
schema_version: ai4av-public-spec-v1
revision: 1
title: "Integra DRC-R1. Series Control Spec"
...
retrieved_at: 2026-06-25T11:47:45.643Z
last_checked_at: 2026-06-25T12:08:17.532Z
generated_at: 2026-06-25T12:08:17.532Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "model string contains trailing dot (DRC-R1.) — preserved verbatim from source. No firmware range stated. No hardware specs (voltage/current) in source."
  - "source documents command parameters as code tables, not as named"
  - "source does not document multi-step sequences."
  - "source contains no safety warnings, interlock procedures, or"
  - "model identifier trailing dot preserved as in source. Firmware compatibility range not stated. No hardware electrical specs."
verification:
  verdict: verified
  checked_at: 2026-06-25T12:08:17.532Z
  matched_actions: 160
  action_count: 160
  confidence: medium
...
# Integra DRC-R1. Series Control Spec

## Summary
AV receiver / processor exposing ISCP (Integra Serial Control Protocol) over 3-wire RS-232C (9600/8/N/1) and over TCP via eISCP wrapping (default port 60128). Spec covers power, muting, volume, input select, listening mode, tone, dimmer, sleep, speakers, display, memory, info, RECOUT/audio selectors, 12V triggers, video output, HDMI out, resolution, ISF, Re-EQ/Academy, Audyssey (2EQ/MultEQ/DSX/Dynamic EQ/Dynamic Volume), Dolby Volume, Music Optimizer, tuner/preset/RDS, XM/SIRIUS/HD Radio metadata, Net-Tune/Network playback, and Zone 2/3/4 control, plus RI dock commands.

<!-- UNRESOLVED: model string contains trailing dot (DRC-R1.) — preserved verbatim from source. No firmware range stated. No hardware specs (voltage/current) in source. -->
...
## Traits
```yaml
- powerable       # PWR / ZPW / PW3 / PW4 + standby
- routable        # SLI / SLR / SLA / SLZ / SL3 / SL4 / HDO / VOS
- queryable       # QSTN on virtually every command
- levelable       # MVL / ZVL / VL3 / VL4 / TFR/TCT/TSR/TSB/TSW / ZTN/TN3 / SWL/CTL / ZBL/BL3
```
...
## Variables
```yaml
# UNRESOLVED: source documents command parameters as code tables, not as named
# settable variables distinct from their parent command. No free-standing
# variable definitions in source.
```

## Events
```yaml
# Source describes unsolicited status messages sent when device state changes
# (Section 2.1, 2.3). Each state-change notification reuses the same QSTN-style
# response format (e.g. "NSTprs"). Not separately enumerated; track via
# Feedbacks tied to the corresponding query action.
```
...
## Notes
- ISCP message format: `!1CMD<param>[CR|LF|EOF]` (3 fixed command chars, variable-length params). `!` start, `1` unit type (Receiver), end with `[CR]`/`[LF]`/`[CR][LF]` over RS-232 or `[EOF]`/`[EOF][CR]`/`[EOF][CR][LF]` over eISCP.
- eISCP wraps the ISCP message in a 16-byte header (0x10 bytes) with big-endian header-size / data-size / version=0x01 / reserved=0x000000, followed by ISCP data terminated with `[EOF]`.
- Interval between messages must be ≥ 50ms; Receiver responds within 50ms or communication is considered failed.
- Hold one persistent TCP connection — Receiver only sends unsolicited status updates to a continuously-connected client; only one client connection supported.
- FF/REW Net-Tune commands must be sent continuously with ≤ 100ms between codes.
- 12V Triggers TGA/TGB/TGC only available when each 12V Trigger parameter is "OFF" in the Setup Menu.
- TUNER/XM/SIRIUS/HD Radio is shared between MAIN and ZONE (control separated). ZVL only works when main is ON.
- VOS command is Japanese Model Only.
- Some commands are conditional on hardware: `RDS`/`PTS`/`TPS` RDS-only, `XCN`/`XAT`/`XTI`/`XCH`/`XCT` XM-only, `SCN`/`SAT`/`STI`/`SCH`/`SCT`/`SLK` SIRIUS-only, `HAT`/`HCN`/`HTI`/`HDS`/`HPR`/`HBL`/`HTS` HD Radio-only, `TUN`/`PRS` Tuner Pack-only.
- Volume "00"–"64" hex = 0–100 decimal.

<!-- UNRESOLVED: model identifier trailing dot preserved as in source. Firmware compatibility range not stated. No hardware electrical specs. -->
