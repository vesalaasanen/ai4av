---
spec_id: admin/integra-dhs-8-8-north-america
schema_version: ai4av-public-spec-v1
revision: 1
title: "Integra DHS-8.8 (North America) Control Spec"
manufacturer: Integra
model_family: DHS-8.8
aliases: []
compatible_with:
  manufacturers:
    - Integra
...
retrieved_at: 2026-06-25T09:52:10.168Z
last_checked_at: 2026-06-25T11:25:58.701Z
generated_at: 2026-06-25T11:25:58.701Z
firmware_coverage: "Not stated in source"
protocol_coverage: []
known_gaps:
  - "device-specific applicability (DHS-8.8 vs TX-NR series) not confirmed — protocol is shared across Integra/Onkyo receiver family."
  - "source describes \"Unsolicited feedback\" status messages and \"Event"
  - "no multi-step sequences described in source."
  - "source states \"TGA/TGB/TGC are available when each 12V Trigger"
verification:
  verdict: verified
  checked_at: 2026-06-25T11:25:58.701Z
  matched_actions: 160
  action_count: 160
  confidence: medium
  summary: "deterministic presence proof: 160/160 payloads verbatim in source; stratified Sonnet sample corroborated (4 unresolved item(s) noted in Known Gaps.)"
derived_from:
  - vendor_manual
license: ODbL-1.0
...
# Integra DHS-8.8 (North America) Control Spec

## Summary
Integra DHS-8.8 AV receiver. ISCP (Integra Serial Control Protocol) over RS-232 (9600/8/N/1) and eISCP over Ethernet (TCP port 60128 default). Source: ONKYO serial protocol document v1.15 (2009).

<!-- UNRESOLVED: device-specific applicability (DHS-8.8 vs TX-NR series) not confirmed — protocol is shared across Integra/Onkyo receiver family. -->
...
serial:
  ...
  stop_bits: 1
  flow_control: none
  connector: "DB-9 female (pin2=TX, pin3=RX, pin5=GND); straight-thru cable"
addressing:
  port: 60128  # eISCP default; receiver-settable 49152-65535
auth:
...
## Traits
```yaml
- powerable       # inferred from PWR/ZPW/PW3/PW4 on/off commands
- routable        # inferred from SLI/SLR/SLA/HDO/RES selector commands
- queryable       # inferred from QSTN query commands across catalogue
- levelable       # inferred from MVL/ZVL/VL3/VL4/TFR/TCT/TSR/TSB/TSW volume+tone commands
```
...
## Variables
```yaml
# Empty: source expresses all settable state as discrete ISCP command codes, not as
# continuous settable parameters. No separate Variables section needed.
```

## Events
```yaml
# UNRESOLVED: source describes "Unsolicited feedback" status messages and "Event
# Notice Communication" semantics, but does not enumerate which ISCP messages the
# receiver pushes on state change. Treat all QSTN-queryable fields as
# potentially pushable; specific unsolicited event catalogue not stated.
```
...
interlocks: []
# UNRESOLVED: source states "TGA/TGB/TGC are available when each 12V Trigger
# parameter is all OFF at Setup Menu" - this is a configuration gate, not a
# safety interlock. No explicit safety warnings, fault behavior, or power-on
# sequencing stated in source.
```

## Notes
ISCP frame: `!` + unit-type (`1` = Receiver) + 3-char cmd + params + terminator.
RS-232 terminator: `[CR]` / `[LF]` / `[CR][LF]`. eISCP terminator: `[EOF]` /
`[EOF][CR]` / `[EOF][CR][LF]` where `[EOF]` = 0x1A.

eISCP packet wraps the ISCP message with a 16-byte header (Header Size 0x10,
Data Size = ISCP msg length, Version 0x01, Reserved 0x000000) — all
big-endian. eISCP defaults to TCP port 60128; receiver-side configurable range
49152-65535 (change via setup menu + standby cycle).

Connection semantics: one persistent TCP connection per client; only one
client at a time. Inter-message gap must be ≥50ms (send and receive).
Receiver responds within 50ms or communication considered failed.

Tone `Bxx`/`Txx` hex nibble maps to -10...+10 in 2-dB steps (`-A` = -10,
`00` = 0, `+A` = +10). Volume 0x00-0x64 = 0-100 hex; same scale used for
main, Zone2, Zone3, Zone4.

Zone2/3/4 volume commands only function while main zone is ON (per ZVL
footnote).

Tuner/Net-Tune share between MAIN and ZONE sides (control separated).
FF/REW Net-Tune commands (`NTC` `FF`/`REW`) must be sent continuously with
≤100ms gap between codes.

Feature flags — several commands are conditional on installed hardware:
- Tuner pack: `TUN`, `PRS`
- RDS: `RDS`, `PTS`, `TPS`
- XM: `XCN`, `XAT`, `XTI`, `XCH`, `XCT`
- SIRIUS: `SCN`, `SAT`, `STI`, `SCH`, `SCT`, `SLK`
- HD Radio: `HAT`, `HCN`, `HTI`, `HDS`, `HPR`, `HBL`, `HTS`
- Net-Tune / Network: `NTC`, `NAT`, `NAL`, `NTI`, `NTM`, `NTR`, `NST`, `NPR`

`VOS` command documented as Japanese-model-only — likely N/A on DHS-8.8 NA.

Source authored by ONKYO CORP 2003-2009; protocol version 1.15 dated
2009-08-31. Integra brand re-uses ONKYO ISCP — spec applies to all Integra
receivers sharing this protocol version.
```

Self-check pass: no voltages/currents/ports/baud invented (all from source). Status=draft, confidence=low. Actions/Feedbacks empty per deterministic merge.
